...
' ------------------------------------------------------------------------------
	Private Function GetData(ByRef pvDataAry)
		...
		Dim lnRowIndex,lnRowCount,lnColCount,lnColIndex,lsHTML,lsClass,lsItemStock
		Dim lvPrimaryValue,lnMax,lsData,iCounter,iChkCnt,iEntNo
		...
		lnColCount = mnDisplayFieldCount - 1
	 	For lnRowIndex = 0 To lnRowCount
			...
			lsItemStock = pvDataAry(3,lnRowIndex)
			...
			For lnColIndex = 0 To lnColCount
				...
				ElseIF cint(lnColIndex) = cint(iCnt) then ' or  lnColIndex = 3
					...
					lsData = pvDataAry(9, lnRowIndex)
					lsItmCode = pvDataAry(5, lnRowIndex)
					lsClass = pvDataAry(6, lnRowIndex)
					...
					'response.write "<font color=red>stock = "& lsItemStock
					iCtr = 1
						...
						do while not dcrs.EOF
							...
							lsHTML = lsHTML & "<OPTION value="""& dcrs(0) &"#0:"&dcrs(1)&""" selected>"&dcrs(1)&"</OPTION>"
								...
								with objrs
									...
									.CursorType = 3
									.ActiveConnection = con
									'.Source = "Select A.ItemTypeAttributeID,A.ItemTypeAttributeName,O.OptionValue,O.OptionName from INV_M_ItemTypeAttributes as A,Inv_M_ItemTypeOptions as O where O.ItemTypeAttributeID = A.ItemTypeAttributeID and ItemTypeID = '"& dcrs(2) &"' and A.ItemTypeAttributeID = "& dcrs(0)
			...
			'***
			    ...
			    lsHTML = lsHTML & "<INPUT type=""hidden"" name=""hMarketPrice"&lsItmCode&lsClass&""" value=0>"
				End IF
				lsHTML = lsHTML & "</TD>"
			Next
...
' ------------------------------------------------------------------------------
	Private Function GetFooter()
		...
		'sChkBut = DisableBut
		''Added By Ragav On March 21,2012
		sChkBut = "Y"
