...
%>
<%
	'Program Name				:	clsDatalistParSubTypeMultiple.asp
	'Module Name				:
...
' ------------------------------------------------------------------------------
	Public Function GetTable(ByRef poDatabase)
		...
		lsTable = lsTable & _
			"<table border=""0"" width=""100%"" cellspacing=""0"" cellpadding=""0"" class=""ExcelTable"">" & _
				"<tr>" & _
...
Function XmlFun(Obj)
	...
	if sSelectMode = "M" then
		if Obj.checked then
			...
			node1.SetAttribute "ParType",Arr1(4)
			node1.SetAttribute "ParSubType",Arr1(3)
			node1.SetAttribute "ParCode",Arr1(1)
			node1.SetAttribute "ParName",Arr1(0)			
			Root.appendchild node1
		else
				for each temp in Root.childnodes			
					if Strcomp(temp.nodename,"PartyDetails")=0 then
						if trim(Temp.getAttribute("ParCode")) =  trim(Arr1(1)) and trim(Temp.getAttribute("ParSubType")) =  trim(Arr1(3)) and trim(Temp.getAttribute("ParType")) =  trim(Arr1(4))  then
			...
			if Root.haschildnodes then
				for each temp in Root.childnodes
					if ucase(temp.nodename) = ucase("PartyDetails") then
						...
						if trim(Temp.getAttribute("ParCode")) =  trim(TempArr(1)) and trim(Temp.getAttribute("ParSubType")) =  trim(TempArr(3)) and trim(Temp.getAttribute("ParType")) =  trim(TempArr(4))  then
		...
		for i = 0 to document.FormName.elements.length - 1
			if document.FormName.elements(i).type = "checkbox"   then
				if document.FormName.elements(i).name = "pKey"   then
					...
					TempArr = split(n1,":")
					'	alert n1 		
					if trim(Arr1(0)) = trim(TempArr(1))  then 
...
window.__itmsDataListCompatConfig = {
	...
	attrs: [
		{ name: "ParType", index: 4 },
		{ name: "ParSubType", index: 3 },
		{ name: "ParCode", index: 1 },
