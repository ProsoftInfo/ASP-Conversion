...
Function XmlFun(Obj)
	...
	if sSelectMode = "M" then
		if Obj.checked then
			set node1 = ObjTemp.createElement("AccHead")
			node1.SetAttribute "AccHeadCode",Arr1(0)
			node1.SetAttribute "AccHeadName",Arr1(5)
			node1.SetAttribute "BalAmount",Arr1(7)
			Root.appendchild node1
		else
				for each temp in Root.childnodes
			
					if Strcomp(temp.nodename,"AccHead")=0 then
						if trim(Temp.getAttribute("AccHeadCode")) =  trim(Arr1(0))  then
	...
	for i = 0 to document.FormName.elements.length - 1
		if document.FormName.elements(i).type = "checkbox" then
			if Root.haschildnodes then
				for each temp in Root.childnodes
					if ucase(temp.nodename) = ucase("AccHead") then
						n1 = trim(document.FormName.elements(i).value)
						TempArr = split(n1,":")
						if trim(Temp.getAttribute("AccHeadCode")) =  trim(TempArr(0)) then
							document.FormName.elements(i).checked=true
							exit for
						end if 'if trim(temp.getAttribute("AccHeadCode")) = trim(TempArr(0)) then
						
					end if 'if Strcomp(temp.nodename,"AccHead")= 0 then
				next
		...
		for i = 0 to document.FormName.elements.length - 1
			if document.FormName.elements(i).type = "checkbox"   then
				if document.FormName.elements(i).name = "pKey"   then
					...
					if trim(Arr1(0)) = trim(TempArr(0))  then
	...
	if Root.haschildnodes then
		for each temp in Root.childnodes
			if ucase(temp.nodename) = ucase("AccHead") then
				...
				s1= trim(s1) & "<input type=checkbox name=chk value='" & trim(temp.getAttribute("AccHeadCode")) &"' checked onClick=RemoveNode(this) >" 
				s1= trim(s1) & "</td>"
				s1= trim(s1) & "<td class=ExcelDisplayCell >" & trim(temp.getAttribute("AccHeadCode")) & "</td>"
				s1= trim(s1) & "<td class=ExcelDisplayCell >" & trim(temp.getAttribute("AccHeadName")) & "</td>"
...
window.__itmsDataListCompatConfig = {
	...
	nodeName: "AccHead",
	attrs: [
		{ name: "AccHeadCode", index: 0 },
		{ name: "AccHeadName", index: 5 },
		{ name: "BalAmount", index: 7 }
	],
	keyAttrs: ["AccHeadCode"],
	keyIndexes: [0],
	removeAttrs: ["AccHeadCode"],
	removeIndexes: [0],
