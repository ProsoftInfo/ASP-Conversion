...
%>
<%
	'Program Name				:	clsDatalistPartyMultiple.asp
	'Module Name				:
	...
	Private Function GetFooter()
		...
		If mnLastPage >= 2 Then
			...
			If mnLastPage >= 2 Then
				...
				lsPageJump = "<SELECT class=""FormElem"" onChange=""showpage('" & lsQueryString & "' + this[this.selectedIndex].value)"" id=select1 name=select1>"
		...
		lsHTML = lsHTML & "<INPUT type=""Hidden"" name=""hButtPress"" value="""" >"
		lsHTML = lsHTML & "</TD>"
		lsHTML = lsHTML & "</TR>"
...
Function XmlFun(Obj)
	...
	if sSelectMode = "M" then
		if Obj.checked then
			set node1 = ObjTemp.createElement("PartyDetails")
			node1.SetAttribute "ParType",Arr1(5)
			node1.SetAttribute "ParSubType",Arr1(4)
			node1.SetAttribute "ParCode",Arr1(2)
			node1.SetAttribute "ParName",Arr1(0)
			node1.SetAttribute "BalAmount",Arr1(1)
			Root.appendchild node1
		else
				for each temp in Root.childnodes			
					if Strcomp(temp.nodename,"PartyDetails")=0 then
						if trim(Temp.getAttribute("ParCode")) =  trim(Arr1(2)) and trim(Temp.getAttribute("ParSubType")) =  trim(Arr1(4)) and trim(Temp.getAttribute("ParType")) =  trim(Arr1(5))  then
	...
	for i = 0 to document.FormName.elements.length - 1
		if document.FormName.elements(i).type = "checkbox" then
			if Root.haschildnodes then
				for each temp in Root.childnodes
					if ucase(temp.nodename) = ucase("PartyDetails") then
						n1 = trim(document.FormName.elements(i).value)
						TempArr = split(n1,":")
						
						if trim(Temp.getAttribute("ParCode")) =  trim(TempArr(2)) and trim(Temp.getAttribute("ParSubType")) =  trim(TempArr(4)) and trim(Temp.getAttribute("ParType")) =  trim(TempArr(5))  then
							document.FormName.elements(i).checked=true
							exit for
						end if 'if trim(temp.getAttribute("ParCode")) = trim(TempArr(1)) then
						
					end if 'if Strcomp(temp.nodename,"PartyDetails")= 0 then
				next
...
function RemoveNode(this)
	...
	if this.checked = false then
		for each temp in Root.childnodes
			...
			if Strcomp(temp.nodename,"PartyDetails")=0 then
				if trim(Temp.getAttribute("ParCode")) =  trim(Arr1(0)) and trim(Temp.getAttribute("ParSubType")) =  trim(Arr1(1)) and trim(Temp.getAttribute("ParType")) =  trim(Arr1(2))  then
					Root.Removechild temp
				end if	
			end if
		next
		...
		for i = 0 to document.FormName.elements.length - 1
			if document.FormName.elements(i).type = "checkbox"   then
				if document.FormName.elements(i).name = "pKey"   then
					...
					TempArr = split(n1,":")
					'	alert n1 		
					if trim(Arr1(0)) = trim(TempArr(2))  then 
	...
	if Root.haschildnodes then
		for each temp in Root.childnodes
			if ucase(temp.nodename) = ucase("PartyDetails") then
				...
				s1= trim(s1) & "<input type=checkbox name=chk value='" & trim(temp.getAttribute("ParCode")) & ":" & trim(temp.getAttribute("ParSubType")) & ":" & trim(temp.getAttribute("ParType")) &"' checked onClick=RemoveNode(this) >" 
				s1= trim(s1) & "</td>"
				s1= trim(s1) & "<td class=ExcelDisplayCell >" & trim(temp.getAttribute("ParCode")) & "</td>"
				s1= trim(s1) & "<td class=ExcelDisplayCell >" & trim(temp.getAttribute("ParName")) & "</td>"
				s1= trim(s1) & "<td class=ExcelDisplayCell >" & trim(temp.getAttribute("BalAmount")) & "</td>"
				s1= trim(s1) & "</tr>"
			end if 'if Strcomp(temp.nodename,"PartyDetails")= 0 then
		next
	end if 'if Root.haschildnodes then
...
function showpage(sArguments)
	...
	'alert(sret)
	document.formname.hButtPress.value = "Page"
	'sButtonPressed = 
	set Root = ObjTemp.DocumentElement
...
Function sendValue()
	'sButtonPressed = "Done"
	document.formname.hButtPress.value =  "Done"
	set Root = ObjTemp.DocumentElement
...
Function window_onunload() 
	...
	sButtonPressed = document.formname.hButtPress.value
...
window.__itmsDataListCompatConfig = {
	...
	attrs: [
		{ name: "ParType", index: 5 },
		{ name: "ParSubType", index: 4 },
		{ name: "ParCode", index: 2 },
		{ name: "ParName", index: 0 },
		{ name: "BalAmount", index: 1 }
	],
	keyAttrs: ["ParCode", "ParSubType", "ParType"],
	keyIndexes: [2, 4, 5],
	removeAttrs: ["ParCode", "ParSubType", "ParType"],
	removeIndexes: [0, 1, 2],
	displayAttrs: ["ParCode", "ParName", "BalAmount"]
};
</SCRIPT>
