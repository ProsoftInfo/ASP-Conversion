...
Function Validate()
	Dim sProcessCode,sPracticeCode,sActivityName
	sProcessCode  = Trim(document.formname.selProcess.value)
	sPracticeCode = Trim(document.formname.selActivityType.value)

	sActivityName = Trim(document.formname.txtActivityName.value)

	document.formname.hProcessName.value = sProcessCode
	document.formname.hPracticeName.value = sPracticeCode
	document.formname.hActivityName.value = sActivityName
	document.formname.selApplication.value = sProcessCode
...
Function ChkReset()
	document.formname.TxtProcessName.value  = ""
	document.formname.TxtProcessName.value  = ""
	document.formname.txtActivityName.value = ""
	document.formname.submit()
End Function

Function Paginate(nPage)
	document.formname.hPageSelection.value = nPage
	document.formname.selApplication.value = document.formname.hAppCode.value
	document.formname.selProcess.value = document.formname.hAppCode.value
	document.formname.submit()
End function
...
Function ShowRoleActivityMapOld(sCallFrom,sTempVal)
	...
	For iCtr = 1 to nlength
		...
		If check Then
			If Trim(n1) <> "" Then
				...
				nActvityCode = Split(sTemp,":")(0)
				sActvityName = Split(sTemp,":")(1)
			End If
	...
	IF iNoOfSelect > 1 then
		alert("Select any one Activty")
		Exit Function
	end if

	If iNoOfSelect = "0" and trim(sCallFrom) <> "ADD" Then
		alert("Select Activity")
		Exit Function
	End IF


	If Trim(sCallFrom) = "ADD" Then
		sTemp = Trim(sCallFrom) & ":" & sTempVal
		set OutValue = showModalDialog ("ActivityMappingPopUp.asp?PassData="&sTemp,"","dialogHeight:220px;dialogWidth:520px;center:Yes;help:No;resizable:No;status:No;scroll:no")
	End If

	If Trim(sCallFrom) = "EDT" Then
		sTemp = Trim(sCallFrom) & ":" & sTempVal & ":" & nActvityCode & ":" & sActvityName
		set OutValue = showModalDialog ("ActivityMappingPopUp.asp?PassData="&sTemp,"","dialogHeight:220px;dialogWidth:520px;center:Yes;help:No;resizable:No;status:No;scroll:no")
	End If
...
Function ShowRoleActivityMap(sCallFrom,sTempVal)
	...
	If Trim(sCallFrom) = "ADD" Then
		set OutValue = showModalDialog ("ActivityMappingPopUp.asp?PassData="&sTemp,"","dialogHeight:450px;dialogWidth:520px;center:Yes;help:No;resizable:No;status:No;scroll:no")
	End If
...
Function ShowDetails(sType,sTemp)
	...
	set OutValue = showModalDialog ("AppActivityDetailsPopUp.asp?PassData="&sTemp,"","dialogHeight:320px;dialogWidth:450px;center:Yes;help:No;resizable:No;status:No;scroll:no")
...
Function DelActivity()
	...
	Dim sPassData,iCtr,nActvityCode,nAppCode,nProcessCode
	...
	For iCtr = 1 to nlength
		...
		If check Then
			If Trim(n1) <> "" Then
				...
				nActvityCode = Split(sTemp,":")(0)
				sActvityName = Split(sTemp,":")(1)
				nAppCode     = Split(sTemp,":")(2)
				nProcessCode = Split(sTemp,":")(3)
				nRoleID		 = Split(sTemp,":")(4)
			End If
			iNoOfSelect = iNoOfSelect + 1
		End IF
	Next
	...
	IF iNoOfSelect > 1 then
		alert("Select any one Activty For Delete")
		Exit Function
	end if

	If iNoOfSelect = "0" Then
		alert("Select Activity For Delete")
		Exit Function
	End IF

	sPassData = nRoleID & ":" & nAppCode & ":" & nProcessCode & ":" & nActvityCode

	document.formname.action= "ActivityDelete.asp?sData="&sPassData
	document.formname.submit

	'set objHttp = CreateObject("Microsoft.XMLHTTP")
	'objHttp.open "POST","ActivityDelete.asp?sData="&sPassData,False
	'objHttp.send

	'If objHttp.responseText <> "" Then
	'	alert(objHttp.responseText)
	'Else
	'	alert("Selected Activity Deleted")
	'	document.formname.submit()
	'End IF
...
Function AssaignApplicationCode(nAppcode)
	document.formname.hAppCode.value = nAppcode
	document.formname.submit
End Function
