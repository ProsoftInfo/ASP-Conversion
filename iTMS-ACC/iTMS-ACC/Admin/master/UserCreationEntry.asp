...
<script language="vbscript" src="../../scripts/GetPopUpWindowSize.vbs"></script>
...
Function SelectEmployee()
    ...
    sUserAccessMode = document.formname.hUAM.value
    
        if trim(sUserAccessMode)="I" then

            sTempValWindowSize = GetWindowSizeForPopup("8")
    	    ...
    	    Set	EmpDataXML = showModalDialog("../../Common/"&sProgramName,"","dialogHeight:"& sPopupHeight &"px;dialogWidth:"& sPopupWidth &"px;Status:No")
		    sAct = UCase(trim(EmpDataXML.getAttribute("Action")))
		    sQuery = trim(EmpDataXML.getAttribute("PassQuery"))
		    if ucase(trim(sAct)) <> "CLOSE" then
			    do while sAct <> "DONE"
				    set EmpDataXML = showModalDialog("../../Common/"&sProgramName&"?"&sQuery,"","dialogHeight:"& sPopupHeight &"px;dialogWidth:"& sPopupWidth &"px;Status:No")
				    sAct = UCase(trim(EmpDataXML.getAttribute("Action")))
		    ...
		    set ndRoot = EmpDataXML
		    'alert(ndRoot.xml)
		    if ndRoot.hasChildNodes() then
		        For each ndChild in ndRoot.childNodes
		            sEmpID = ndChild.getAttribute("RetField1")
		            sEmpCode = ndChild.getAttribute("RetField2")
		            sEmpName = ndChild.getAttribute("RetField3")
		            sArrName=split(sEmpName,"-")
		            document.formname.hCode.value = sEmpID
    		        
		            document.formname.txtEmployeeID.value = sEmpCode
		            document.formname.txtFName.value = sArrName(0)
		            document.formname.txtMName.value = sArrName(1)
		            document.formname.txtLName.value = sArrName(2)
    		        
		            set objhttp = CreateObject("Microsoft.XMLHTTP")
		            objhttp.open "GET","../../Common/XMLGetEmpAddress.asp?EMPID="&sEmpID,false
		            objhttp.send
		            'alert(objhttp.responseText)
		            if trim(objhttp.responseXML.xml)<>"" then
		                EMPADDXML.loadXML(objhttp.responseXML.xml)
		            end if
		            set ndAddRoot = EMPADDXML.documentElement
		            if ndAddRoot.hasChildNodes() then
		                for each ndEmpAdd in ndAddroot.childNodes
		                    if ndEmpAdd.nodeName="EmpAdd" then
		                        sWorkPhone = ndEmpAdd.getAttribute("WorkPhone")
                                sMobilePhone = ndEmpAdd.getAttribute("MobilePhone")
                                sWorkEmail = ndEmpAdd.getAttribute("WorkEmail")
                                sAdd1 = ndEmpAdd.getAttribute("Add1")
                                sAdd2 = ndEmpAdd.getAttribute("Add2")
                                sCity = ndEmpAdd.getAttribute("City")
                                sState = ndEmpAdd.getAttribute("State")
                                sPostal = ndEmpAdd.getAttribute("Postal")
                                sCountry = ndEmpAdd.getAttribute("Country")
                                
							    document.formname.txtStreet.value = sAdd1
							    document.formname.txtAddr2.value = sAdd2 
							    document.formname.txtCity.value = sCity
							    document.formname.txtState.value = sState
							    document.formname.txtPostal.value = sPostal
							    document.formname.txtWorkPhone.value = sWorkPhone
							    document.formname.txtWorkEmail.value = sWorkEmail
							    document.formname.txtCell.value = sMobilePhone
                                exit for
		                    end if
		                next
		            end if
		    ...
		    sTempValWindowSize = GetWindowSizeForPopup("9")
            sArrTempValWindowSize = split(sTempValWindowSize,":")
            sProgramName = sArrTempValWindowSize(0)
            sPopupHeight = sArrTempValWindowSize(1)
            sPopupWidth = sArrTempValWindowSize(2)
		    ...
		    if ndRoot.hasChildNodes() then
		        For each ndChild in ndRoot.childNodes
		            sEmpID = ndChild.getAttribute("RetField1")
		            sEmpCode = ndChild.getAttribute("RetField2")
		            sEmpName = ndChild.getAttribute("RetField0")
		            
		            document.formname.txtEmployeeID.value = sEmpCode
		            document.formname.txtFName.value = sEmpName
		            document.formname.hCode.value = sEmpID
		            
		            set objhttp = CreateObject("Microsoft.XMLHTTP")
		            objhttp.open "GET","../../Include/XMLGetPartyAddress.asp?PartyCode="&sEmpID,false
		            objhttp.send
		            'alert(objhttp.responseText)
		            if trim(objhttp.responseXML.xml)<>"" then
		                EMPADDXML.loadXML(objhttp.responseXML.xml)
		            ...
		            if ndAddRoot.hasChildNodes() then
		                for each ndEmpAdd in ndAddroot.childNodes
		                    if ndEmpAdd.nodeName="Party" then
		                        sWorkPhone = ndEmpAdd.getAttribute("PhoneNo")
                                sMobilePhone = ndEmpAdd.getAttribute("MobileNo")
                                sAdd1 = ndEmpAdd.getAttribute("Address1")
                                sAdd2 = ndEmpAdd.getAttribute("Address2")
                                sCity = ndEmpAdd.getAttribute("City")
                                sState = ndEmpAdd.getAttribute("State")
                                sCountry = ndEmpAdd.getAttribute("Country")
