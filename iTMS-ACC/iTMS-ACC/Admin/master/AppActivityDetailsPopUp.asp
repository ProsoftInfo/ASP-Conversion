...
<SCRIPT LANGUAGE=vbscript>
	...
	Function addRow()
		...
		headerCell.className  = "ExcelInputCell"
		headerCell.align="Center"
		Set oText = document.createElement("<input type=""TEXT"" name='txtOrder" & iCtr&"' onBlur=""RowAdd("&iCtr&")"" size=""11""    class=""FormElem"" maxlength=""4"" >")
		headerCell.appendChild(oText)
		...
		document.formname.hNoOfRows.value  = iCtr
		'alert(document.formname.hItemRows.value)
	End Function

	Function RowAdd(iCtr)
		If Eval("document.formname.txtPractice"&iCtr).value = "" Then
			'alert("Enter practice Name")
			Exit Function
		Elseif Eval("document.formname.txtOrder"&iCtr).value = "" Then
			'alert("Enter Order Number")
			Exit Function
		Elseif Not IsNumeric(Eval("document.formname.txtOrder"&iCtr).value) Then
	...
	Function SaveData(sCallFrom)
		...
		'alert("IR"& " " & document.formname.hItemRows.value )
		If sCallFrom = "ADDPRACTICE" Then
			'nRows = nRows + 1
			'alert("NR"& " " &document.formname.hNoOfRows.value)
			nRows = document.formname.hNoOfRows.value
