...
Function SetDates(obj)
	if obj.selectedIndex = 0 then
		idPFinEndDate.innerHTML = "&nbsp;"
		idCFinStartDate.innerHTML = "&nbsp;"
		idCFinEndDate.innerHTML = "&nbsp;"
		exit function
	end if
	...
	idPFinEndDate.innerHTML = arrTemp(1) & "&nbsp;"

	arrSYear = split(arrTemp(0),"/")
	idCFinStartDate.innerHTML = arrSYear(0) & "/" & arrSYear(1) & "/" & arrSYear(2)+1 & "&nbsp;"
	arrEYear = split(arrTemp(1),"/")
	idCFinEndDate.innerHTML = arrEYear(0) & "/" & arrEYear(1) & "/" & arrEYear(2)+1 & "&nbsp;"
...
Function NoSeriesSubmit()
    dim sRetVal
    iSelCount = 0
	nRowCtr = document.formname.hRowCtr.value
	For iCnt = 0 to nRowCtr - 1
	    if eval("document.formname.radModule("&iCnt&")").Checked then
	        iSelCount = iSelCount + 1
	        iSelValue = eval("document.formname.radModule("&iCnt&")").value
	    end if 'if eval("document.formname.radModule("&iCnt&")").Checked then
	Next
	if iSelCount = 0 then
	    alert("Select Module")
	    exit function
	...
	if iSelCount = 1 then
	    'alert(iSelValue)
	    sArrSelValue = Split(iSelValue,"#")

	    sWhat = sArrSelValue(1)
	    iApp = sArrSelValue(2)
		sUnit = sArrSelValue(4)
		sOrgName = sArrSelValue(3)
		dPreFinStartDate = sArrSelValue(5)
		dPreFinEndDate = sArrSelValue(6)
		dCFinStartDate = sArrSelValue(7)
		dCurFinEndDate = sArrSelValue(8)
		sTemp =  sUnit &"||"& sOrgName &"||"& dCFinStartDate  &"||"& dCurFinEndDate  &"||"&  dPreFinStartDate &"||"& dPreFinEndDate &"||"& iApp

		sRetVal = MsgBox("Do U Want to Proceed?",4,"Accounts Closing")
	    If sRetVal = 7 then exit function

		if sWhat = "1" then
		    ShowModalDialog "TransferClosingPopUp.ASP?Para="&sTemp,"","dialogHeight:450px;dialogWidth:300px;center:Yes;status:no"
		end if
	    document.formname.action = "CloseEntry.asp?Frm=NS"
...
Function AccSubmit()
	...
	nRowCtr = document.formname.hRowCtr.value
	For iCnt = 0 to nRowCtr - 1
	    if eval("document.formname.radFinPeriod("&iCnt&")").Checked then
	        iSelCount = iSelCount + 1
	        iSelValue = eval("document.formname.radFinPeriod("&iCnt&")").value
	    end if 'if eval("document.formname.radFinPeriod("&iCnt&")").Checked then
	Next
	if iSelCount = 0 then
	...
	if iSelCount = 1 then
	    sArrSelValue = Split(iSelValue,"#")
	    document.formname.selPFinStartDate.value  = sArrSelValue(0)
	    sArrSelFinPeriod = split(sArrSelValue(0),":")
	    sArrSelFromPerid = split(sArrSelFinPeriod(0),"/")
	    sArrSelToPerid = split(sArrSelFinPeriod(1),"/")
	    iCurrFromDate = sArrSelFromPerid(0) &"/"& sArrSelFromPerid(1)&"/"& sArrSelFromPerid(2)+1
	    iCurrToDate = sArrSelToPerid(0) &"/"& sArrSelToPerid(1) &"/"& sArrSelToPerid(2)+1
	end if

	sRetVal = MsgBox("Do U Want to Proceed?",4,"Accounts Closing")
	If sRetVal = 7 then exit function

	document.formname.action = "AccountsClosingNew.asp?UnitCode="& sArrSelValue(1)&"&UnitName="&sArrSelValue(2)&"&PrevFromDate="& sArrSelFinPeriod(0)&"&PrevToDate="&sArrSelFinPeriod(1)&"&CurrFromDate="&iCurrFromDate&"&CurrToDate="& iCurrToDate
	document.formname.hOrgName.value = sArrSelValue(2)
	document.formname.hCFinStartDate.value = iCurrFromDate
	document.formname.hCFinEndDate.value = iCurrToDate
	document.formname.submit
...
Function StockSubmit()
	...
	nRowCtr = document.formname.hRowCtr.value
	For iCnt = 0 to nRowCtr - 1
	    if eval("document.formname.radIType("&iCnt&")").Checked then
	        iSelCount = iSelCount + 1
	        iSelValue = eval("document.formname.radIType("&iCnt&")").value
	    end if 'if eval("document.formname.radIType("&iCnt&")").Checked then
	Next
	...
	sRetVal = MsgBox("Do U Want to Proceed?",4,"Stock Closing")
	If sRetVal = 7 then exit function

	document.formname.action = "InvClosingStockDetailsEntry.asp?CategoryCodes="&sArrSelValue(1)&"&PrevFromDate="& sArrSelFinPeriod(0)&"&PrevToDate="& sArrSelFinPeriod(1)&"&CurrFromDate="& iCurrFromDate&"&CurrToDate="& iCurrToDate &"&UnitCode="&sArrSelValue(3)&"&UnitName="& sArrSelValue(2)
	document.formname.submit
...
Function CheckSubmit()
	...
	if document.formname.selUnit.selectedIndex = "0" then
		...
		exit Function
	elseif document.formname.selPFinStartDate.selectedIndex = "0" then
		alert("Previous Financial Year Start Date")
		document.formname.selPFinStartDate.focus
		exit Function
	End IF
	sRetVal = MsgBox("Do U Want to Proceed?",4,"Transfer Closing")

	If sRetVal = 7 then exit function
	IF document.formname.hFor.Value = "NS" Then
		document.formname.action = "TransferClosingDetailsEntryNew.asp?Frm=NS"
		document.formname.hOrgName.value = document.formname.selUnit(document.formname.selUnit.selectedIndex).text
		document.formname.hCFinStartDate.value = idCFinStartDate.innerText
		document.formname.hCFinEndDate.value = idCFinEndDate.innerText
		document.formname.submit
		Exit Function
	Elseif document.formname.hFor.Value = "IS" Then
		document.formname.action = "TransferClosingDetailsEntryNew.asp?Frm=IS"
		document.formname.hOrgName.value = document.formname.selUnit(document.formname.selUnit.selectedIndex).text
		document.formname.hCFinStartDate.value = idCFinStartDate.innerText
		document.formname.hCFinEndDate.value = idCFinEndDate.innerText
		document.formname.submit
		Exit Function
	Elseif document.formname.hFor.Value = "PROLOSS" Then
		document.formname.action = "GLPartyUpdate.asp"
		document.formname.hOrgName.value = document.formname.selUnit(document.formname.selUnit.selectedIndex).text
		document.formname.hCFinStartDate.value = idCFinStartDate.innerText
		document.formname.hCFinEndDate.value = idCFinEndDate.innerText
		document.formname.submit
		Exit Function
	Else
		document.formname.action = "AccountsClosingNew.asp"
		document.formname.hOrgName.value = document.formname.selUnit(document.formname.selUnit.selectedIndex).text
		document.formname.hCFinStartDate.value = idCFinStartDate.innerText
		document.formname.hCFinEndDate.value = idCFinEndDate.innerText
		document.formname.submit
...
Function ItemStockSetup()
    showModalDialog "ItemStockCloseSetup.asp","","dialogWidth:600px;dialogHeight:600px;Status:No"
    document.formname.submit
End Function
</SCRIPT>
</HEAD>
