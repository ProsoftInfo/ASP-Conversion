...
Function GetLot(obj,str,iCtr,sStoName)
	...
	Set OutDataValue = showModalDialog("receiptLotSerPop.asp?sTemp="&sTempValues,objTemp,"dialogHeight:370px;dialogWidth:750px;center:Yes;help:No;resizable:No;status:No")
				...
				if HeaderNode.HaschildNodes() then
					For Each HNode In HeaderNode.childNodes
						if HNode.Attributes.Item(0).nodeValue = iStore and HNode.Attributes.Item(1).nodeValue = iBin then
							...
							Q.value = cdbl(FormatNumber(HNode.Attributes.Item(2).nodeValue,3,,,0))
						end if
					next
				end if
			end if 
...
Function GetLotDetails(iRecNo,iItem,iClass,sOrgID,sFlag)
	...
	'alert "Ok"
	objhttp.Open "GET","XMLGetLotDetails.asp?iRecNo="&iRecNo&"&iItem="&iItem&"&iClass="&iClass&"&sOrgID="&sOrgID&"&sFlag="&sFlag&"&sRecType="&document.formname.hRec.value, false
