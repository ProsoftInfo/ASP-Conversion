...
Function CheckSubmit(todaysdate)
	...
	Set Root = OutData1.documentElement
		if trim(document.formname.hUsage.value)="SUB" then
		...
		elseif trim(document.formname.hUsage.value)="DIS" then
		...
		elseif trim(document.formname.hUsage.value)="SER" or trim(document.formname.hUsage.value)="JWK" then
