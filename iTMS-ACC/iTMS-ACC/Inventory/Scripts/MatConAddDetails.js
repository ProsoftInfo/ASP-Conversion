(function (window, document) {
	...
	function isMaterialUsage() {
		return usage === "MAT";
	}

	function findItemDetails() {
		var items = elementChildren(root, "ItemDet");
		for (var i = 0; i < items.length; i += 1) {
			if (attr(items[i], "Class") === trim(classCode) &&
					attr(items[i], "Item") === trim(itemCode) &&
					attr(items[i], "IssEntryNo") === trim(issueEntryNo)) {
				return items[i];
			}
		}
		return null;
	...
	window.fnInit = function (sOrg, sClass, sItm, sUsag, sIssNo) {
		ensureCompat();
	...
	window.CheckEntry = function () {
		...
		if (mc && mc.selectedIndex === 0 && mc.length > 1) {
	...
	window.CheckSubmit = function () {
		...
		if (isMaterialUsage()) {
			if (Math.abs(currentMachineTotal() - toNumber(textOf("idQty"))) > 0.000001) {
