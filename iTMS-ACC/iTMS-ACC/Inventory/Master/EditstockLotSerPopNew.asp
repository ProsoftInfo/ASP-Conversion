...
<%
	...
	bPackFlag = false
'Response.Write "Vales = "& Request.QueryString("sTemp")
...
'	 Response.write "sTemp:"&sTemp
	...
	if iItem = "0" then
		sItemName = arrTemp(10)
	else
		sItemName = ItemDisplay(iItem,"NULL")
	end if
	...
	with dcrs
		.CursorLocation = 3
		.CursorType = 3
		'.Source = "SELECT PACKINGCODE,PACKINGSHORTNAME,PACKINGNAME FROM APP_M_PACKINGTYPE WHERE PACKINGCODE IN (SELECT PACKINGCODE FROM SAL_R_ITEMTYPEPACK)"
		.Source = "SELECT PACKINGCODE,PACKINGSHORTNAME,PACKINGNAME FROM APP_M_PACKINGTYPE" 'WHERE PACKINGCODE IN (SELECT PACKINGCODE FROM SAL_R_ITEMTYPEPACK)"
		.ActiveConnection = con
		.Open
	end with
...
<script type="application/xml" id="AttributeData" data-itms-xml-island><Root/></script>
...
'***************************
Function GetAttributeListName(AttributeList,ItemType)
	Dim objhttp
	Dim ndRoot
	set objhttp = CreateObject("Microsoft.XMLHTTP")
	objhttp.open "GET","GetAttributeDetails.asp?sTemp="&AttributeList&":"&ItemType,false
	objhttp.send
	if objhttp.responseXML.xml<>"" then
		AttributeData.loadXML(objhttp.responseXML.xml)
	else
		alert(objhttp.responseText)
	end if

	set ndRoot = AttributeData.documentElement
	if ndRoot.hasChildNodes() then
		for each ndAtt in ndRoot.childNodes
			GetAttributeListName = ndAtt.getAttribute("Name")
		next
	end if
...
Function LoadDetails(obj)
	...
	dim cntr,NoofPacks, iSubVal,iTemp,Qty,iAttributeList,sAttributeListName,sItemType
	'alert(obj)
	arrTemp = split(obj,"``")
	sType = arrTemp(0)
	iItem = arrTemp(1)
	...
	For iCounter = 0 to StorageNode.Length - 1
		...
		iSubValRate = cDbl(StorageNode.Item(iCounter).Attributes.getNamedItem("IVALUE").Value) /  cDbl(StorageNode.Item(iCounter).Attributes.getNamedItem("QTY").Value)
...
'	alert StorageNode.Length
	For iCounter = 0 to StorageNode.Length - 1
		...
		iAttributeList = StorageNode.Item(iCounter).Attributes.getNamedItem("ATTRIBUTELIST").Value
	'	alert(sQtyRec)
	'	alert(sTar)
	'	alert(iSubValRate)

		sAttributeListName = GetAttributeListName(iAttributeList,sItemType)
		'alert(sAttributeListName)
		iSubVal = FormatNumber((CDbl(sQtyRec) - CDbl(sTar))* CDbl(iSubValRate),2,,,0)
		...
		If sType = "S" and iCnt = 0 Then
			'alert iTemp
				...
				'ALERT("iTotValue")
					set oText = document.createElement("<input type=""text"" name=""txtIValue"&CStr(iFixedNo)&""" size=""12"" value="""&iTemp&""" onBlur=""RecalculateTotal()"" class=""Formelem"" style=""text-align=right"" onkeypress=""DoKeyPress('"&sCheck&"',7,3)"">")
					headerCell.className="ExcelDisplayCell"
					headerCell.align="center"
					headerCell.width = "8"
					headerCell.appendChild(oText)

					set oText = document.createElement("<input type=""text"" name=""txtVal"&CStr(iFixedNo)&""" size=""12"" value="""&" ["&cDbl(iTotValue)\cDbl(Qty)&"]"&""" onBlur=""RecalculateTotal()"" READONLY class=""Formelem"" style=""text-align=Left"" onkeypress=""DoKeyPress('"&sCheck&"',7,3)"">")
					headerCell.className="ExcelDisplayCell"
					headerCell.align="center"
					headerCell.width = "8"
					headerCell.appendChild(oText)
		...
		if sTempLotNo <> sLotNo and sLotNo <> "" Then
			...
			'if document.formname.hrec.value <> "S" then
				...
				set oText = document.createElement("<input type=""text"" name=""txtVal"&CStr(iFixedNo)&""" size=""10"" value="""&" ["&cDbl(iTotValue)\cDbl(Qty)&"]"&""" onBlur=""RecalculateTotal()"" READONLY class=""Formelem"" style=""text-align=Left"" onkeypress=""DoKeyPress('"&sCheck&"',7,3)"">")
				headerCell.className="ExcelDisplayCell"
				headerCell.align="center"
				headerCell.width = "8"
				headerCell.appendChild(oText)
...
'		headerCell.width = "8"
'		headerCell.align="center"
		set oText = document.createElement("<input type=""hidden"" name=""txtSerial"&CStr(j)&""" value="""&sLotserialNo&""" >" )
		document.formname.appendchild(oText)

		set headerCell=oRow.insertCell()
		set oText = document.createElement("<input type=""text"" name=""txtGross"&CStr(j)&""" onBlur=""RecalculateQty()"" size=""12"" value="""&sQtyRec&""" maxlength=10 class=""Formelem"" style=""text-align=right"">" )
		headerCell.appendChild(oText)
		headerCell.width = "10"
		headerCell.className="ExcelInputCell"

		set headerCell=oRow.insertCell()
		set oText = document.createElement("<input type=""text"" name=""txtTare"&CStr(j)&""" onBlur=""RecalculateQty()"" size=""12"" value="""&sTar&""" maxlength=10 class=""Formelem"" style=""text-align=right"">")
		headerCell.className="ExcelInputCell"
		headerCell.width = "10"
		headerCell.appendChild(oText)

		set headerCell=oRow.insertCell()
		set oText = document.createElement("<SELECT name=""selPack"&CStr(j)&""" class=""FormElem"" >" )
		...
		setIndex eval("document.formname.selPack"&CStr(j)),sPack

		set headerCell=oRow.insertCell()
		set oText = document.createElement("<input type=""text"" name=""txtPackNo"&CStr(j)&""" size=""17"" value="""&iPackNo&""" maxlength=30 class=""Formelem"" style=""text-align=right"">" )
		headerCell.appendChild(oText)
		headerCell.width = "10"
		headerCell.className="ExcelInputCell"

		set headerCell=oRow.insertCell()
		set oText = document.createElement("<input type=""text"" name=""txtValue"&CStr(j)&""" size=""12"" value="""&iSubVal&"""  tabindex="&TabInd+5&" onBlur=""RecalculateTotal()"" class=""Formelem"" style=""text-align=right"" onkeypress=""DoKeyPress('"&sCheck&"',7,3)"">")
		headerCell.className="ExcelInputCell"
		headerCell.width = "10"
		headerCell.appendChild(oText)

		set headerCell=oRow.insertCell()
		set oText = document.createElement("<SELECT name=""selSell"&CStr(j)&""" class=""FormElem"" >" )
		...
		setIndex eval("document.formname.selSell"&CStr(j)),sSell

		set headerCell=oRow.insertCell()
		set oText = document.createElement("<input type=""text"" name=""txtSellWeight"&CStr(j)&""" size=""12"" maxlength=10 value="""&iWeight&""" class=""Formelem"" style=""text-align=right"">" )
		headerCell.appendChild(oText)
		headerCell.width = "10"
		headerCell.className="ExcelInputCell"

		'setIndex eval("document.formname.selForm"&CStr(j)),sSellingForm

	'	if document.formname.hrec.value="S" then
	'		set headerCell=oRow.insertCell()
	'		set oText = document.createElement("<input type=""text"" name=""txtIValue"&CStr(j)&""" size=""12"" value="""&iTempValue&""" maxlength=30 class=""Formelem"" style=""text-align=right"">" )
	'		headerCell.appendChild(oText)
	'		headerCell.width = "10"
	'		headerCell.className="ExcelInputCell"
	'	end if
		if ucase(document.formname.hItemType.value)="FAB" then
			set headerCell=oRow.insertCell()
			set oText = document.createElement("<input type=""button"" name=""BtnEnter"&CStr(j)&""" value=""Yes"" onClick=""ShowPopup(this)"" class=""AddButtonX"">" )
			headerCell.appendChild(oText)
			headerCell.align="center"
			headerCell.className="ExcelFieldCell"
	...
	set headerCell=oRow.insertCell()
		set oText = document.createElement("<SELECT name=""selForm"&CStr(j)&""" class=""FormElem"" >" )
		...
		set headerCell=oRow.insertCell()
		headerCell.innerText = sAttributeListName
		headerCell.className="ExcelDisplayCell"
		headerCell.align="center"
...
Function AddDetails(sCheck)
	...
	'alert STDate
	If (trim(document.formname.txtNoOfPacks.value)) = "" Then
	...
	ElseIf (trim(document.formname.txtRcptNumbering.value) = "LOT\SERIAL" or trim(document.formname.txtRcptNumbering.value) = "LOT") and (Trim(document.formname.txtLotNumber.value) = "" and Trim(document.formname.chkNew.checked=false)) Then
		...
		Exit Function
	ElseIf trim(document.formname.txtQtyIn.value) = "" Then
	...
	ElseIf Not checkNumbers(trim(document.formname.txtValue.value)) Then
		...
		document.formname.txtValue.focus
		Exit Function
	ElseIf trim(document.formname.selSellType.selectedIndex) > 0 and Trim(document.formname.txtWeight.value) = "" Then
		 alert "Enter W/t. Per form"
		 Exit Function
	ElseIf (document.formname.selPackType.selectedIndex > 0) and Trim(document.formname.txtWeightPerPack.value) = "" Then
		 alert "Enter W/t. Per Pack"
		 Exit Function
	End If
...
'
		'	set oText = document.createElement("<input type=""hidden"" name=""txtSerial"&CStr(j)&""" value="""&trim(document.formname.txtSerialFrom.value)&""" >" )
			set oText = document.createElement("<input type=""hidden"" name=""txtSerial"&CStr(j)&""" value="" >" )
			document.formname.appendchild(oText)
			...
			if document.formname.radQty(1).checked then
				set oText = document.createElement("<input type=""text"" name=""txtGross"&CStr(j)&""" size=""12"" onBlur=""RecalculateQty()"" value="""&trim(document.formname.txtQuantity.value)&""" class=""Formelem"" style=""text-align=right"" onkeypress=""DoKeyPress('"&sCheck&"',7,3)"">")
			elseif document.formname.radQty(0).checked then
				set oText = document.createElement("<input type=""text"" name=""txtGross"&CStr(j)&""" size=""12"" onBlur=""RecalculateQty()"" class=""Formelem"" style=""text-align=right"" onkeypress=""DoKeyPress('"&sCheck&"',7,3)"">")
			end if
			...
			set oText = document.createElement("<input type=""text"" name=""txtTare"&CStr(j)&""" size=""12"" value=""0"" READONLY class=""FormelemRead"" style=""text-align=right"" onkeypress=""DoKeyPress('"&sCheck&"',7,3)"">")
			headerCell.className="ExcelDisplayCell"
			...
			if (document.formname.radWeight(1).Checked=true  and document.formname.radWeight(1).disabled=false) then
				set oText = document.createElement("<input type=""text"" name=""txtSellWeight"&CStr(j)&""" size=""12"" value="""&trim(document.formname.txtWeight.value)&""" class=""Formelem"" style=""text-align=right"">")
			else
				set oText = document.createElement("<input type=""text"" name=""txtSellWeight"&CStr(j)&""" size=""12"" maxlength=10 class=""Formelem"" style=""text-align=right"">" )
			end if
			headerCell.className="ExcelInputCell"
			headerCell.appendChild(oText)
			headerCell.width = "10"
			...
			set headerCell=oRow.insertCell()
			'if sType <> "L" then
			'	set oText = document.createElement("<input type=""text"" name=""txtPackNo"&CStr(j)&""" value="" size=""17"" maxlength=30 class=""Formelem"" onBlur=""PackNoLocalCheck1(this,"&CStr(j)&")"" style=""text-align=left"">" )
			'elseif sType = "L" then
				set oText = document.createElement("<input type=""text"" name=""txtPackNo"&CStr(j)&""" value="""&trim(document.formname.txtLotNumber.value)&""" size=""17"" maxlength=30 class=""Formelem"" onBlur=""PackNoLocalCheck1(this,"&CStr(j)&")"" style=""text-align=left"">" )
			'end if
			headerCell.className="ExcelInputCell"
			headerCell.appendChild(oText)
			headerCell.width = "10"
			...
			if ucase(document.formname.hItemType.value)="FAB" then
				set headerCell=oRow.insertCell()
				set oText = document.createElement("<input type=""button"" name=""BtnEnter"&CStr(j)&""" onClick=""ShowPopup(this)""  value=""Yes"" class=""AddButtonX"">" )
				headerCell.appendChild(oText)
				headerCell.align="center"
				headerCell.className="ExcelFieldCell"
...
'					set oText = document.createElement("<input type=""text"" name=""txtIValue"&CStr(j)&""" size=""12"" value="""&iTempVal&""" onBlur=""RecalculateTotal()"" class=""Formelem"" style=""text-align=right"" onkeypress=""DoKeyPress('"&sCheck&"',7,3)"">")
'					headerCell.className="ExcelInputCell"
'				end if
'
'				headerCell.width = "10"
...
'			end if
			...
			eval("document.formname.txtGross"&CStr(j)).focus
...
'					set oText = document.createElement("<input type=""text"" name=""txtIValue"&CStr(iFixedNo)&""" size=""12"" value="""&cDbl(document.formname.txtValue.value)*cDbl(document.formname.txtNoOfPacks.value) &""" onBlur=""RecalculateTotal()"" class=""Formelem"" style=""text-align=right"" onkeypress=""DoKeyPress('"&sCheck&"',7,3)"">")
					set oText = document.createElement("<input type=""text"" name=""txtIValue"&CStr(iFixedNo)&""" size=""12"" value="""&NewVal&""" onBlur=""RecalculateTotal()"" class=""Formelem"" style=""text-align=right"" onkeypress=""DoKeyPress('"&sCheck&"',7,3)"">")
					headerCell.className="ExcelDisplayCell"
					headerCell.align="center"
					headerCell.width = "100"
					headerCell.appendChild(oText)

					set oText = document.createElement("<input type=""text"" name=""txtVal"&CStr(iFixedNo)&""" size=""10"" value="""&" ["&cDbl(document.formname.txtValue.value)&"]"&""" onBlur=""RecalculateTotal()"" class=""Formelem"" style=""text-align=Left"" onkeypress=""DoKeyPress('"&sCheck&"',7,3)"">")
					headerCell.className="ExcelDisplayCell"
					headerCell.align="center"
					headerCell.width = "100"
					headerCell.appendChild(oText)
			...
			elseif sLotNo <> document.formname.txtLotNumber.value then
				...
				If Not LotExist(sLotNo) Then
					...
					set oText = document.createElement("<input type=""text"" name=""txtVal"&CStr(iFixedNo)&""" size=""12"" value="""&" ["&document.formname.txtValue.value&"]"&""" onBlur=""RecalculateTotal()"" class=""Formelem"" style=""text-align=right"" onkeypress=""DoKeyPress('"&sCheck&"',7,3)"">")
					headerCell.className="ExcelDisplayCell"
		...
		for j = No  to No + iLotNo -1
			...
			set headerCell=oRow.insertCell()
			set oText = document.createElement("<input type=""text"" name=""txtGross"&CStr(j)&""" size=""12"" tabindex="&TabInd+1&" onkeypress=""DoKeyPress('"&sCheck&"',7,3)"" value="""&trim(document.formname.txtWeightPerPack.value)&""" class=""Formelem"" style=""text-align=right"">")
			headerCell.className="ExcelInputCell"
			headerCell.width = "10"
			headerCell.appendChild(oText)

			set headerCell=oRow.insertCell()
				set oText = document.createElement("<input type=""text"" name=""txtTare"&CStr(j)&""" size=""12"" tabindex="&TabInd+2&"  onkeypress=""DoKeyPress('"&sCheck&"',7,3)"" value="""&trim(document.formname.txtPackTare.value)&""" class=""Formelem"" style=""text-align=right"">")
				headerCell.className="ExcelInputCell"
			headerCell.width = "10"
			headerCell.appendChild(oText)
			...
			setIndex eval("document.formname.selPack"&CStr(j)),document.formname.selPackType.options(document.formname.selPackType.selectedIndex).Value
			...
			If autogen = True and (trim(document.formname.txtRcptNumbering.value) = "LOT" or trim(document.formname.txtRcptNumbering.value)="LOT\SERIAL"  or trim(document.formname.txtRcptNumbering.value)="SERIAL") Then
				...
				sSrlNo = AutoSerial
				set headerCell=oRow.insertCell()
				set oText = document.createElement("<input type=""text"" name=""txtPackNo"&CStr(j)&""" maxlength=30 class=""Formelem"" Value="&sSrlNo&" tabindex="&TabInd+4&" onBlur=""PackNoLocalCheck1(this,"&CStr(j)&")"" style=""text-align=right"">" )
				headerCell.className="ExcelInputCell"
				headerCell.appendChild(oText)
				headerCell.width = "10"
'				FType = "SL"
...
'				End If
			Else
				set headerCell=oRow.insertCell()
				'set oText = document.createElement("<input type=""text"" name=""txtPackNo"&CStr(j)&""" value="&CalculatePackNo(k)&" size=""17"" maxlength=30 class=""Formelem"" onBlur=""PackNoLocalCheck1(this,"&CStr(j)&")"" style=""text-align=left"">" )
				set oText = document.createElement("<input type=""text"" name=""txtPackNo"&CStr(j)&""" maxlength=30 class=""Formelem""  tabindex="&TabInd+4&" onBlur=""PackNoLocalCheck1(this,"&CStr(j)&")"" style=""text-align=right"">" )
				headerCell.className="ExcelInputCell"
				headerCell.appendChild(oText)
				headerCell.width = "10"
			End If
...
'				elseif document.formname.radValue(1).checked then
					...
					set oText = document.createElement("<input type=""text"" name=""txtValue"&CStr(j)&""" size=""12"" value="""&SValue&"""  tabindex="&TabInd+5&" onBlur=""RecalculateTotal()"" class=""Formelem"" style=""text-align=right"" onkeypress=""DoKeyPress('"&sCheck&"',7,3)"">")
					headerCell.className="ExcelInputCell"
'				end if
	...
	'		end if
			...
			if ucase(document.formname.hItemType.value)="FAB" then
				...
				'	sQty = "G:" & document.formname.txtQtyIn.value
				'elseif document.formname.radQtyIn(1).checked then
					sQty = "N:" & document.formname.txtQtyIn.value
				'end if
			...
			set headerCell=oRow.insertCell()
			set oText = document.createElement("<input type=""text"" name=""txtSellWeight"&CStr(j)&""" size=""12""  tabindex="&TabInd+3&" value="""&trim(document.formname.txtWeight.value)&"""  class=""Formelem"" style=""text-align=right"">")
			headerCell.className="ExcelInputCell"
			headerCell.appendChild(oText)
			headerCell.width = "10"
			...
			'if (document.formname.radPackType(1).Checked=true and document.formname.radPackType(1).disabled=false) then
			'	SetIndex eval("document.formname.selPack"&CStr(j)),document.formname.selPackType.value
			'end if
			...
			sOldLotNo = sLotNo
		next
		eval("document.formname.txtGross"&CStr(No)).focus
...
''blocked by ragav on Feb 27,20120
''begin
'	if (cdbl(iQtyTotGross-iQtyTotTare) > cdbl(idQty.innerText)) then
'		ClearTable()
...
'		if HeaderNode.Attributes.Item(0).nodeValue = iStore and HeaderNode.Attributes.Item(1).nodeValue = iBin then
'			For Each LotNod In HeaderNode.childNodes
'				if StrComp(Trim(LotNod.NodeName),"LotSerial") = 0 then
'					set oRem = HeaderNode.removeChild(LotNod)
'				end if
'			next
'		end if
'	next
...
Dim sAttributeID
	sAttributeID = document.formname.hAttID.value
	if trim(sAttributeID)<>"NULL" and not IsNull(sAttributeID) and trim(sAttributeID)<>"" then
...
'	alert sPubLot
	...
	For Each StorageDet In Root.childNodes
	  For Each childNod in StorageDet.childNodes
		...
		if childNod.Attributes.Item(0).nodeValue = iStore and childNod.Attributes.Item(1).nodeValue = iBin then
			...
			'alert LotExist(sPubLot) & "LotExist"
				If LotExist(sPubLot) Then   'Lot Exist
					For Each GrandChildNod In ChildNod.childNodes  'GrandChild
					'alert cstr(GrandChildNod.attributes.getNamedItem("LOT").value) & "Lot"
					'alert cStr(sPubLot)  & "sPubLot"
						...
						if document.formname.hrec.value="S"  then set objValue = eval("document.formname.txtValue"&CStr(i))

						Set newElem1 = objTemp.createElement("LotSerialDetails")
					'				newElem1.setAttribute "SERIALNO", i
					'				newElem1.setAttribute "LOTSERIAL", trim(objSerial.value)

'						If autogen = True and (trim(document.formname.txtRcptNumbering.value)="LOT\SERIAL") Then
'							FType = "SL"
'							set objhttp = CreateObject("MSXML2.XMLHTTP")
'							objhttp.Open "GET","../../Inventory/transaction/LotSerialGenrate.asp?orgID="&sOrgID&"&iItem="&iItem&"&FTYPE="&FType&"&STDATE="&STDate,False
'							objhttp.send
'							If Trim(objhttp.responsetext) <> "" Then
'								sSrlNo = Trim(objhttp.responsetext)
'								'set oText = document.createElement("<input type=""text"" name=""txtIValue"&CStr(iFixedNo)&""" size=""12"" value="""&sSrlNo&""" onBlur=""RecalculateTotal()"" class=""Formelem"" style=""text-align=right"" onkeypress=""DoKeyPress('"&sCheck&"',7,3)"">")
'								newElem1.setAttribute "LOTSERIAL", sSrlNo
'							End If
'						Else
...
'						End If
						...
						newElem1.setAttribute "ATTRIBUTELIST",sAttributeID
...
'						ChildNod.appendChild GrandChildNod
					...
					'	GrandChildNod.setAttribute "ALTGROSS", document.formname.txtQtyIn.Value
					'	GrandChildNod.setAttribute "ALTNETT", cDbl(document.formname.txtQtyIn.Value) - cDbl(document.formname.txtTare.value)
			...
			'elseif document.formname.radTare(0).checked then
...
'				End If
		...
		'	alert document.formname.txtAltGross.value & "ok"
		...
		'	newElem.setAttribute "ALTGROSS", document.formname.txtQtyIn.Value
		'	newElem.setAttribute "ALTNETT", cDbl(document.formname.txtQtyIn.Value) - cDbl(document.formname.txtTare.value)
...
'			alert Root.XML
'			alert objTemp.xml
			'alert FabricData.xml

'			alert SerialData.xml

			'iQtyTotGrossSerial = 0
			'iQtyTotTareSerial = 0
...
'		document.formname.B4.disabled = true ' AddSerial
'		document.formname.B5.disabled = true ' AddLot
'		document.formname.B3.disabled = false 'AddDetails

		EnableDone()
'		alert Root.XML
		exit function
		end if
      Next 'For Each childNod in StorageDet.childNodes
	next ' for each childNod in Root.childNodes
'	alert Root.XML
end Function
...
Function CheckSubmit()
	...
	'end if
	''end
'	if Root.hasChildNodes then checkExists()
	...
	'alert objTemp.XML & "objtemp.XMlafter"
	'alert Root.XML  & "Root.XMLafter"
	iQtyTotGross=0
	iQtyTotTare=0
...
'	if ucase(document.formname.hItemType.value)<>"FAB" then
'		iCalQty = FormatNumber(CalculateLotQty,3)
...
'	end if
	...
	if (iCalQty <> iInnerQty) then
		Set Root = objTemp.documentElement
		for each HeaderNode in Root.childNodes
			if StrComp(Trim(HeaderNode.NodeName),"STOREDET") = 0 then
			    for each ndStorage in HeaderNode.childNodes
			        if ndStorage.attributes.getNamedItem("STORE").value = iStore and ndStorage.attributes.getNamedItem("BIN").value = iBin then
			            ndStorage.setAttribute "QTY",iCalQty
			            ndStorage.setAttribute "STORAGEVALUE",iTotalValue
...
Function AddSerial()
	...
	dim objQ,objD,objSerial,Root,objValue
	avail = True
	if document.all.tblLot.rows.length = 2 then
		alert("No Details Entered")
		exit function
	...
	for i = 1 to iCnt
		...
		if document.formname.hrec.value="S" then
			set objValue = eval("document.formname.txtValue"&CStr(i))
...
'		newElem.setAttribute "LOTSERIAL", trim(objSerial.value)
		...
		newElem.setAttribute "ATTRIBUTELIST",sAttributeID
		Root.appendChild newElem
	...
	'document.formname.txtSerialFrom.focus
...
'	sExp ="//STORAGE [ @STORE = "&iStore&" and @BIN = '"&iBin&"']/LotSerial/LotSerialDetails"
	...
	For i = 0 to Node.Length - 1
		If Node.Item(i).Attributes.getNamedItem("TAREREC").Value = "" then  Node.Item(i).Attributes.getNamedItem("TAREREC").Value = 0
...
Function UpdateValues()
	...
	For i = 1 to Node.Length
		Node.item(i-1).Attributes.getNamedItem("PACKNUMBER").value = eval("document.formname.txtPackNo"&CStr(i)).value
		Node.item(i-1).Attributes.getNamedItem("PACKINGTYPE").value = eval("document.formname.selpack"&CStr(i)).value
		Node.item(i-1).Attributes.getNamedItem("WEIGHTSTYPE").value = eval("document.formname.txtSellWeight"&CStr(i)).value
		Node.item(i-1).Attributes.getNamedItem("QTYREC").value =  eval("document.formname.txtGross"&CStr(i)).value
		Node.item(i-1).Attributes.getNamedItem("TAREREC").value = eval("document.formname.txtTare"&CStr(i)).value
		Node.item(i-1).Attributes.getNamedItem("SELLINGFORM").value  = eval("document.formname.selForm"&CStr(i)).value
	Next
	'alert Root.XML
	...
	Set Node = Root.Selectnodes(sExp)
	For i = 1 to Node.Length
			Node.item(i-1).Attributes.getNamedItem("IVALUE").value = eval("document.formname.txtIvalue"&CStr(i)).value
	Next
...
'	alert sExp
	...
	For i = 1 to Node.Length
		...
		if document.formname.hrec.value<>"S" then Node.item(i-1).Attributes.getNamedItem("IVALUE").value = eval("document.formname.txtIValue"&CStr(i)).value
	next
...
'			set sAttribute = objTemp.createAttribute("QTYREC")
'			sAttribute.value = cdbl(objQty.value)
'			Node.item(i-1).Attributes.setNamedItem sAttribute
'
'			set sAttribute = objTemp.createAttribute("TAREREC")
'			sAttribute.value = cdbl(objTare.value)
'			Node.item(i-1).Attributes.setNamedItem sAttribute
'
'			set sAttribute = objTemp.createAttribute("LOTSERIAL")
'			sAttribute.value = cdbl(objLotSerial.value)
'			Node.item(i-1).Attributes.setNamedItem sAttribute

'			Node.item(i-1).Attributes.getNamedItem("SERIALNO").value = i
'			Node.item(i-1).Attributes.getNamedItem("LOTSERIAL").value = cdbl(objLotSerial.value)
			...
			'if document.formname.hrec.value="S" then Node.item(i-1).Attributes.getNamedItem("IVALUE").value = cdbl(objValue.value)
...
Function ShowPopup(Obj)
	...
	sTemp = document.formname.sTemp.value & ":" & ObjQty.value & ":" & No &":"&document.formname.selAltUom.value&":"&document.formname.txtAltGross.value&":"&document.formname.txtAltNett.value
	'alert sTemp
...
Function CheckFabricData()
	...
	for each FabricHeaderNode in FabricRoot.childNodes
		i = i + 1
		set ObjBtn = eval("document.formname.BtnEnter"&CStr(i))
...
<input type="hidden" name="hAttID" value="<%=sAttributeID%>">
															...
															<table cellpadding="0" cellspacing="0">
																...
																<tr>
																    ...
																    <%	if ucase(sItmType) = "FAB" then %>
																        <input type="text" name="txtQtyIn" size="11" maxlength=10 onkeypress="DoKeyPress('<%=sCheck%>',7,3)" class="FormElem"  value="0">
																	<%	else %>
																		<input type="text" name="txtQtyIn" size="11" maxlength=10 onkeypress="DoKeyPress('<%=sCheck%>',7,3)" class="FormElem"  value="0">
																    <%	end if	%>
																    ...
																    <td class="FieldCellSub">
																		<input type="text" name="txtTare" size="11" maxlength=10 value="0" onkeypress="DoKeyPress('<%=sCheck%>',7,3)" class="FormElem"> <%=sStoresUom%>
																		&nbsp; Rate / <%=sStoresUom%> :&nbsp;&nbsp;
																		<input type="text" name="txtValue" size="10" value="0" class="FormElem">
																    </td>
																</tr>
																...
																<tr>
																	...
																	<td class="FieldCellSub">
																		...
																		&nbsp; No of Packs : &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
																		<input type="text" name="txtNoOfPacks" size="11" class="FormElem" value="0">
														            </td>
														        </tr>
																<tr>
																	<td class="FieldCell"></td>
														            ...
														            <td class="FieldCellSub">
																		<input type="text" name="txtWeightPerPack" size="11" class="FormElem" value="0">
																		&nbsp; Tare w/t per pack:&nbsp;
																		<input type="text" name="txtPackTare" size="11" class="FormElem" value="0">
																	</td>
																...
																<tr>
																	...
																	<td class="FieldCell">
																		<select size="1" name="selAttribute" class="FormElem">
																			<%
																				sQuery = "Select A.ItemTypeAttributeID,A.ItemTypeAttributeName,0 from  INV_M_ItemTypeAttributes as A,Inv_M_ItemTypeOptions as O where "&_
								...
								<td>
									<div class="frmBody" id="frm2" style="width: 100%; height:170;">
										<table border="0" cellspacing="1" id="tblLot" class="ExcelTable" width="100%">
											<tr>
											<%	if ucase(sItmType) = "FAB" then %>
												<td class="ExcelHeaderCell" align="Left" colspan="11" >Lot No. / Value [Rate/<%=sStoresUom%>]</td>
											<%	else	%>
											...
											<%	'end if%>
												...
												<td class="ExcelHeaderCell" align="center" rowspan="2">Attribute List</td>
...
<%
	...
	Function PopulateStage(sItmType)
		...
		with dcrs
			.CursorLocation = 3
			.CursorType = 3
			.Source = "SELECT STAGEID, STAGENAME FROM INV_M_STAGE WHERE (CATEGORYID IS NULL OR CATEGORYID = 'Q')"
			.ActiveConnection = con
			.Open
		end with
		set dcrs.ActiveConnection = nothing
