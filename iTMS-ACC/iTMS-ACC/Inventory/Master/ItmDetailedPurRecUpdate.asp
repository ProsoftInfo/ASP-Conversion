...
<%	option explicit	%>
<%
	'Program Name				:	ItmDetailedPurRecUpdate.asp
...
<%
    ...
    sCreatedBy = getuserId()
     ...
     if objFSO.FileExists(Server.MapPath("../temp/Master/PurchaseDetUpdate"&Session.SessionID&".xml")) then
        oDOM.load Server.MapPath("../temp/Master/PurchaseDetUpdate"&Session.SessionID&".xml")
            ...
            sQuery =" Delete from INV_M_ItemOrgAlternate where ItemCode = "& sItemCode &" and ClassificationCode ="& sClassCode &" and Organisationcode = "& sOrgCode 
            Response.Write "<p>"& sQuery
            con.execute sQuery
            
            sQuery = "Delete from INV_M_ItemOptionalUOM where ItemCode = "& sItemCode &" and ClassificationCode ="& sClassCode &" and Organisationcode = "& sOrgCode &" and OptionalUoMFor ='P'"
            Response.Write "<p>"& sQuery
            con.execute sQuery
            
            sQuery = "Delete from Inv_R_ItemSupplier where ItemCode = "& sItemCode &" and ClassificationCode ="& sClassCode &" and Organisationcode = "& sOrgCode 
            Response.Write "<p>"& sQuery
            con.execute sQuery
            
            sQuery = "Delete from INV_M_ItemOrgPurchase where ItemCode = "& sItemCode &" and ClassificationCode ="& sClassCode &" and Organisationcode = "& sOrgCode 
            Response.Write "<p>"& sQuery
            con.execute sQuery
            ...
            if ndEntry.length>0 then
                ...
                if trim(sBuyer)="" or IsNull(sBuyer) or trim(sBuyer)="select" then sBuyer = "NULL"
                if trim(sInvMatch)="select" or IsNull(sInvMatch) or trim(sInvMatch)="" then sInvMatch ="NULL"
                sQuery = "Insert into INV_M_ItemOrgPurchase (ItemCode,ClassificationCode,OrganisationCode,"&_
                         ...
                         " AllowShipTo,ItemDefinedBy,ItemDefinedOn,EarlyDays,LateDays,ReceiptDate) values"&_
                         ...
                         " "& sEnforceShipTo &","& sCreatedBy &",Convert(datetime,getDate(),103),"& sRecDaysEarly &","& sRecDaysLate &",'"& sRecDateAction &"')" 
