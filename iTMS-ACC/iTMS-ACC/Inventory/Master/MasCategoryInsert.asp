...
<%	option explicit	%>
<%
	'Program Name				:	MasCategoryInsert.asp	
...
<!--
	function msgbox(strr,flag) {
		if (flag == "Y") {
			alert(strr);
			if (confirm("Do You want to define another Category")) 
				window.location.href = "MasCategoryEntry.asp"
			else
				window.location.href = "MasUOMEntry.asp"
		}
		else {
			alert(strr);
...
dim dcrs,sSql
dim sCatCode,sCatName,sCatShName

sCatCode = trim(Request.Form("txtCatCode"))
