...
<%
	Dim dcrs,dcrs1,dcrs2,rsTemp,oDOM,oDOM1,oDOM2,Root,Root1, Node,Node1,Node2,Node3,Node4,sItemTypeId,iClass
	...
	Dim bTareEligible,bSubLevel,iLotValue



	Dim arrFin,sFinFrom,sFinTo,sTempMonYr,sMonYr,sCheckFinYear,iLotDetValue,ndAtt,ndAttDet,sAttList,sLot
...
'	sITypeCode = trim(Request.Form("hItemTypeCode"))
	...
	with dcrs
		...
		.Source = "SELECT DISTINCT COMPANYITEMCODE,IsNull(ItemType,1) FROM INV_M_ITEMMASTER WHERE ITEMCODE = " & iItemCode & ""
	...
	with dcrs
		...
		.Source = " SELECT DISTINCT COMPANYITEMCODE,ITEMDESCRIPTION, ISNULL(SHORTDESCRIPTION,''), ISNULL(ADDITIONALDESCRIPTION,''), ISNULL(DRAWINGNUMBER,''), "&_
				  ...
				  " ISNULL(SALETOSTOREOPERATOR,0),IsNull(RECEIPTNUMBERING,''),IsNull(RECEIPTROUTING,''),IsNull(ACCOUNTINGTYPE,''),IsNull(CATEGORYCODE,''),ISNULL(ATTRIBUTELIST,0), "&_
...
'					  " CONVERT(DATETIME,"& Pack(sFinTo) &",103) "
		...
		sSql =" Select Distinct YearOpeningStock,YearOpeningValue,isNull(Convert(Varchar,FinancialYearFrom,103),'')  "&_
			  " from Inv_T_ItemLocationStock where OrganisationCode = "& Pack(sUnitID)&" And ItemCode = "& iItemCode &" "&_
			  " And ClassificationCode = "& sClassCode &" And LocationNumber ="& sLoc &"  "&_
			  " and CONVERT(DATETIME,FinancialYearFrom,103) = CONVERT(DATETIME,"& Pack(sFinFrom) &",103)  "
	'	Response.Write "<textarea>"&sSql&"</textarea>"
		...
		Do while Not dcrs2.EOF
			...
			sSql = " Select IsNull(LotNumber,0),IsNull(AttributeList,0),SUM(LotQuantityNett),PackingCode from Inv_T_LocationLot where ItemCode ="& iItemCode & " And ClassificationCode = "& sClassCode &" And OrganisationCode ="& Pack(sUnitID)&" And SrcType = 'RO' and LotQuantityNett > 0 Group By LotNumber,AttributeList,PackingCode "
		'	Response.Write "<textarea>"& sSql &"</textarea>"
			dcrs1.Open sSql,con
			if not dcrs1.EOF then
			    bOpeningPacks = true
			...
			dcrs1.close
		'	Response.Write "<textarea>"& bOpeningPacks &"</textarea>"


			if not bOpeningPacks then
			    sSql = "Select IsNull(L.LotNumber,0),IsNull(L.AttributeList,0),SUM(L.LotQuantityNett),L.PackingCode from INV_T_ItemLedgerOpeningPacks IL join INV_T_LocationLot L on IL.SerialNumber = L.SerialNumber where "
			    sSql =sSql & " IL.ItemCode ="& iItemCode & " And IL.ClassificationCode = "& sClassCode &" And IL.OrganisationCode ="& Pack(sUnitID)&"  And IL.TransactionType = 'RO' and IL.LotQuantityNett > 0 Group By L.LotNumber,L.AttributeList,L.PackingCode  "
			else
			    sSql = " Select IsNull(LotNumber,0),IsNull(AttributeList,0),SUM(LotQuantityNett),PackingCode from Inv_T_LocationLot where ItemCode ="& iItemCode & " And ClassificationCode = "& sClassCode &" And OrganisationCode ="& Pack(sUnitID)&" And SrcType = 'RO' and LotQuantityNett > 0 Group By LotNumber,AttributeList,PackingCode "
			end if
		'	Response.Write "<textarea>"& sSql &"</textarea>"
			...
			Do while not dcrs1.EOF
			    ...
			    Set Node2 =  oDOM2.createElement("LotSerial")
					...
					if not bOpeningPacks then
			            sQuery = " Select L.ClassificationCode,IsNull(L.LotNumber,0),IsNull(L.SerialNumber,0),L.LotQuantityGross,L.LotQuantityNett,"
                        sQuery =sQuery & " L.StorageLocationNo,IsNull(L.StorageBinNumber,0),IsNull(L.PackingNumber,0),IsNull(L.PackingCode,0),IsNull(L.SellingNumber,0),"
                        sQuery =sQuery & " IsNull(L.WeightPerSellingForm,0),IsNull(L.SellingForm,0),L.Rate,L.QuantityIssued,IsNull(L.Stage,0),IsNull(L.AttributeList,0),"
                        sQuery =sQuery & " isNull(Convert(Varchar,L.DateOfReceipt,103),''),IsNull(L.LotQuantityTare,0) from INV_T_ItemLedgerOpeningPacks IL join "
                        sQuery =sQuery & " Inv_T_LocationLot L on IL.SerialNumber = L.SerialNumber where "
                        sQuery =sQuery & " IL.ItemCode ="& iItemCode & " And IL.ClassificationCode = "& sClassCode &" And IL.OrganisationCode ="& Pack(sUnitID)&" And "
                        sQuery =sQuery & " IL.TransactionType = 'RO' and LotQuantityNett > 0 "
			        else
			            sQuery = " Select L.ClassificationCode,IsNull(L.LotNumber,0),IsNull(L.SerialNumber,0),L.LotQuantityGross,L.LotQuantityNett,L.StorageLocationNo, "&_
						 " IsNull(L.StorageBinNumber,0),IsNull(L.PackingNumber,0),IsNull(L.PackingCode,0),IsNull(L.SellingNumber,0),IsNull(L.WeightPerSellingForm,0), "&_
						 " IsNull(L.SellingForm,0),L.Rate,L.QuantityIssued,IsNull(L.Stage,0),IsNull(L.AttributeList,0),isNull(Convert(Varchar,L.DateOfReceipt,103),''),IsNull(L.LotQuantityTare,0) "&_
						 " from Inv_T_LocationLot L where L.ItemCode ="& iItemCode & " And L.ClassificationCode = "& sClassCode &" And L.OrganisationCode ="& Pack(sUnitID)&" And L.SrcType = 'RO' and L.LotQuantityNett > 0 "
				    end if

						 if trim(dcrs1(0))<>"" and trim(dcrs1(0))<>"0" then
						    sQuery = sQuery &" and L.LotNumber = '"& dcrs1(0) &"'"
						 end if

						 if trim(dcrs1(1))<>"" and trim(dcrs1(1))<>"0" then
						    sQuery = sQuery &" and L.AttributeList = '"& dcrs1(1) &"'"
						 end if
						  if trim(dcrs1(3))<>"" then
						    sQuery = sQuery &" and L.PackingCode = "& dcrs1(3)
						  end if
				...
				if not rsTemp.eof then
				    do while not rsTemp.eof
				        ...
				        iLotDetValue =round(cdbl(iRate) * cdbl(iLQtyNet))
				        ...
				        Node3.setAttribute "QTYREC", iLQtyGrs
				        Node3.setAttribute "TAREREC",rsTemp(17)
...
<script type="application/xml" id="ItemAttData" data-itms-xml-island>
    <Root>
        <%
            sQuery = "Select M.ItemTypeAttributeID,T.HeaderID,ItemTypeAttributeName,ItemTypeAttributeType,ItemTypeAttributeDataLength,ItemTypeAttributeDecimal,AttributeValue from INV_M_ItemMasterAttributes M,INV_M_ItemTypeAttributes T where M.ItemTypeAttributeID = T.ItemTypeAttributeID and M.ItemCode = "& iItemCode
            dcrs.open sQuery,con
            if not dcrs.eof then
                do while not dcrs.eof
                    %>
						...
						<td height="20" valign="bottom">
							<table border="0" cellpadding="0" cellspacing="0" >
								<tr>
									...
									<td class="TabCell" valign="bottom" width="90">
										<table border="0" cellpadding="0" cellspacing="0" width="100%" class="TabTable" onMouseOver="tabrollover(this)" onMouseOut="tabrollout(this)">
											<tr><a href="ItmDetailedDefnAmd.asp?ItemCode=<%=iItemCode%>&ClassCode=<%=sClassCode%>">
									...
									<td class="TabCell" valign="bottom" width="145">
									    <table border="0" cellpadding="0" cellspacing="0" width="100%" class="TabTable" onMouseOver="tabrollover(this)" onMouseOut="tabrollout(this)">
										    <tr><a href="ItmInvDetAmd.asp?ItemCode=<%=iItemCode%>&ClassCode=<%=sClassCode%>">
								    ...
								    <td class="TabCell" valign="bottom" width="145">
									    <table border="0" cellpadding="0" cellspacing="0" width="100%" class="TabTable" onMouseOver="tabrollover(this)" onMouseOut="tabrollout(this)">
										    <tr><a href="ItmManufactureAmd.asp?ItemCode=<%=iItemCode%>&ClassCode=<%=sClassCode%>">
															...
															<td class="FieldCellSub">
																<table border="0" cellspacing="0" cellpadding="0">
																	...
																	<tr>
																		...
																		<td class="FieldCell">
																			<input type="radio" value="1" name="radBoM" class="FormElem" <%If iBOMApplicable = 1 then Response.Write "checked"%> disabled="true">   Yes&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
																			<input type="radio" value="0" name="radBoM" class="FormElem" <%If iBOMApplicable = 0 then Response.Write "checked"%> disabled="true">   No
										...
										<tr>
											...
											<center>
												<table cellpadding="0" cellspacing="0" width="100%">
													...
													<tr>
														<td class="FieldCellSub">
															<div class="frmBody" id="frm2" style="width: 750; height:75;">
