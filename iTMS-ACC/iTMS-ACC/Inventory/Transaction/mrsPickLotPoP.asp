...
dim iItem,iClass,sItemName,iLot,iEntryNo,iIssueEntryNo
dim arrTemp,iMRSNo,sOrgID,sOrgName,dMRSDate,iQty
dim arrLocation,sStoreName,sStoreCode,sBinCode,arrStore
dim arrUoM,sUoMDesc,sUoMCode,sQuery,sPackingNo,sRcptNumbering
...
sQuery = "SELECT GROUPNAME,ITEMDESCRIPTION,ORGUNITSHORTDESCRIPTION,CONVERT(VARCHAR,ISSUEDATE,103),ORGANISATIONCODE,CLASSIFICATIONCODE,ITEMCODE FROM VW_INV_IssuedForPick WHERE  ISSUEENTRYNO = " & iIssueEntryNo & " and ITEMENTRYNO = "& iEntryNo
									...
									<div class="frmBody" id="frm2" style="width: 100%; height:200;">
										...
										<%
										   Response.write "<font color=red>"
											...
											if iLot = "N/A" then
											    if trim(sRcptNumbering)="LS" then
												    sQuery = "SELECT SERIALNUMBER,ISNULL(LotQuantityNett,0) FROM VW_ITEMLOCATIONLOT_STOCK WHERE ITEMCODE = " & iItem & " AND CLASSIFICATIONCODE = " & iClass & " AND ORGANISATIONCODE = " & Pack(sOrgID) & " AND STORAGELOCATIONNO = " & sStoreCode & " AND (STORAGEBINNUMBER = " & sBinCode & " OR STORAGEBINNUMBER IS NULL) "
												else
												    sQuery = "SELECT SERIALNUMBER,ISNULL(LotQuantityNett,0) FROM VW_ITEMLOCATIONLOT_STOCK WHERE ITEMCODE = " & iItem & " AND CLASSIFICATIONCODE = " & iClass & " AND ORGANISATIONCODE = " & Pack(sOrgID) & " AND (LOTNUMBER IS NULL or LotNumber = '0' or LotNumber = 'N/A') AND STORAGELOCATIONNO = " & sStoreCode & " AND (STORAGEBINNUMBER = " & sBinCode & " OR STORAGEBINNUMBER IS NULL) "
												end if
											else
												sQuery = "SELECT SERIALNUMBER,ISNULL(LotQuantityNett,0) FROM VW_ITEMLOCATIONLOT_STOCK WHERE ITEMCODE = " & iItem & " AND CLASSIFICATIONCODE = " & iClass & " AND ORGANISATIONCODE = " & Pack(sOrgID) & " AND LOTNUMBER = " & Pack(iLot) & " AND STORAGELOCATIONNO = " & sStoreCode & " AND (STORAGEBINNUMBER = " & sBinCode & " OR STORAGEBINNUMBER IS NULL)"
											end if
											sQuery = sQuery & " and SerialNumber not in (Select SerialNo from INV_T_MaterialIssueDetails where IssueEntryNo in ("& iIssueEntryNo &"))"
											...
											if not dcrs2.EOF then
												Do While Not dcrs2.EOF
													...
													if trim(sRcptNumbering)="LS" and trim(iLot) = "N/A" then
													    sQuery = "Select LotNumber +'-'+ isNull(PackingNumber,SerialNumber) from INV_T_LocationLot where SerialNumber = "& Trim(dcrs2(0))
													else
