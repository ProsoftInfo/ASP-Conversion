...
<%
	...
	dim sTemp,sReceiptNumberStatus,sItemTypeID
	...
	if ndRoot.hasChildNodes() then
		For each ndItem in ndRoot.childNodes
			...
			with dcrs
				...
				'Response.Write "<p>Sql="&dcrs.Source 
			...
			Do While Not dcrs.EOF
				...
				with dcrs2
					...
					.Source = "SELECT DISTINCT ISNULL(SUM(LOTQUANTITYNETT),0) - ISNULL(SUM(QUANTITYISSUED),0),STORAGELOCATIONNO,STORAGEBINNUMBER FROM INV_T_LOCATIONLOT  WHERE ITEMCODE = "& iItem &" AND CLASSIFICATIONCODE = "& iClass &" AND (STORAGELOCATIONNO = "& sLoc &" AND (STORAGEBINNUMBER = "& sBin &" OR STORAGEBINNUMBER IS NULL)) AND (ISNULL(LOTQUANTITYNETT,0) - ISNULL(QUANTITYISSUED,0)) > 0 GROUP BY STORAGELOCATIONNO,STORAGEBINNUMBER ORDER BY 2,3"
					...
					Do While Not dcrs2.EOF
						...
						'if not iSer = "NULL" then
							...
							if (cdbl(dcrs2(0)) - cdbl(iLotQtyReserved)) > 0 then
								...
								newElem1.setAttribute "INVRECNO", "0"'trim(dcrs2(3))
					...
					If Not dcrs1.EOF Then	
						do while Not dcrs1.EOF
							if not (cdbl(trim(dcrs1(0))) - cdbl(iTotLotQty)) <= 0 then
								...
								newElem1.setAttribute "QTYISS", ""
								newElem1.setAttribute "TYPE", ""
								newElem1.setAttribute "ENABLE", "N"
								newElem.appendChild newElem1
