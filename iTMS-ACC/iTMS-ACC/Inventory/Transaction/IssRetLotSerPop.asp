...
dim iClass,iItem,iIssNo,iMRSNo,sOrgID,sType,iDINo,sAttributeList
...
'*************
	...
	sExp = "//ITEM[@ITMCODE="& iItem &" and @CLACODE="& iClass &" and @ATTID='"& sAttributeList &"']/STORAGE/LotSerial"
	set ndLotSerial = Root.SelectNodes(sExp)
	if ndLotSerial.Length>0 then
	    For iCtr = 0 to ndLotSerial.length -1
	        ii = ii + 1
	        set Q = eval("document.formname.txtQty"&ii)
	        Q.value = ndLotSerial.Item(iCtr).Attributes.getNamedItem("QTY").Value
	    Next
	end if
end Function
'*************************************************
...
'alert(obj.name)
	...
	set OutDataValue = showModalDialog("IssRetSerialPop.asp?sTemp="&sTempValues,ItemData,"dialogHeight:400px;dialogWidth:400px;center:Yes;help:No;resizable:No;status:No")	
    ...
    if sLot = "N/A" or sLot ="NULL" or isNull(sLot) then
        sExp = "//ITEM[@ITMCODE="& trim(iItem) &" and @CLACODE="& trim(iClass) &" and @ATTID='"& trim(sAttributeList) &"']/STORAGE/LotSerial[@LOT='0']"
    else
	    sExp = "//ITEM[@ITMCODE="& trim(iItem) &" and @CLACODE="& trim(iClass) &" and @ATTID='"& trim(sAttributeList) &"']/STORAGE/LotSerial[@LOT='"& trim(sLot) &"']"
    end if
    
    set ndLotSerial = RootO.SelectNodes(sExp)
	if ndLotSerial.Length>0 then
	
	    
	    if ndLotSerial.Length=1 then
	        Q.value = ndLotSerial.Item(0).Attributes.getNamedItem("QTY").Value
	    else
	        For iCtr = 0 to ndLotSerial.length -1
	            Q.value = ndLotSerial.Item(iCtr).Attributes.getNamedItem("QTY").Value
	        Next
...
Function CheckSubmit()
    ...
    iIssRate = document.formname.hIssRate.value 
    'alert(iIssRate)
    if trim(iIssRate)="" or IsNull(iIssRate) then iIssRate = "0"
		
	Set Root = ItemData.documentElement
	For Each HeaderNode In Root.childNodes
	    if HeaderNode.nodeName="ITEM" then
	        if HeaderNode.getAttribute("ITMCODE")=iItem and HeaderNode.getAttribute("CLACODE")=iClass and HeaderNode.getAttribute("ATTID")=sAttributeList then
	            HeaderNode.setAttribute "QTY",iQtyTot
	            For Each ndStorage in HeaderNode.childNodes
	                if ndStorage.nodeName="STORAGE" then
                        ndStorage.setAttribute "QTY",iQtyTot
	                    ndStorage.setAttribute "STORAGEVALUE", cdbl(iQtyTot)* cdbl(iIssRate)
	                    For each ndLotSer in ndStorage.childNodes
	                        iCtr = iCtr +1
	                       set Q= eval("document.formname.txtQty"&iCtr)
	                       ndLotSer.setAttribute "QTY",Q.value
	                    Next
	                end if
	            Next
	        end if 'if HeaderNode.getAttribute("ITMCODE")=iItem and HeaderNode.getAttribute("CLACODE")=iClass and HeaderNode.getAttribute("ATTID")=sAttributeList then
        end if
	Next
	...
	For Each HeaderONode In RootO.childNodes
	    if HeaderONode.getAttribute("ITMCODE")=iItem and HeaderONode.getAttribute("CLACODE")=iClass and HeaderONode.getAttribute("ATTID")=sAttributeList then
		    RootO.removeChild(HeaderONode)
		end if'if HeaderONode.getAttribute("ITMCODE")=iItem and HeaderONode.getAttribute("CLACODE")=iClass and HeaderONode.getAttribute("ATTID")=sAttributeList then
	Next

    For Each HeaderNode in Root.ChildNodes
        RootO.appendChild HeaderNode
    Next
	'alert(RootO.xml)
	window.close
end Function
