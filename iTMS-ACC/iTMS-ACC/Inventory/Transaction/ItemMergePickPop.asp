...
dim iClass,iItem,iQty,i,Q,j,iarrAttributeList,iEntNo
...
'	alert("Root="&Root.xml)
	...
	For Each ndItemDet In Root.childNodes   
	    if ndItemDet.nodeName="Item" then
	            if ndItemDet.Attributes.getNamedItem("CLASSCODE").Value = iClass and ndItemDet.Attributes.getNamedItem("ITEMCODE").Value  = iItem and ndItemDet.Attributes.getNamedItem("ENTRYNO").Value  = iEntNo then
	           	    For Each childNod In ndItemDet.childNodes					
					    if childNod.NodeName = "Pick" then
						    ...
						    For Each childNodO In childNod.childNodes
							    ...
							    next
						    next
						    ndItemDet.removeChild childNod
					    end if
				    next 'For Each childNod In ndItemDet.childNodes
			    end if
	    end if 'if ndItemDet.nodeName="ItemDetails" then
	next
end Function
'***************************************************
Function CheckLot(obj,iEntNo,iClass,iItem,sOrg,sOptName,sAttID,sAttList)
	...
	sTempValues = obj.name&":"&iClass&":"&iItem&":"&sOrg&":"&iEntNo&":"&sOptName&":"&sAttID&":"&sAttList
	'alert(sTempValues)

	set OutDataValue = window.showModalDialog("ItemMergeBagSelPop.asp?sTemp="&sTempValues,OutData,"dialogHeight:580px;dialogWidth:580px;center:Yes;help:No;resizable:No;status:No")
...
End IF	
	...
	if not sTotQty = 0 then
		...
		For Each ndItemDet In RootO.childNodes
		    if ndItemDet.nodeName="Item" then
                if ndItemDet.Attributes.getNamedItem("CLASSCODE").Value = iClass and ndItemDet.Attributes.getNamedItem("ITEMCODE").Value  = iItem and ndItemDet.Attributes.getNamedItem("ENTRYNO").Value = iEntNo then
		            For Each childNod In ndItemDet.childNodes
			            if StrComp(Trim(childNod.NodeName),"Pick") = 0 then
				            set oRem = ndItemDet.removeChild(childNod)
			            end if						
		            next
		            ndItemDet.setAttribute "QTY",""' trim(Q.value)
		            if not Root.Attributes.getNamedItem("TOT").Value = "" then				
			            ndItemDet.appendChild(OutData.documentElement)
		            end if
		            exit for
	            end if
			 end if 'if ndItemDet.nodeName="Item" then
		next
	else
		...
		For Each ndItemDet In RootO.childNodes
		    if ndItemDet.nodeName="Item" then
                if ndItemDet.Attributes.getNamedItem("ENTRYNO").Value = iEntNo And ndItemDet.Attributes.getNamedItem("CLASSCODE").Value = iClass and ndItemDet.Attributes.getNamedItem("ITEMCODE").Value  = iItem  then
			        For Each childNod In ndItemDet.childNodes
				        if StrComp(Trim(childNod.NodeName),"Pick") = 0 then
					        set oRem = ndItemDet.removeChild(childNod)
				        end if
