...
%>
<%
	'Program Name				:	IssuePickLotPop.asp
...
dim arrTemp,iMRSNo,sOrgID,sOrgName,dMRSDate,iQty,iInvRecNo,iIssueEntryNo
...
Dim sFilter,sSearchSeparator,sSearchBy,arrTempFilter,sFrom,sTo,sReceiptNumbering
...
with dcrs2
	.CursorLocation = 3
	.CursorType = 3
	.Source = "SELECT DISTINCT GROUPNAME,SHORTDESCRIPTION,ORGUNITSHORTDESCRIPTION,ITEMDESCRIPTION,ReceiptNumbering FROM VWITEM WHERE ITEMCODE = " & iItem & " AND CLASSIFICATIONCODE = " & iClass & " AND ORGANISATIONCODE = " & Pack(sOrgID) & ""
...
if not dcrs2.EOF then
	...
	sReceiptNumbering = trim(dcrs2(4))
end if
dcrs2.Close
...
'Response.write "OrgID = "& sOrgID &"  iClass="&iClass &" Item = "& iItem
'Response.write DisplayUoM(sOrgID,iClass,iItem)
sUoMCode = arrUoM(0)
sUoMDesc = arrUoM(1)
...
Dim iAttrbList,sAttrbName,sTempAttrb,iOptVal,i
	...
	iRowCount = cdbl(Request.QueryString("hRowCount"))

	Select Case Request("hSubmit")
    	Case "Previous"								'if prev button clicked
    		iCurrentPage = cdbl(iCurrentPage) - 1	'decrease current page
    	Case "Next"									'if next button clicked
    		iCurrentPage = cdbl(iCurrentPage) + 1	'increase page count
    	Case "First"
    		iCurrentPage = 1
    	Case "Last"
    		iCurrentPage = iLastPage
    End Select
	iCurrentPage = cdbl(Request.QueryString("hSubmit"))
                            ...
                            <tr>
								...
								<td>
									...
									<!--<div class="frmBody" id="frm2" style="width: 100%; height:250px;">-->
										...
										<%
										Dim sMarkPackFlag,sLotWithPackFlag
										if trim(iLot)="N/A" or trim(iLot)="" or isNull(iLot) then iLot = "NULL"

										sQuery = "Select isNull(MarkPackFlag,'L') from INV_T_MaterialIssueHeader  where IssueEntryNo = "& iIssueEntryNo
										...
										if trim(sMarkPackFlag)="N" then
										    sQuery = "Select * from INV_T_MaterialIssuedForPick where IssueEntryNo = "& iIssueEntryNo &" and ItemCode ="& iItem &" and ClassificationCode="& iClass &" and OrganisationCode="& pack(sOrgID) &" and ItemEntryNo = "& iEntNo &" and SerialNo is not Null"
										    ...
										    if trim(sLotWithPackFlag)="Y" then
										        sQuery = "Select SerialNo,IsNull(QuantityForPick,0),IsNull(PackingNumber,SerialNo),IsNull(ItemAttributes,0),InventoryReceiptNo,LotNo from Vw_IssuedForPick_LocLot"
										        sQuery = sQuery & " where IssueEntryNo = "& iIssueEntryNo &" and ItemCode ="& iItem &" and ClassificationCode="& iClass &" and OrganisationCode="& pack(sOrgID)
										        sQuery = sQuery & " and LocationNumber ="& sStoreCode &" and (BinNumber="& sBinCode &" or BinNumber Is Null) and QuantityPicked = 0 and SerialNo is not Null "
										        if iLot="NULL" then
										            sQuery = sQuery &" and (LotNo is Null or LotNo = '0' or LotNo = 'N/A') "
										        else
										            sQuery = sQuery &" AND LotNo  = " & Pack(iLot)
										        end if

										        if Trim(sAttID)<>"" and (not isNull(sAttID)) then
									                sQuery = sQuery & " and ItemAttributes like '%" & sAttID &"%'"
									            end if

									            if sFilter<>"" then
										            if trim(sSearchBy)="LN" then
										                if trim(sFrom)<>"" and trim(sTo)<>"" and sSearchSeparator ="R" then
													        sQuery = sQuery & " and (LotNo >= '"& sFrom &"' and LotNo <='"& sTo &"')"
												        else
													        sQuery = sQuery & " and LotNo in ('"& lcase(sFrom)&"')"
											        ...
											        elseif trim(sSearchBy)="PQ" then
												        if trim(sFrom)<>"" and trim(sTo)<>"" and sSearchSeparator ="R" then
													        sQuery = sQuery & " and ( QuantityForPick >= "& sFrom &" and QuantityForPick <="& sTo &")"
												        else
													        sQuery = sQuery & " and QuantityForPick in ("& sFrom &")"
												        end if 'if trim(sSearchType)="R" then
											        end if
										        end if' if  sFilter<>"" then

											    sQuery = sQuery & " ORDER BY 1"
											else 'if trim(sLotWithPackFlag)="N" then
										        sQuery = "Select SerialNo,IsNull(QuantityForPick,0),IsNull(PackingNumber,SerialNo),IsNull(ItemAttributes,0),InventoryReceiptNo,LotNo from Vw_IssuedForPick_LocLot"
										        sQuery = sQuery & " where IssueEntryNo = "& iIssueEntryNo &" and ItemCode ="& iItem &" and ClassificationCode="& iClass &" and OrganisationCode="& pack(sOrgID)
										        sQuery = sQuery & " and LocationNumber ="& sStoreCode &" and (BinNumber="& sBinCode &" or BinNumber Is Null) and QuantityPicked = 0 and SerialNo is Null "
										        if iLot="NULL" then
										            sQuery = sQuery &" and (LotNo is Null or LotNo = '0' or LotNo = 'N/A') "
										        else
										            sQuery = sQuery &" AND LotNo  = " & Pack(iLot)
										...
										end if'    if trim(sMarkPackFlag)="N" then

										if trim(sMarkPackFlag)="L" or trim(sLotWithPackFlag)="N" then
										        if iLot = "NULL" then
										            if trim(sReceiptNumbering)="LS" then
											            sQuery = "SELECT DISTINCT SERIALNUMBER,ISNULL(AVAILABLENETSTOCK,0),ISNULL(PACKINGNUMBER,SERIALNUMBER),ISNULL(ATTRIBUTELIST,0),InventoryReceiptNo,LOTNUMBER FROM VW_ITEMLOCATIONLOT_STOCK WHERE ITEMCODE = " & iItem & " AND CLASSIFICATIONCODE = " & iClass & " AND ORGANISATIONCODE = " & Pack(sOrgID) & "  AND STORAGELOCATIONNO = " & sStoreCode & " AND (STORAGEBINNUMBER = " & sBinCode & " OR STORAGEBINNUMBER IS NULL) AND AVAILABLENETSTOCK > 0 AND SERIALNUMBER IS NOT NULL "
											        else
											            sQuery = "SELECT DISTINCT SERIALNUMBER,ISNULL(AVAILABLENETSTOCK,0),ISNULL(PACKINGNUMBER,SERIALNUMBER),ISNULL(ATTRIBUTELIST,0),InventoryReceiptNo,LOTNUMBER FROM VW_ITEMLOCATIONLOT_STOCK WHERE ITEMCODE = " & iItem & " AND CLASSIFICATIONCODE = " & iClass & " AND ORGANISATIONCODE = " & Pack(sOrgID) & "  AND ( LOTNUMBER IS NULL OR LOTNUMBER = '0' or LOTNUMBER = 'N/A') AND STORAGELOCATIONNO = " & sStoreCode & " AND (STORAGEBINNUMBER = " & sBinCode & " OR STORAGEBINNUMBER IS NULL) AND AVAILABLENETSTOCK > 0 AND SERIALNUMBER IS NOT NULL "
											        end if
										        else
									           ...
									           ' Response.write "<textarea>"& sQuery &"</textarea>"
										        ...
										        if sFilter<>"" then
										            if trim(sSearchBy)="LN" then
										                if trim(sFrom)<>"" and trim(sTo)<>"" and sSearchSeparator ="R" then
													        sQuery = sQuery & " and (LotNumber >= '"& sFrom &"' and LotNumber <='"& sTo &"')"
												        else
													        sQuery = sQuery & " and LotNumber in ('"& lcase(sFrom)&"')"
												        end if 'if trim(sSearchType)="R" then
											        elseif trim(sSearchBy)="PN" then
												        if trim(sFrom)<>"" and trim(sTo)<>"" and sSearchSeparator ="R" then
													        sQuery = sQuery & " and ( cast(PACKINGNUMBER as integer) >= "& sFrom &" and cast(PACKINGNUMBER as integer) <="& sTo &")"
										        ...
										        %>
											        ...
											        <tr>
												        ...
												        <%if trim(sReceiptNumbering)="LS" and trim(iLot)="NULL" then%>
												            <span id="spanSerial<%=iCtr%>"><%=trim(dcrs2(5))%>-<%=FindPackNumber(trim(dcrs2(0)))%></span>
													        <input type="hidden" name="txtSerial<%=iCtr%>"  value="<%=FindPackNumber(trim(dcrs2(0)))%>" maxlength=10 class="FormElemDisp" READONLY>
												        <%else %>
												        ...
												        <%end if%>
													        ...
													        <input type="hidden" name="hLotNoZ<%=iCtr%>" value="<%=trim(dcrs2(5))%>">
												        </td>
												        <td class="ExcelDisplayCell" >
													...
													<tr>
														<td class="ExcelHeaderCell" >Search By
															<Select name="selSearchBy" class=FormElem>
																...
																<option value="PT">Pack Type</option>
																<option value="PD">Pack Date</option>
																<option value="LN">Lot Number</option>
															</Select>
														</td>
