...
<%
	...
	'Module Name				:	JobOrder (Common Program)
...
<%
	...
	if bFlag then
		NewXml.save server.MapPath("../temp/transaction/" & sValue & Session.SessionID & ".xml")
	else
		NewXml.save server.MapPath("../temp/transaction/" & sValue & ".xml")
	end if
	set NewXml=nothing
%>
