...
'**************************************************
Function ListChange()
    set objType = eval("document.formname.radType")
    if objType(0).checked then
        sIMType = objType(0).value
    elseif objType(1).checked then
        sIMType = objType(1).value
    end if

    sItemCode = document.formname.hItemCode.value
    sCatCode = document.formname.hClassCode.value
    sSelCategory = document.formname.hCategory.value
    sPartyCode =document.formname.hPartyCode.value
    selAttr = document.formname.hAttID.value
    selIType = document.formname.hItemType.value
    sFromDate = document.formname.hFrmDate.value
    sToDate = document.formname.hToDate.value
    document.formname.action = "MaterialConsumptions.asp?ItemCode="& sItemCode&"&ClassCode="&sCatCode&"&AttID="& selAttr&"&ItemType="&selIType&"&FromDate="&sFromDate&"&ToDate="&sToDate&"&Category="&sSelCategory&"&PartyCode="&sPartyCode&"&IMType="&sIMType
	document.formname.submit
End Function
'***************************************************
Function MatConsumption(sItemCode,sClassCode,sFrmDate,sToDate)
    document.formname.action = "MatConsDetailsEntry.asp?ItemCode="&sItemCode&"&ClassCode="&sClassCode&"&FromDate="&sFrmDate&"&ToDate="& sToDate
    document.formname.submit
End Function
'***************************************************
Function MatConsumptionIss(IssNo,sFrmDate,sToDate)
    document.formname.action = "MatConsDetailsEntry.asp?FromDate="&sFrmDate&"&ToDate="& sToDate&"&IssueNo="&IssNo
    document.formname.submit
End Function
'*******************************************************
Function MatConsumptionIssueWise(sIssNo)
    document.formname.action = "MatConsDetailsEntry.asp?IssueNo="&sIssNo
    document.formname.submit
End Function
'*******************************************************
...
'		exit function
		...
		hFinFromDate = document.formname.hFrmDate.value
	...
	if document.formname.selCategory.selectedIndex>-1 then
	    sSelCategory = document.formname.selCategory(document.formname.selCategory.selectedIndex).value
	else
	    sSelCategory=""
	end if
	...
	''Temp Blocked by ragav Dec 16,2013
	'	sExp ="//ATTRIBUTES [ @ITEMTYPEID = '"&selIType&"']"
	'	Set CodeNode = Root.Selectnodes(sExp)
'
'		for iCnt = 1 to CodeNode.Length
'			iCode = CodeNode.item(iCnt-1).attributes.getNamedItem("ATTRID").value
'			set selObj = eval("document.formname.selAttrZ"&iCnt)
...
'		selAttr = trim(mid(selAttr,2))
		sCatVal = ""
    	sItemCode = "0"
		IF trim(document.formname.hClassCode.value) <> "" then
			sCatCode = document.formname.hClassCode.value
		End IF
		...
		document.formname.action = "MaterialConsumptions.asp?ItemCode="& sItemCode&"&ClassCode="&sCatCode&"&AttID="& selAttr&"&ItemType="&selIType&"&FromDate="&sFromDate&"&ToDate="&sToDate&"&Category="&sSelCategory&"&PartyCode="&sPartyCode
		document.formname.submit
	end if
end Function
'*****************************************
...
'	for iCnt=0 to document.formname.selIType.length - 1
'		if document.formname.selIType.options(iCnt).selected then
'			sIType = sIType&":"&document.formname.selIType.options(iCnt).value
'			sITypeName = sITypeName &":"& document.formname.selIType.options(iCnt).text
'		end if
'	next

	If sIType = "FIB" Then
		sITypeName = "Fibre"
	ElseIf sIType = "YRN" Then
		sITypeName = "Yarn"
	ElseIf sIType = "STO" Then
		sITypeName = "Stores"
	End If
	sIType=mid(sIType,2)
	sOutValue = showModalDialog("../../include/ClassificationSelectPop.asp?sIType="& sIType&"&sOrgID="&sOrgID &"&sITypename="& sITypeName ,"Classification","dialogWidth:650px;dialogHeight:500px;Help:No")

	arrTemp = split(sOutValue,"*****")
...
'alert(arrTemp(0))
	...
	if objhttp.responseXML.xml <> "" then
		...
		if Root.HaschildNodes() then
			For Each HeaderNode In Root.childNodes
				sClassName = sClassName &","& HeaderNode.Attributes.getNamedItem("CLASSNAME").Value
				sClassCode = sClassCode &","& HeaderNode.Attributes.getNamedItem("CLASSCODE").Value
			next
		end if
	end if
	if sClassName<>"" then
	...
	if sClassCode<>"" then
	    sClassCode = mid(sClassCode,2)
	end if

	document.formname.hClassName.value = sClassName
	document.formname.hClassCode.value = sClassCode
    spanClassification.innerText  = sClassName

End Function
'*******************************************************************
Function AddAttrib(sVal)
clearTable()
	set trow=document.all.tblLot.Insertrow(document.all.tblLot.rows.length)
	set Cell=trow.insertCell()
	For iCnt = 1 to sVal
		Cell.innerHtml =Cell.innerHtml + "<Select size=""1"" name=""selAttrZ"& trim(iCnt)&"""  class=""FormElem"" >"
	Next
	Cell.width = "190"
...
Function clearTable()
Dim iCtr
	for	iCtr = 0 to document.all.tblLot.rows.length - 1
		document.all.tblLot.deleterow(iCtr)
	next
End Function
'*******************************************************************
Function GetData()
...
'	sIType = document.formname.selIType.value
	...
	Set CodeNode = Root.Selectnodes(sExp)
	''Begin '-temp blocked by ragav on Dec 16,2013
	...
	'
	'next
	'for iCtr = 1 to CodeNode.Length
	'
	'	iCode = CodeNode.item(iCtr-1).attributes.getNamedItem("ATTRID").value
	'
	'	set selObj = eval("document.formname.selAttrZ"&iCtr)
	'	selObj.length = selObj.length+1
	'	selObj.options(selObj.length-1).text = CodeNode.item(iCtr-1).attributes.getNamedItem("ATTRNAME").value
	'	selObj.options(selObj.length-1).Value = CodeNode.item(iCtr-1).attributes.getNamedItem("ATTRID").value
	'
	...
	'		if selobj.options(selObj.length-1).value = document.formname.hAttID.value then
	'		    SelObj.SelectedIndex = SelObj.length-1
	'		end if
	'	next
	'
	'next
	if trim(document.formname.hAttID.value)="" then
	    document.formname.hClassCode.value = ""
	    document.formname.hClassName.value = ""
	    document.formname.hItemCode.value = ""
	    spanClassification.innerText = "All Classification"
	    idItemName.innerText = "All Items"
	end if

	sCategory = document.formname.hCategory.value
	if trim(sCategory)<>"" then
	    For iCnt = 0 to document.formname.selCategory.length-1
	        if trim(sCategory)=trim(document.formname.selCategory(iCnt).value) then
	            document.formname.selCategory.selectedIndex = iCnt
	            exit for
	        end if
	    Next
	end if
...
'	For iCnt = 0 to document.formname.selIType.length -1
'	    if document.formname.selIType(iCnt).value = document.formname.hItemType.value then
'	        document.formname.selIType.selectedIndex = iCnt
	        GetData()
'	        exit for
'	    end if
'	Next
...
Function Search()
	...
	set ResData = showModalDialog("../../Common/"&sProgramName&"?orgID="& sUnit &"&sIType=" & sIType & "&Stock=" & iStock & "&hSelectMode=M&Flag="+cstr(nFlag)&"&hClassCodes="& sClassCode,ItemData,"dialogHeight:"& sPopupHeight &"px;dialogWidth:"& sPopupWidth &"px;Status:No")
	...
	if Root.hasChildNodes() then
		...
		For Each HeaderNode In Root.childNodes
			...
			sTemp = sTemp &"|"& HeaderNode.getAttribute("ItemCode")
			IF Left(sTemp,1) = "|" then
				sTemp = Mid(sTemp,2)
			End IF

			sVal = split(HeaderNode.getAttribute("ItemName"),"--")
			sText = sText &","& sVal(0)
...
Function SelectParty()
	...
	set ResData = showModalDialog("../../Common/"&sProgramName&"?orgID="& sUnit,PartyData,"dialogHeight:"& sPopupHeight &"px;dialogWidth:"& sPopupWidth &"px;Status:No")
	...
	if Root.hasChildNodes() then
		For Each HeaderNode In Root.childNodes
			...
			sTemp = sTemp &"|"& HeaderNode.getAttribute("RetField1")
			IF Left(sTemp,1) = "|" then
				sTemp = Mid(sTemp,2)
			End IF

			sVal = split(HeaderNode.getAttribute("RetField0"),"--")
			sText = sText &","& sVal(0)
			spanParty.innerText = Mid(sText,2)
