...
%>
<%
	'Program Name				:	DirectIssueMixEntry.asp
	'Module Name				:	Inventory (Issue Additional Details)
	'Author Name				:	MOHAMMED ASIF
	'Created On					:	February 13, 2004
...
dim arrTemp,sOrgID,sOrgName,iQty,arrUoM,sUoMCode,sUoM,sMixCode

arrTemp = split(trim(Request.QueryString("sTemp")),"|")
'Response.Write trim(Request.QueryString("sTemp"))
iClass = arrTemp(0)
iItem = arrTemp(1)
sOrgID = arrTemp(2)
sUsage = arrTemp(3)
...
<SCRIPT LANGUAGE=javascript SRC="../scripts/DirectIssueMixDetails.js"></SCRIPT>
									...
									<div class="frmBody" id="frm2" style="width: 100%; height:130;">
										<table border="0" cellspacing="1" id="tblData" class="ExcelTable" width="100%">
											<tr>
												...
												<td class="ExcelHeaderCell" align="center">Mix Name</td>
										...
										<%
											with dcrs
												.CursorLocation = 3
												.CursorType = 3
												'.Source = "SELECT DISTINCT AP.MIXCODE,MIXNAME FROM APP_M_MIXVARIETYQUALITYPARAMETERS AP, APP_M_MIXMASTER AM WHERE ITEMCODE = " & iItem & " AND VARIETYCODE = " & iClass & " AND AP.MIXCODE = AM.MIXCODE AND AP.MIXCODE IN (" & sMixCode & ") AND STATUS = 'Y' ORDER BY 1"
												.Source = "SELECT DISTINCT AP.MIXCODE,MIXGROUPNAME FROM APP_M_MIXVARIETYQUALITYPARAMETERS AP, APP_M_MIXGROUP AM, APP_M_MIXMASTER MM WHERE MM.MIXGROUPCODE = AM.MIXGROUPCODE AND AP.ITEMCODE = " & iItem & " AND AP.VARIETYCODE = " & iClass & " AND MM.MIXCODE = AP.MIXCODE AND AP.MIXCODE IN (" & sMixCode & ") AND AM.STATUS = 'Y' ORDER BY 1"
												.ActiveConnection = con
												.Open
											end with
										...
										%>
											<tr>
												...
												<td class="ExcelInputCell" align="right" width="80">
													...
													<input type=hidden name="hMC<%=iCtr%>" value="<%=trim(dcrs(0))%>">
												</td>
											</tr>
										<%
												dcrs.MoveNext
