...
Function Init()
    ...
    for iCnt = 0 to document.formname.SelRefName.length - 1
        if document.formname.SelRefName(iCnt).value=sAppRefType then
            document.formname.SelRefName.selectedIndex = iCnt
        end if
    next
    
    sRefNoDate = document.formname.hRefNoDate.value
    if trim(sRefNoDate)<>"" then
        RefNoDate.innerText = split(sRefNoDate,",")(1)
    end if
    
    sPartyName = document.formname.hSupplierName.value
    idSupplier.innerText=sPartyName 
    
    sPartyCode = document.formname.hSupplierCode.value
    LoadPartySubType
    sSubType = document.formname.hSubType.value
    for iCnt = 0 to document.formname.selPartySubType.length - 1
        if document.formname.selPartySubType(iCnt).value = sSubType then
            document.formname.selPartySubType.selectedIndex = iCnt
            exit for
        end if
    next
    
    sPayFor = document.formname.hPayFor.value
    
    for iCnt = 0 to document.formname.selPayFor.length - 1
        if document.formname.selPayFor(iCnt).value = sPayFor then
            document.formname.selPayFor.selectedIndex = iCnt
            exit for
        end if
    next
    document.formname.ctlChequeDate.setdate = document.formname.hChequeDate.value
    PaymentForChange
...
Function CheckSubmit()
    ...
    set ndVouRoot = VoucherData.documentElement
        ...
        ndVouRoot.setAttribute "ReferenceNo",RefNoDate.innerText 
		ndVouRoot.setAttribute "hRefNo",document.formname.hAppRefNo.value
		ndVouRoot.setAttribute "AppRefNo",document.formname.hAppRefNo.value
		if trim(RefNoDate.innerText)<>"N/A" then
		    ndVouRoot.setAttribute "AppRefDate",split(RefNoDate.innerText,"-")(1)
		else
		    ndVouRoot.setAttribute "AppRefDate",""
		end if
		ndVouRoot.setAttribute "AppRefType",document.formname.SelRefName(document.formname.SelRefName.selectedIndex).value
		ndVouRoot.setAttribute "PartyCode",document.formname.hSupplierCode.value
   ...
   ' alert(ndVouRoot.xml)
    ...
    set objhttp = CreateObject("Microsoft.XMLHTTP")
    objhttp.open "POST","XMLSave.asp?Name=MISCPaymentEdit&SessionFlag=true",false
    objhttp.send ndVouRoot.xml
    
    document.formname.action = "MiscInvoiceUpdate.asp?InvNo="&document.formname.hMiscInvNo.value
