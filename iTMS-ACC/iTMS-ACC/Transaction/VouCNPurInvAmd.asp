...
<%	option explicit	%>
<%
	'Program Name				:	VouCNPurInvAmd.asp
...
dim sSalType,sOrgId,sOrgName,sQuery,sPartyName,sInvoiceNo,iInvNo,sVouDate
...
Dim ObjInv,sNarration,iBookName,iCrAltAccHd,sCrAltAccName,sAccChg,sAmdTy,sRetVal
Dim sFinPeriod,dInvInvAmount,iItemCode,iClassCode
...
sQuery = "Select Convert(Varchar,VoucherDate,103),PurchaseBillType From  "&_
	...
	if oNodHeader.nodeName="Header" then
		for Each oNodEntry in  oNodHeader.childNodes
			...
			if oNodEntry.nodeName="PurInvoice" then
				...
				iInvNo = oNodEntry.Attributes.getNamedItem("PurTransNo").Value
			end if
		next
	end if
...
'Response.Write sCrAgain

IF CStr(sCrAgain) = "" Then
...
Function SaveXML()
	...
	IF document.formname.SelCrAgain.value = "A" Then
		...
		IF TempNode.length <> 0 Then
			For iCtr = 0 To TempNode.length - 1
				Set dNewVal = Eval("document.formname.txtAmount"&iCtr+1)
				TempNode.Item(iCtr).Attributes.getNamedItem("Amount").Value = dNewVal.Value
			Next
		End IF
	End IF
...
Function SetRetVal(sObj,sCallType)
	...
	ElseIF sCallType = "2" Then
		...
		'	For iCtr = 0 To TempNode.length - 1
		'		iSno = CDbl(iCtr) + 1
		'		Eval("document.formname.txtQty"&iSno).Value = TempNode.Item(iCtr).Attributes.getNamedItem("Qty").Value
		'		Eval("document.formname.txtRate"&iSno).Value = TempNode.Item(iCtr).Attributes.getNamedItem("Rate").Value
		'		Eval("document.formname.txtDis"&iSno).Value = TempNode.Item(iCtr).Attributes.getNamedItem("DisPer").Value
		'
		'	Next
		'End IF
...
Function SetDate()
	document.formname.ctlDate.setDate = document.formname.hSelVouDate.value
End Function
...
<BODY leftMargin=0 topMargin=0 MARGINHEIGHT="0" MARGINWIDTH="0" onLoad="Init('<%=sCrAgain%>');SetDate()">
<form method="POST" name="formname" action="VouCNPurInvAmdUpdate.asp">
<Input type="hidden" name="hdTransNo" value="<%=iInvNo%>">
<Input type="hidden" name="hAccType" value="C">
<Input type="hidden" name="hCallType" value="PA">
...
<input type="hidden" name="hSelVouDate" value="<%=sVouDate%>">
<input type="hidden" name="hFinFrm" value="<%=Left(sFinPeriod,4)%>04">
<input type="hidden" name="hFinTo" value="<%=Right(sFinPeriod,4)%>03">
...
<table border="0" width="100%" cellspacing="0" cellpadding="0">
	<tr>
		...
		Credit Note For Purchase Invoice Amendment
	...
	For Each oInvNodEntry in oInvNodDeatils.childNodes
		...
		if oNodtemp.length> 0 then
            ...
            %>
                ...
                <td class="ExcelDisplayCell" align="Right" id="tQty<%=isno%>"><input type="text" style="text-align: Right" NAME="txtqty<%=isno%>" onBlur="setQty(this,'<%=iSno%>','Q')" value="<%=FormatNumber(sQty,3,,,0)%>" onkeypress="DoKeyPress('Y',12,2)" class="FormelemRead" size="13"></td>
                <td class="ExcelDisplayCell" align="Right" id="tRate<%=isno%>"><input type="text" style="text-align: Right" NAME="txtRate<%=isno%>" onBlur="setQty(txtqty<%=isno%>,'<%=iSno%>','R')" value="<%=FormatNumber(sRate,2,,,0)%>" onkeypress="DoKeyPress('Y',12,2)" class="FormelemRead" size="13"></td>
                ...
                <input type="text" style="text-align: Right" NAME="txtDis<%=isno%>" value="<%=FormatNumber(sDiscount,2,,,0)%>" class="FormelemRead" size="13" onkeypress="DoKeyPress('Y',12,2)" onBlur="setQty(txtqty<%=isno%>,'<%=iSno%>','R')">
                </td>


                <td class="ExcelInputCell" align="Right"><input type="text" style="text-align: Right" NAME="txtAmount<%=iSno%>" onBlur="setTotal(this,'<%=iSno%>')" value="<%=FormatNumber(sAmount,2,,,0)%>" class="Formelem" size="13" onkeypress="DoKeyPress('Y',12,2)"></td>
