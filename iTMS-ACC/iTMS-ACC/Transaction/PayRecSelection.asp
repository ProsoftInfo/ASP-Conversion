...
function finaldone()
	dim sDocNo,PayRecRoot,sTemp,bFlag,sExp,TempNode,sTemp2
	bFlag=0
'Assign Cost Center Root and Analytical Root Nodes
	Set PayRecRoot = AccHeadData.documentElement.childNodes(0)
'Travse through all the elements in the forms
	for i=0 to document.frm1.elements.length-1
		' Check if the element is an check box
		if document.frm1.elements(i).type="checkbox" then
			' Check if the element is cost center
			if InStr(document.frm1.elements(i).name,"chkDocument")>0 then
				' Check if the cost center element is selected
				if document.frm1.elements(i).checked then
					bFlag=1
					sDocNo=document.frm1.elements(i).value
					sTemp2 = Split(sDocNo,"Z")
					sTemp=Split(eval("document.frm1.hDoc"&sDocNo).value,"?")
					'Create an Cost Center Entry Node under Cost Center Node
					Set newElem = AccHeadData.createElement("Doc")
						newElem.setAttribute "No", sTemp2(0)
						newElem.setAttribute "InvNo",trim(sTemp(0))
						newElem.setAttribute "InvDate",trim(sTemp(1))
						newElem.setAttribute "TransAmount",FormatNumber(trim(sTemp(2)),2,,,0)
						newElem.setAttribute "AmtAdjusted",FormatNumber(trim(sTemp(3)),2,,,0)
						newElem.setAttribute "AmtToAdjust","0"
						newElem.setAttribute "DocType",document.frm1.hVouType.value
						newElem.setAttribute "AmtToAccount",FormatNumber(trim(sTemp(4)),2,,,0)
						newElem.setAttribute "PayableNo",trim(sTemp(5))
						newElem.setAttribute "AdjType",trim(sTemp(6))

					 PayRecRoot.appendChild newElem
				end if ' End of Cost Center element selected or not
			end if ' End of CostCenter element check
		end if ' End of Check Box element check
	next

	AccHeadData.documentElement.Attributes.Item(0).nodeValue=bFlag

	sExp = "//RecCount"
	Set TempNode = PayRecRoot.selectNodes(sExp)
	IF TempNode.length <> 0 Then
		TempNode.Item(0).Attributes.Item(0).nodeValue = document.frm1.hRecCount.value
	End IF

	set window.returnValue= AccHeadData.documentElement
	window.close()
end function
