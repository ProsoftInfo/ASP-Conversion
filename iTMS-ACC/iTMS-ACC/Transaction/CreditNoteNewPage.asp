...
'==============================================================================
Function Validate()
	document.formname.hUnitID.value	= document.formname.selUnitId.value
...
dim i,iCnt,sVouType,iBookNo,iChkCnt,objhttp,UnitRoot,sSCVal,sOldAgent,sNewAgent
	sVouType = sPara
	 'alert  sVouType
	iCnt = document.formname.hCnt.value
	sUnit = document.formname.selUnitId.value
	iBookNo  = document.formname.hBookNo.value
	iChkCnt = 0
	For i = 1 to iCnt
		if eval("document.formname.OptCriteria"&i).Checked = true then
			if trim(document.formname.hInvVal.value)<>"SC" then
				sVal = eval("document.formname.OptCriteria"&i).value
			else
				sVal = sVal & eval("document.formname.OptCriteria"&i).value &":"
				sSCVal = sSCVal & eval("document.formname.hOptCriteria"&i).value &","
			end if
			iChkCnt = iChkCnt + 1
		end if
		'sOldAgent = sNewAgent
	...
	IF iChkCnt = 0 then
		'alert document.formname.txtPartyName.value  &"=="& document.formname.selPartyType.value
		If trim(document.formname.selPartyType.value) = "S" or document.formname.txtPartyName.value = "" then
			MsgBox "Select 'Party Type' from Expand band to create Other type voucher.",6,"Credit Note Vouchers"
			exit function
		End IF
		sRetVal = MsgBox("This will Create 'Other' Type Voucher for the Party Selected.Do U want to Continue?",4,"Credit Note Vouchers")
	...
	'	sBookName = "JOURNAL BOOK"
		set objhttp = CreateObject("MSXML2.XMLHTTP")
			...
			if objhttp.responseXML.xml <> "" then
				...
				Set UnitRoot = UnitBook.documentElement
	...
	if trim(document.formname.hInvVal.value)<>"SC" then
		document.formname.hVouDetails.value  = sVal
	else
		document.formname.hVouDetails.value  = sSCVal
	end if
	document.formname.hBookName.value = sBookName
	IF trim(document.formname.hInvVal.value) = "SI" then
		document.formname.action = "VouCNOtherInvEntry.asp?hCallFrom="& sPara
	ElseIF trim(document.formname.hInvVal.value) = "SC" then
		document.formname.action="VouCNSalCommission.asp"
		'document.formname.action="VouCNCommisionEntry.asp"
	ElseIF trim(document.formname.hInvVal.value) = "SR" then
		document.formname.action = "VouCNOtherInvEntry.asp?hCallFrom="& sPara
		''Blocked By Ragav
		'document.formname.action="VouCNSalReturnEntry.asp?hCallFrom="& sPara
	ElseIF trim(document.formname.hInvVal.value) = "PI" then
		document.formname.action = "VouCNPurInvEntry.asp?hCallFrom="& sPara
...
<%
	...
	Dim sChkSR,sChkSI,sChkPI,sChkSC
	Dim sSelVouTy,sOrgId,sPartyType,sPartypeName
...
%>
	<form method="POST" name="formname" action="CreditNoteNewPage.asp">
	...
	<table border="0" width="100%" cellspacing="0" cellpadding="0">
		<tr>
			<td align="center" class="PageTitle" height="20">
				<p align="center">Credit Vouchers
			</td>
		</tr>
...
<Input type=radio name=voutype value="SC"  <%=sChkSC%>  onclick=setInvoiceNo(this) >Sales Commission&nbsp;
<Input type=radio name=voutype value="SR" <%=sChkSR%> onclick=setInvoiceNo(this) >Sales Return&nbsp;
...
<tr>
<td class="FieldCellSub" width="168">Select Party Type</td>
...
<td class="FieldCellSub" colspan="3"> <input type="text" name="txtPartyName" size="40" class="FormElem"></td>
  ...
  <tr>
<td class="FieldCellSub" width="108">From Date</td>
...
<td class="ExcelHeaderCell" align="center" >Party Name
</td>
<%if trim(sSelVouTy)="SI" then%>
	<td class="ExcelHeaderCell" align="center" ><%Response.Write "Invoice Value"%></td>
<%elseif trim(sSelVouTy)="SC" then%>
	<td class="ExcelHeaderCell" align="center" ><%Response.Write "Commission Value"%></td>
<%end if%>
