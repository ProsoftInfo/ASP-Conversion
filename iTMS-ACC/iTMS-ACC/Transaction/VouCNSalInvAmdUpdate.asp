...
<%	option explicit	%>
<%
	'Program Name				:	VouCNSalInvAmdUpdate.asp
...
dim sDocType,sVouStatus,sAppBy,sAppTy,sExp,NarrNode,iInvNo
...
dim dAdjAmtTotal,sItemCode,sClassCode,objfs
...
''blocked by ragav on Jan 19,2012 to avoid the in balance because we done the same concept in Accounting Stage
''Begin
...
Con.Execute(sQuery)
sQuery = "Delete From Acc_T_CreatedRcvbleAdjDet Where CreatedTransNo = "&iCrTransNo&" "
Con.Execute(sQuery)
sQuery = "Delete From Acc_T_CreatedPybleAdjDet Where CreatedTransNo = "&iCrTransNo&" "
Con.Execute(sQuery)
sQuery = "Delete From Acc_T_CreatedPayables Where CreatedTransNo = "&iCrTransNo&" "
Con.Execute(sQuery)
...
sQuery=sQuery&",'Sales Return ','OS','"&sVouType&"',"&getUserid&",getdate(),NULL,'"&sVouStatus&"', "&iInvNo&",'"&sDispType&"')"
...
for each EntryNode in oNodTaxRoot.childNodes
    ...
    ''blocked and Added BY Ragav on Jan 18,2012
    ''begin
...
if con.Errors.count <>0 then
   ...
   ' Response.End 
	...
	end if 'if objfs.FileExists(server.MapPath("../temp/transaction/Voucher EntryAmd_DN_"&Session.SessionID&".xml")) then
