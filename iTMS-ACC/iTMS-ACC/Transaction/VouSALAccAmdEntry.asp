...
IF TempNode.length <> 0 Then
	For iCtr = 0 To TempNode.length - 1
		...
		IF CStr(document.formname.hFrmOthApp.Value) = "0" Then
			...
			dRndVal = TempNode.Item(iCtr).Attributes.Item(10).nodeValue
		Else
			dRatePer = TempNode.Item(iCtr).Attributes.getNamedItem("RatePer").Value
			sDiscount=FormatNumber(TempNode.Item(iCtr).Attributes.getNamedItem("DisAmount").Value,2,,,0)
			dRndVal = 0
		End IF
		...
		IF CStr(document.formname.hFrmOthApp.Value) = "0" Then
			...
			sAmount = CDbl(sAmount) / CDbl(dRatePer)
			sAmount = CDbl(sAmount) - Cdbl(sDiscount)
			sAmount = CDbl(sAmount) + CDbl(dRndVal)
			sAmount = FormatNumber(sAmount,2,,,0)
		Else
			sAmount = FormatNumber(TempNode.Item(iCtr).Attributes.getNamedItem("Amount").Value,0)
			'MsgBox sAmount
			sAmount = FormatNumber(sAmount,2,,,0)
		End IF
...
if bEditFlag then
	...
	IF TempNode.length <> 0 Then
		...
		IF CStr(document.formname.hFrmOthApp.Value) = "0" Then
			document.formname.txtBagno.value = TempNode.item(0).Attributes.Item(11).nodeValue
		...
		IF CStr(document.formname.hFrmOthApp.Value) = "0" Then
			...
			Else
				document.formname.optRound(1).checked = False
			End IF
		End IF
