...
<%	option explicit	%>
<%
	'Program Name				:	VouAmdPURBkSel.asp
...
<%
dim objRs,sQuery,sFinPeriod,sFinFun,iTransNo,sRetVal,oDOM,sFormVal,objfs
...
Function VouCreate
	...
	if validate then
		...
		IF sTempNode.length <> 0 Then
			sTempNode.Item(0).Attributes.getNamedItem("OrgId").Value = document.formname.horgID.value
			'sTempNode.Item(0).Attributes.getNamedItem("AccUnit").Value = document.formname.selUnitId.value
			sTempNode.Item(0).Text = document.formname.horgName.value 'document.formname.selUnitId.options(document.formname.selUnitId.selectedIndex).text
		End IF
...
Function DispOldVal()
	...
	IF sTempNode.length <> 0 Then
		...
		For iCtr = 0 To document.formname.selPartyType.length - 1
			'alert(document.formname.selPartyType.options(iCtr).value)
			IF document.formname.selPartyType.options(iCtr).value = trim(sTemp) Then
...
End Function
	...
	<BODY leftMargin=0 topMargin=0 MARGINHEIGHT="0" MARGINWIDTH="0" onLoad="DisplayBook('<%=Session("organizationcode")%>');DispOldVal()">

<form method="POST" name="formname" action="VouPURAmdDetails.asp">
