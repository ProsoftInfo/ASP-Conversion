...
Dim iPurInvoiceNo,iCRDRNoteEntryNo,rsTemp,objfs,iItemCode,iClassCode
...
sQuery=sQuery&",'Purchase Invoice ','OP','"&sVouType&"',"&getUserid&",getdate(),NULL,'"&sVouStatus&"', "&ActualTransNo&",'"&sPayAt&"','"&sDrAgain&"')"
...
sQuery = "Insert into ACC_T_CreatedVoucherCRDRNotes (CRDRNoteEntryNo,CROrDRNote,CreatedTransNo,RefType,RefNumber)"&_
         " values("& iCRDRNoteEntryNo &",'DNR',"& iCrTransNo &",'OP',"& iPurInvoiceNo &")"
Response.Write "<p>"& sQuery
con.execute sQuery
...
'	Response.End
	...
	if Trim(sCallFrom)="DR" then
    	Response.Redirect ("VouDNPurReturnDisplay.asp?TransNo="&iCrTransNo)
	else
	    Response.Redirect ("GJVOUCHERS.ASP?ACTN=L")
	end if 'if Trim(sCallFrom)="DR" then
end if
