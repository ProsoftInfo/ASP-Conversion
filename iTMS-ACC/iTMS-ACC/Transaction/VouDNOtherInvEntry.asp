...
<%	option explicit	%>
<%
	'Program Name				:	VouDNOtherInvEntry.asp
...
Dim sFromApp,iItemCode,iClassCode,sSql,rsTemp,sActFor,sActQty,iAmt,sQnty,sQlty,sRat,sCallFrom
Dim iPurInvoiceNo
...
For Each oNodEntry in oNodDeatils.childNodes
	iItemCode = oNodEntry.Attributes.GetNamedItem("ItemCode").value
	iClassCode = oNodEntry.Attributes.GetNamedItem("ClassCode").value
	sSql  = "Select T.ActionFor From RCV_T_ItemActionTaken T,RCV_T_ActualRcptItemDet A Where T.ActionTakenNo = "&iAct&" And "&_
			" A.ReceiptNumber = T.ReceiptNumber And A.EntryNo = T.EntryNo And A.ItemCode = "& iItemCode &" And A.ClassificationCode = "&iClassCode&" "
	'	Response.Write "<P>" &sSql
		...
		End If
		rsTemp.Close
		If trim(sActFor) = "T" then
			sQnty = "Selected"
		ElseIF trim(sActFor) = "L" then
			sQlty = "Selected"
		ElseIF trim(sActFor) = "P" then
			sRat = "selected"
		End If

'Response.Write " sQnty="& sQnty
Next
%>
...
Function SaveXML()
	...
	Dim sExp,TempNode,sCheckVal
	set RootNode=TaxData.documentElement
	setFlag = false

if trim(document.formname.hCallFromDebit.value)="DR" then

	For Each oNodTemp in RootNode.childNodes
	    if oNodTemp.nodeName="Header" then
			oNodTemp.setAttribute "PayableAt",document.formname.SelCrAgain.value
		end if
	...
	End IF
		...
		if objhttp.responseText <> "" then
		    ...
		    document.formname.action = "VouDNPurInvGenerate.asp"
			document.formname.submit()
		end if

else

    document.formname.hVouCode.value = "08"
                            ...
                            <tr>
								...
								<td valign="top" width="100%">
                                                 ...
                                                 <% ' Function Call to Insert Date Picker
                                                    ...
                                                    <Option Value="R" <%=sRat%>>Rate</Option>
                                                    <Option Value="D" >Discount</Option>
                                                    <Option Value="A" <%=sQlty%>>Quality</Option>
                                                    <Option Value="Q" <%=sQnty%>>Quantity</Option>
...
<%
	...
	'Response.Write "dTotal = "& sAmount &"<BR>"
		sSql  = " Select T.ActionFor,T.ActionOnQty,T.Amount,T.EntryNo From RCV_T_ItemActionTaken T, "&_
				" RCV_T_ActualRcptItemDet A Where T.ActionTakenNo = "&iAct&" And A.ReceiptNumber = T.ReceiptNumber "&_
				" And A.EntryNo = T.EntryNo And A.ItemCode = "& iItemCode &" And A.ClassificationCode = "&iClassCode&" "
	'	Response.Write "<P>" &sSql
		rsTemp.Open sSql,con
...
<%
	...
	dim rTotal
		Do while not rsTemp.EOF
			...
			If trim(sActQty) <> "0" then
				sQty = sActQty
			End If
			If 	trim(iAmt) <> "0" then
				sRate = iAmt
			End IF
			sAmount = CDbl(sQty)*CDbl(sRate)

		dTotal = CDbl(dTotal)+CDbl(sAmount)
		rTotal = CDbl(rTotal)+CDbl(sRate)
		'Response.Write "amount="& dTotal &"<BR>"
%>
    ...
    <td class="ExcelInputCell" align="Right"><input type="text" style="text-align: Right" NAME="txtAmount<%=iSno%>" onBlur="setTotal(this,'<%=iSno%>')" value="<%=FormatNumber(sAmount,2,,,0)%>" class="Formelem" size="13">
...
%>
    ...
    <td align="center" ></td>
    <td class="ExcelDisplayCell" align="right"><b><%=FormatNumber(rTotal,2,,,0)%></b></td>
    <td align="right" ></td>
...
%>
    ...
    <td class="ExcelDisplayCell" align="right"> <input type="text" style="text-align: Right" NAME="txtFInvValue"  size="13" value="<%=FormatNumber(dTotal,2,,,0)%>" class="FormelemRead"></td>
    <td align="right" colspan="3"></td>
    <td class="ExcelInputCell" align="right"> <input type="text" style="text-align: Right" NAME="txtInvValue"  size="13" value="<%=FormatNumber(dTotal,2,,,0)%>" class="Formelem">
