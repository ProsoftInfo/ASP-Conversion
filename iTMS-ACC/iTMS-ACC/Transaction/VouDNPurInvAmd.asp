...
Function SaveXML()
	...
	IF CStr(document.formname.hAmdTy.Value) = "A" Then
		sExp = "//Book"
	...
	IF document.formname.SelCrAgain.value = "D"  Then
		sExp = "//Entry"
		Set TempNode = RootNode.SelectNodes(sExp)
		IF TempNode.length <> 0 Then
			For iCtr = 0 To TempNode.length - 1
				Set dDisPer = Eval("document.formname.txtDis"&iCtr+1)
				IF Not IsNumeric(dDisPer.value) then 
				...
				TempNode.Item(iCtr).Attributes.getNamedItem("DisPer").Value = dDisPer.value
				TempNode.Item(iCtr).Attributes.getNamedItem("DisAmount").Value = dDisPer.value
...
Function ResetInvVal()
	...
	'MsgBox document.formname.txtCrNoteValue.value
