...
Function ShowVouch(iCrTransNo)
	showModalDialog "BankVouchView_San.asp?TransNo="&iCrTransNo,"","dialogHeight:410px;dialogWidth:670px;center:Yes;help:No;resizable:No;status:No"
	Exit Function
End Function
%>
...
FUNCTION actionDone()
dim RootNode,AdvRoot,dAmt,dTotal,iAdvNo
	...
	For Each oNodTemp in RootNode.childNodes
		...
		elseif oNodTemp.nodeName="MiscAdvanceDetails" then
	        set MiscRoot=oNodTemp
		end if
	next
	...
	next
	''Added by Ragav on Sep 15,2011
	For each ndChild in MiscRoot.ChildNodes
	    iMiscNo = ndChild.getAttribute("MiscNo")
	    if Eval("document.formname.chkDocumentZ"&iMiscNo).checked then
	        		dToAccount = ndChild.getAttribute("Amount")
					ndChild.setAttribute "TobeAdjAmount",dToAccount
					dTotal=CDbl(dToAccount)+dTotal
	    end if 'if Eval("document.formname.chkDocumentZ"&iMisc).checked then
	Next
    ''end
	...
	'alert AdvanceData.xml

	set objhttp = CreateObject("Microsoft.XMLHTTP")
	objhttp.Open "POST","XMLSave.asp?Mod=OthSal&Name=Voucher Entry", false
...
%>
	...
	<a href="#" LANGUAGE="VBSCRIPT" onclick="ShowVouch(<%=iCrTransNo %>)" class="ExcelDisplayLink"><%=sVouNo%></a></td>
