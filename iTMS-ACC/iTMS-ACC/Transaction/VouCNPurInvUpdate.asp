...
dim sDocType,sVouStatus,sAppBy,sAppTy,sExp,NarrNode
dim iSeriesNo,iSeriesCode,sPurchaseType, ActualTransNo,sDispType
...
dim dAdjAmtTotal,dDiffVal,iCRDRNoteEntryNo,iPurInvoiceNo,sItemCode,sClassCode
...
oDOM.load server.MapPath("../temp/transaction/Voucher Entry_DN_"&Session.SessionID&".xml")
...
for each oNodTemp in oNodRoot.childNodes
	...
	if oNodTemp.nodeName="PurchaseInvoiceEntry" then
	    iPurInvoiceNo = oNodTemp.getAttribute("InvoiceNo")
	    Response.Write "<p> Invoice No = "& iPurInvoiceNo
	end if
next
...
sQuery=sQuery&",'Purchase ADD ','PA','"&sVouType&"',"&getUserid&",getdate(),NULL,'"&sVouStatus&"', "&ActualTransNo&",'"&sDispType&"')"
...
sQuery = "Insert into ACC_T_CreatedVoucherCRDRNotes (CRDRNoteEntryNo,CROrDRNote,CreatedTransNo,RefType,RefNumber)"&_
         " values("& iCRDRNoteEntryNo &",'CNR',"& iCrTransNo &",'PA',"& iPurInvoiceNo &")"
...
for each EntryNode in oNodDeatils.childNodes
	...
	sItemCode = EntryNode.getAttribute("ItemCode")
	sClassCode = EntryNode.getAttribute("ClassCode")
...
'		End IF
		...
		sQuery=sQuery&" '"&sUOM&"',"&dRate&","&dBasicAmount&","&dDisPer&","&dDisAmount&","& sItemCode &","& sClassCode &")"
...
'	Response.End
	...
	'Response.Redirect ("VouCNPurInvDisplay.asp?TransNo="&iCrTransNo&"&DispType="&sDispType)
	if objfs.FileExists(server.MapPath("../temp/transaction/Voucher Entry_DN_"&Session.SessionID&".xml")) then
	    objfs.DeleteFile(server.MapPath("../temp/transaction/Voucher Entry_DN_"&Session.SessionID&".xml"))
	end if
