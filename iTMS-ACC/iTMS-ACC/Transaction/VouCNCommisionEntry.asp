...
<%	option explicit	%>
<%
	'Program Name				:	VouCNCommisionEntry.asp
...
Dim oDom,Root,MainNode,PartyNode
...
Response.Write "test="&Request.Form("hVouDetails")
...
For iCtr = 0 To UBound(sInvTemp)
	...
	MainNode.setAttribute "SalVouNo", sTemp(0)
	MainNode.setAttribute "SalVouDate", sTemp(1)
...
<form method="POST" name="formname" action="VouCNCommGenerate.asp">
<input type="hidden" name="hOrgId" value="<%=sOrgId%>">
<input type="hidden" name="hOrgName" value="<%=sOrgName%>">
<input type="hidden" name="hBookcode" value="<%=iBookNo%>">
<input type="hidden" name="hInvDate" value="<%=sTemp(1)%>">
                    ...
                    &nbsp;
                            ...
                            <tr>
								...
								<td valign="top" width="100%">
									...
									<%IF CStr(sVouchTy) <> "SC" Then %>
										...
										<td><span class="DataOnly"><%=sTemp(0)%>&nbsp;-&nbsp;<%=sTemp(1)%>&nbsp;</span></td>
									<%End IF %>
	                                </tr>
									<tr>
							...
							<tr>
								...
								<td valign="top" width="100%" >
                                                    ...
                                                    <%IF CStr(sVouchTy) <> "SC" Then %>
														<textarea readonly rows="3" name="txtNarration" cols="50" class="FormElem" onKeyPress="ChkEnter()">Commision <%=sTemp(2)%> - <%=sTemp(3)%> For Invoice :<%=sTemp(0)%>-<%=sTemp(1)%></textarea> </td>
													<%Else%>
														<textarea readonly rows="3" name="txtNarration" cols="50" class="FormElem"><%=sNarr%></textarea> </td>
													<%End IF %>
                                                    ...
                                                    <td class="FieldCell" colspan="4">
                                                    <%IF CStr(sVouchTy) <> "SC" Then %>
														<input type="text" name="txtAmount" value="<%=FormatNumber(sTemp(2),2,,,0)%>" size="15" style="text-align:right" class="Formelem"> </td>
													<%Else%>
														<input type="text" name="txtAmount" value="<%=FormatNumber(sAmount,2,,,0)%>" size="15" style="text-align:right" class="Formelem"> </td>
													<%End IF %>
                                                        </tr>
                                                        <tr>
