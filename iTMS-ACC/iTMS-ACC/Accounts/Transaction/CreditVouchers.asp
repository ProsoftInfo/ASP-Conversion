...
<%
	...
	IF Trim(CStr(sBookNo)) = "" Then sBookNo = Request("selBook")
	IF Trim(CStr(sBookNo)) = "" Then sBookNo = Request("hBookVal")
...
<script SRC="../../scripts/CreditVouchersCompat.js"></SCRIPT>
</head>
...
<%
	...
	IF CStr(sSelVouTy) = "A" or CStr(sSelVouTy) = "" or InStr(1,sSelVouTy,CStr("C, P, T"))>0 Then
		if Trim(Cstr(sUnitID))="" then
				...
				& "LEFT JOIN APP_M_PARTYMASTER AS V ON A.PARTYCODE=V.PARTYCODE WHERE A.BOOKCODE='07' AND A.BOOKNUMBER IS NOT NULL AND A.FROMAPPLICATION IS NULL "
		else
			if Trim(Request("hVouNo"))<>"" and CStr(Trim(Request("hVouNo")))<> "0" then
				sSql="SELECT A.CREATEDVOUCHERNO,A.VOUCHERDATE,A.CREATEDTRANSNO,A.TRANSACTIONTYPE,A.VOUCHERAMOUNT,A.ACCOUNTHEAD,A.CREATEDVOUCHSTATUS,A.BANKINSTRUMENTTYPE,V.PARTYNAME,A.PAYABLEAT,A.DRAWNONBANK,A.BOOKNUMBER,A.BOOKCODE FROM ACC_T_CREATEDVOUCHERHEADER AS A " _
				& "LEFT JOIN APP_M_PARTYMASTER AS V ON A.PARTYCODE=V.PARTYCODE WHERE A.BOOKCODE='07'AND A.BOOKNUMBER IS NOT NULL AND A.FROMAPPLICATION IS NULL AND A.OUDEFINITIONID='"&Cstr(sUnitID) &"' AND A.BANKINSTRUMENTTYPE='"&Request("selVouType") &"' "
			else
				sSql="SELECT A.CREATEDVOUCHERNO,A.VOUCHERDATE,A.CREATEDTRANSNO,A.TRANSACTIONTYPE,A.VOUCHERAMOUNT,A.ACCOUNTHEAD,A.CREATEDVOUCHSTATUS,A.BANKINSTRUMENTTYPE,V.PARTYNAME,A.PAYABLEAT,A.DRAWNONBANK,A.BOOKNUMBER,A.BOOKCODE FROM ACC_T_CREATEDVOUCHERHEADER AS A " _
				& "LEFT JOIN APP_M_PARTYMASTER AS V ON A.PARTYCODE=V.PARTYCODE WHERE A.BOOKCODE='07' AND A.BOOKNUMBER IS NOT NULL AND A.FROMAPPLICATION IS NULL AND A.OUDEFINITIONID='"&Cstr(sUnitID) &"' "
			end if
		end if
		IF Cstr(sBookNo) <> "" and Cstr(sBookNo) <> "S" and Cstr(sBookNo) <> "0" and IsNumeric(sBookNo) Then
...
if not aFlag then
	if Trim(Cstr(sUnitID))="" then
			sSql="SELECT A.CREATEDVOUCHERNO,A.VOUCHERDATE,A.CREATEDTRANSNO,A.TRANSACTIONTYPE,A.VOUCHERAMOUNT,A.ACCOUNTHEAD,A.CREATEDVOUCHSTATUS,A.BANKINSTRUMENTTYPE,V.PARTYNAME,A.PAYABLEAT,A.DRAWNONBANK,A.BOOKNUMBER,A.BOOKCODE FROM ACC_T_CREATEDVOUCHERHEADER AS A " _
			& "LEFT JOIN APP_M_PARTYMASTER AS V ON A.PARTYCODE=V.PARTYCODE WHERE A.BOOKCODE='07' AND A.BOOKNUMBER IS NOT NULL AND A.FROMAPPLICATION IS NULL AND(A.CREATEDVOUCHSTATUS='0'"
	else
		if Trim(Request("hVouNo"))<> "" and CStr(Trim(Request("hVouNo")))<> "0" then
			sSql="SELECT A.CREATEDVOUCHERNO,A.VOUCHERDATE,A.CREATEDTRANSNO,A.TRANSACTIONTYPE,A.VOUCHERAMOUNT,A.ACCOUNTHEAD,A.CREATEDVOUCHSTATUS,A.BANKINSTRUMENTTYPE,V.PARTYNAME,A.PAYABLEAT,A.DRAWNONBANK,A.BOOKNUMBER,A.BOOKCODE FROM ACC_T_CREATEDVOUCHERHEADER AS A " _
			& "LEFT JOIN APP_M_PARTYMASTER AS V ON A.PARTYCODE=V.PARTYCODE WHERE A.BOOKCODE='07' AND A.BOOKNUMBER IS NOT NULL AND A.FROMAPPLICATION IS NULL AND A.OUDEFINITIONID='"&Cstr(sUnitID) &"' AND A.BANKINSTRUMENTTYPE='"&Request("selVouType") &"' AND(A.CREATEDVOUCHSTATUS='0' "
		else
			sSql="SELECT A.CREATEDVOUCHERNO,A.VOUCHERDATE,A.CREATEDTRANSNO,A.TRANSACTIONTYPE,A.VOUCHERAMOUNT,A.ACCOUNTHEAD,A.CREATEDVOUCHSTATUS,A.BANKINSTRUMENTTYPE,V.PARTYNAME,A.PAYABLEAT,A.DRAWNONBANK,A.BOOKNUMBER,A.BOOKCODE FROM ACC_T_CREATEDVOUCHERHEADER AS A " _
			& "LEFT JOIN APP_M_PARTYMASTER AS V ON A.PARTYCODE=V.PARTYCODE WHERE A.BOOKCODE='07' AND A.BOOKNUMBER IS NOT NULL AND A.FROMAPPLICATION IS NULL AND A.OUDEFINITIONID='"&Cstr(sUnitID) &"' AND(A.CREATEDVOUCHSTATUS='0' "
		end if
	end if
