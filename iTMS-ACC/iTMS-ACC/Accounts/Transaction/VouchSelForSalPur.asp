...
%>
<%
	'Program Name				:	VouchSelForSalPur.asp
...
dim sBookCode,iBookNo,sTransType,bFlag,sCallTy,sAmount,sUptoDate,sTemp,sFromDate
Dim iPurTy,iParTy,iParCode,sVouchTy,sParTemp
...
sParTemp = Split(iParCode,"?")


'Response.Write iParCode

IF CStr(sVouchTy) = "OIP" Then
	sVouchTy = "OP"
Elseif CStr(sVouchTy) = "OIS" and Cstr(sBookCode) = "07" Then
	sVouchTy = "OS"
Elseif CStr(sVouchTy) = "OIS" and Cstr(sBookCode) = "06" Then
	sVouchTy = "SI"
End IF
...
'Response.Write iParTy &"  " & iParCode
...
function finaldone() {
	...
	if (String(document.formname.hTransTy.value) === "SJR") {
		...
		completeDialog(valueParts[0] + "~" + (textParts[0] || "") + "~" + (valueParts[1] || "") + "~" + (textParts[1] || ""));
		return;
	}
	completeDialog(selectedValue + "~" + (textParts[0] || "") + "~0~" + (textParts[1] || ""));
}
...
if bFlag="C" then
sQuery=" select CreatedTransNo,CreatedVoucherNo,convert(char,VoucherDate,103),str(VoucherAmount,10,2),PayToRecdFrom  from Acc_T_CreatedVoucherHeader Where"&_
...
'IF CStr(iParTy) <> "0" Then
'	sQuery = sQuery &"

else
sQuery=" select CreatedTransNo,CreatedVoucherNo +'-'+ VoucherNumber,convert(char,VoucherDate,103),str(VoucherAmount,10,2),PayToRecdFrom from Acc_T_VoucherHeader Where"&_
				" OUDefinitionID='"&sorgID&"' and BookCode='"&sBookCode&"' and "&_
				"BookNumber="&iBookNo&" and TransactionType='"&sTransType&"' "&_
				"and VoucherStatus in('010104','010106','010107','010108') and "&_
				"convert(datetime,VoucherDate,103) >= Convert(datetime,'"&sFromDate&"',103) "&_
				"and convert(datetime,VoucherDate,103) <= Convert(datetime,'"&sUptoDate&"',103) "&_
				"and CreatedTransNo in (Select CreatedTransNo From Acc_T_CreatedVoucherHeader  "&_
				"Where TransactionType='"&sTransType&"') "
end if

IF CStr(sBookCode) = "07" or CStr(sBookCode) = "06" Then
	sQuery = sQuery &" and isNull(BankInstrumentType,'N') = '"&sVouchTy&"'"

	sQuery = sQuery &" and isNull(PartyType,'') = '"&sParTemp(0)&"' and isNull(PartySubType,0) = "&sParTemp(1)&" and isNull(PartyCode,0) = "&sParTemp(3)&" "

End IF

IF CStr(sBookCode) = "05"  and CStr(iPurTy) <> "0" Then
	sQuery = sQuery &" and isNull(BankInstrumentType,'N') = '"&iPurTy&"'"
End IF
...
<%
	...
	While Not objRs.EOF
		sParCode = objRs(0)
		sAmount = objRs(3)
		IF CStr(sTransType) = "GJR" Then
			sQuery = "Select str(Amount,10,2) From Acc_T_CreatedVoucherDetails Where CreatedTransNo = "&objRs(0)&" "
			with objRs2
				.CursorLocation = 3
				.CursorType = 3
				.Source = sQuery
		...
		IF CStr(sTransType) = "SJR" Then
			Response.Write("<OPTION VALUE="&objRs(0)&"--"&objRs(4)&">"&_
