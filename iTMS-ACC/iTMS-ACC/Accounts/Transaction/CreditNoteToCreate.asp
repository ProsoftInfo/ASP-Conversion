...
'==============================================================================
Function Validate()
	document.formname.hUnitID.value	= document.formname.hUnitID.value
...
dim i,iCnt,sVouType,iBookNo,iChkCnt,objhttp,UnitRoot,sSCVal,sOldAgent,sNewAgent
	...
	Else
		...
		set objhttp = CreateObject("MSXML2.XMLHTTP")
			objhttp.Open "GET","XMLGetOrgBookCountGJ.asp?BkCode=07&orgID=" & sUnit , false
			objhttp.send
			'alert(objhttp.responsetext)
			if objhttp.responseXML.xml <> "" then
				'alert(objhttp.responseXML.xml)
	...
	ElseIF trim(document.formname.hInvVal.value) = "SR" then
		'document.formname.action = "VouCNOtherInvEntry.asp?hCallFrom="& sPara
		document.formname.action = "VouCNSalesReturnEntry.asp?hCallFrom="& sPara
	...
	ElseIF trim(document.formname.hInvVal.value) = "OT" then
		document.formname.action = "VouCNOthersEntry.asp?CallFrom="&sVouType
	ElseIF trim(document.formname.hInvVal.value) = "MI" then
        document.formname.action = "VouCNThrMiscRec.asp?CallFrom="&sVouType
	End IF
	document.formname.submit
