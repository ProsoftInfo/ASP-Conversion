...
<%	option explicit	%>
<%
	'Program Name				:	VouSALAmdAdvance.asp
...
<form method="POST" name="formname" action="VouSALAmdGenerate.asp">
<table border="0" width="100%" cellspacing="0" cellpadding="0">
	<tr>
		<td align="center" class=PageTitle height="20"><p align="center">Sales Voucher Amendment 	</td>
    </tr>
	<tr>
...
'		 "b.BankInstrumentType,b.BankInstrumentNo,convert(char,b.BankInstrumentDate,103),DrawnOnBank, isNull(C.AdvanceAdjusted,0) AdvAdj "&_
...
'		 " and b.VoucherDate<= convert(datetime,'"&sVouDate&"',103) and B.CreatedTransNo = C.CreatedTransNo "

'sQuery = "Select Distinct TransactionNumber,VoucherNumber,VoucherDate,AdvanceAdjusted, "&_
'		 "AdvanceReceived,TransactionType,PayToRecdFrom,BankInstrumentType,BankInstrumentNo, "&_
'		 "BankInstrumentDate,DrawnOnBank,CreatedTransNo,CreatedAdvanceNo,isNull(AmountAdjusted,0),isNull(ToAccount,0) ToAccount,isNull(AdjCrTransNo,0) From  "&_
'		 "VwSalAdvanceDet Where OUDefinitionID = '"&sOrgId&"' and PartyType = '"&sParType&"' and PartySubType = "&sParSubType&" "&_
'		 "and PartyCode = "&sParCode&" and CONVERT(DATETIME,VOUCHERDATE,103)<= CONVERT(DATETIME,'"&sVouDate&"',103) "

'		 '"and AdjCrTransNo = "&iCrTransNo&" "

sQuery = "select a.TransactionNumber,b.VoucherNumber,convert(char,b.VoucherDate,103),isnull(a.AdvanceAdjusted,0), "&_
		 "isnull(a.AdvanceReceived,0),b.TransactionType,b.PayToRecdFrom,isNull(b.BankInstrumentType,''), "&_
		 "isNull(b.BankInstrumentNo,''),isNull(convert(char,b.BankInstrumentDate,103),'01/01/1900'),isNull(DrawnOnBank,''), A.CreatedTransNo, "&_
		 "A.AdvanceNumber,isNull(C.AdvanceAdjusted,0) - isnull(a.AdvanceAdjusted,0) ToAccount,C.CreatedAdvanceNo  "&_
...
'Set dTransAdjAmt = objRs(13)
'Set iAdjTransNo = objRs(15)
...
%>
        ...
        <td class="ExcelDisplayCell" align="right"><%=FormatNumber(dAmtAdjusted,2,,,0)%></td>
        <td class="ExcelInputCell" align="right">
        <input type="text" style="text-align: Right"  name="txtAmount<%=iDocNo%>Z0" value="<%=FormatNumber(cdbl(dAmtReceived)-cdbl(dAmtAdjusted),2,,,0)%>" size="15" maxlength="13" class="Formelem"> </td>
            </tr>
 <%
		iSno = iSno + 1
		objRs.MoveNext
