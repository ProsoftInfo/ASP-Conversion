...
Dim sFinPeriod,sFinFrom,sFinTO
Dim iCnt
...
sSelPaymentType = Request("radPayment")
...
if trim(sSelPaymentType)="" or IsNull(sSelPaymentType) then sSelPaymentType="B" 
...
function Init() {
	dateControl("ctlFromDate").setDate(document.formname.hFromDate.value);
	dateControl("ctlToDate").setDate(document.formname.hToDate.value);
}
</script>
</head>
	...
	<table border="0" width="100%" cellspacing="0" cellpadding="0">
		<tr>
			<td align="center" class="PageTitle">
				<p align="center">PREPAYMENTS (IN/OUT)  
			</td>
		</tr>
...
<tr>
	...
	<td class="FieldCellSub">
	    ...
	    <img src="../../assets/images/iTMS%20Icons/EntryIcon.gif" onclick="SelectParty()" style="cursor:hand;" alt="Click here to select party" />
	</td>
</tr>
<tr>
...
<tr>
	...
	<td class="FieldCellSub">
	    <input type="radio" name="radPayment" value="I" <%if trim(sSelPaymentType)="I" then response.write "Checked" %> >Issued &nbsp;
	    <input type="radio" name="radPayment" value="T" <%if trim(sSelPaymentType)="T" then response.write "Checked" %> >To Issue &nbsp;
	    <input type="radio" name="radPayment" value="B" <%if trim(sSelPaymentType)="B" then response.write "Checked" %> >Both &nbsp;
	</td>
</tr>
<tr>
...
<tr>
    <td class="FieldCell" colspan="3" align="center">
        <b>Payment Listing From <span id="spanFrom"><%=sFromDate%>&nbsp;</span> To <span id="spanTo"><%=sToDate%>&nbsp;</span></b>
    </td>
</tr>
...
<td class="ExcelHeaderCell" align="center" >Party
</td>
<td class="ExcelHeaderCell" align="center" >Issued/To Issue
</td>
...
<td class="ExcelHeaderCell" align="center" >Bank
</td>
</tr>
<%
    ...
    sQuery = sQuery &" and Convert(datetime,VoucherDate,103) between Convert(datetime,'"& sFromDate &"',103) and Convert(datetime,'"& sToDate &"',103) and E.AccUnitPartyCode <>0 "
    
    if trim(sSelPartyCode)<>"" then
        sQuery = sQuery & " and E.AccUnitPartyCode = "& sSelPartyCode
    end if
    
     if trim(sSelPaymentType)="T" then
        sQuery = sQuery & " and A.CreatedTransNo not in (Select CreatedTransNo from Acc_T_VoucherHeader where BookCode = '02')"
    elseif trim(sSelPaymentType)="I" then
        sQuery = sQuery & " and A.CreatedTransNo in (Select CreatedTransNo from Acc_T_VoucherHeader where BookCode = '02')"
    end if
                ...
                <tr>
                    ...
                    <td class="excelDisplayCell" align="center">
                        <%
                            ...
                            sQuery = "Select * from Acc_T_VoucherHeader where CreatedTransNo in ("& rsObj(0) &")"
                            rsTemp.open sQuery,con
                            if not rsTemp.eof then
                                response.write "Issued"
                            else
                                response.write "To Issue"
                            end if
                            rsTemp.close
                        %>
                    </td>
                     <td class="excelDisplayCell" align="center">
                        <a href="#" class="ExcelDisplayLink" onclick="ShowVouch('<%=rsObj(0)%>')"><%=FormatNumber(trim(rsObj(9)),2)%></a>
                    </td>
                    <td class="excelDisplayCell" align="center">
                        <%=trim(rsObj(11))%>
                    </td>
