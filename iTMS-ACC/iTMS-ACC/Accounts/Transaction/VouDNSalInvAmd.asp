...
<%	option explicit	%>
<%
	'Program Name				:	VouDNSalInvAmd.asp
...
Dim ObjInv,sNarration,ObjDom,InvNode,dOrgTotalInv,iCrAltAccHd,sCrAltAccName,sAccChg
Dim iBookNo,sVouDate,sRetVal,iItemCode,iClassCode
Dim dInvInvAmount
...
'ObjDom.load server.MapPath("../xmldata/Voucher/"&iInvNo&".xml")
...
Set InvNode = ObjDom.documentElement

for each oInvNodHeader in InvNode.childNodes
...
ObjDom.save server.MapPath("../temp/transaction/InvDet_ForDN_"&Session.SessionID&".xml")
...
Function ResetInvVal()
	...
	Set TempNode = InvRoot.selectNodes(sExp)
	IF TempNode.Length <> 0 Then
		document.formname.txtFInvValue.Value = FormatNumber(TempNode.Item(0).Attributes.getNamedItem("NettValue").Value,2,,,0)
	End IF
...
Function ResetCrVal()
	...
	Set TempNode = InvRoot.selectNodes(sExp)
	IF TempNode.Length <> 0 Then
		document.formname.txtInvValue.Value = FormatNumber(TempNode.Item(0).Attributes.getNamedItem("NettValue").Value,2,,,0)
	End IF
...
Function SetInvDate()
	Dim sVouDate
	sVouDate = document.formname.hVouDate.Value
	IF IsDate(sVouDate) Then
...
<BODY leftMargin=0 topMargin=0 MARGINHEIGHT="0" MARGINWIDTH="0" onLoad="Init('<%=sCrAgain%>');SetInvDate();">
<form method="POST" name="formname" action="VouDNSalInvAmdUpdate.asp">
...
<table border="0" width="100%" cellspacing="0" cellpadding="0">
	<tr>
		...
		Debit Note For Sales Invoice Amendment
          		</td>
    </tr>
	<tr>
		...
		if oNodtemp.length> 0 then
            ...
            elseif sCrAgain = "D" then
        		...
        		%>
                    ...
                    <td class="ExcelDisplayCell" align="Right" id="tQty<%=isno%>"><input type="text" style="text-align: Right" NAME="txtqty<%=isno%>" onkeypress="DoKeyPress('Y',12,2)" onBlur="setQty(this,'<%=iSno%>','Q')" value="<%=FormatNumber(sQty,3,,,0)%>" class="FormelemRead" size="13"></td>
                    <td class="ExcelDisplayCell" align="Right" id="tRate<%=isno%>"><input type="text" style="text-align: Right" NAME="txtRate<%=isno%>" onkeypress="DoKeyPress('Y',12,2)" onBlur="setQty(txtqty<%=isno%>,'<%=iSno%>','R')" value="<%=FormatNumber(sRate,2,,,0)%>" class="FormelemRead" size="13"></td>
...
<%
	Dim sCheckExp,CheckNode,dOrgRndVal,dRndVal,dOrgTaxVal
'	dim dInvAmount
'	dInvAmount=dTotal
	Dim iCheck
	...
	For Each oInvNodEntry in oInvNodTaxRoot.childNodes
	    ...
	    For Each oNodEntry in oNodTaxRoot.childNodes
		    ...
		    If sCatCode = "0" and sTaxCode = "0" and sTaxMode = "0" Then
			    dTax = oNodTaxRoot.Attributes.getNamedItem("RoundOffValue").Value
		    Else
			    sCheckExp = "//TaxDetails/Tax[@CatCode="&sCatCode&" and @TaxCode="&sTaxCode&" and @RoundOff]"
			    Set CheckNode = oNodRoot.selectNodes(sCheckExp)
			    IF CheckNode.length <> 0 Then
				    dTax = oNodEntry.Attributes.GetNamedItem("RoundOff").value
			    Else
				    dTax = 0
			    End IF
		    End If
...
%>
    ...
    <td class="ExcelDisplayCell" align="right"> <input type="text" style="text-align: Right" NAME="txtFInvValue"  size="13" value="<%=FormatNumber(Round(dInvInvAmount,0),2,,,0)%>" class="FormelemRead"></td>
    <td align="right" colspan="3"></td>
