...
%>
<%
	'Program Name				:	PaymentReminders.asp
...
<script type="application/xml" data-itms-xml-island="1" id="OutStandingData"><Root></Root></script>
...
<table cellspacing="1px" class="ExcelTable" width="100%" >
	...
	<td class="ExcelHeaderCell">Received From
	</td>
	<td class="ExcelHeaderCell">Received On
	</td>
...
sSql = "Select R.ReminderNo,ReminderToPartyCode,Convert(varchar,ReminderDate,103),ReminderReason,ActionTaken,PartyInvoiceNo "&_
       " from APP_R_ApplicationReminders R,ACC_T_OverDueReminderDet T where R.ReminderNo = T.ReminderNo and R.PartyType = 'CR' "&_
       ...
       if not Objrs.EOF then
            do while not Objrs.EOF
                ...
                objrs1.Close
                sSql = "Select PartySubType from APP_R_OrgParty where PartyCode = "& Objrs(1) &"  and PartyType = 'CR'"
                objrs1.Open sSql,con
                if not objrs1.EOF then
            ...
            %>
                <tr>
	                ...
	                <td class="ExcelDisplayCell" align="center">
	                    ...
	                    <input type=hidden name="hPartyDetZ<%=iSNo%>" value="<%=objrs(1)%>:CR:<%=sPartySubType%>:<%=objrs(5)%>">
	                </td>
	                <td class="ExcelDisplayCell" align="Left">
	                    <a href="#" class="ExcelDisplayLink" onclick="return false;"><%=sPartyName%></a>
	                </td>
	                <td class="ExcelDisplayCell" align="center"><%=objrs(2)%>
	                </td>
	                <td class="ExcelDisplayCell" align="center"><%=objrs(3)%>
	                </td>
	                <td class="ExcelDisplayCell" align="center"><%=objrs(4)%>
	                </td>
                </tr>
              <%
                Objrs.MoveNext
...
<%		else%>
<input type="button" value=" |< " class="ActionButtonX" onclick="PaginateAcc('1')" id=button7 name=button7>
<input type="button" value=" << " class="ActionButtonX" onclick="PaginateAcc('<%=iCurrentPage - 1%>')" id=button6 name=button6>
<%		end if	%>
...
<td valign="middle">
	<table border="0" cellpadding="0" cellspacing="0" width="100%">
		<tr>
			<td class="ActionCell">
                 <input type="button" value="Add Reminder" class="ActionButtonX"  id="button1" name=button1 OnClick="CreateRem()">
                 <input type="button" value="View Reminder"  class="ActionButtonX"  id="button2" name=button2 OnClick="ViewRem()">
			</td>
		</tr>
	</table>
</td>
