...
Dim Root
Set Root = SaleData.documentElement


Function ShowInvoice(sPassInvNo)
	document.formname.hInvno.value = sPassInvNo
	document.formname.action = "../../sales/TRANSACTION/AmndSaltrInvoice.asp"
...
FUNCTION checkSubmit()
	...
	IF Cstr(document.formname.hExpInv.Value) = "Y" Then
		set objhttp = CreateObject("Microsoft.XMLHTTP")
		objhttp.Open "POST","XMLSalSave.asp?TransNo="&document.formname.hTransNo.Value, false
		objhttp.send SaleData.XMLDocument
	End IF
...
FUNCTION finalCancel()
	document.formname.action="AppOtherAppSelection.asp"
	document.formname.submit
END FUNCTION

Function ViewInv(iInv)
	Dim sPassStr
	'alert(document.formname.hTransNo.Value)
	sPassStr = "TransNo="&document.formname.hTransNo.Value
	showModalDialog "SalesInvView.asp?"&sPassStr,"A","dialogHeight:600px;dialogWidth:800px;center:Yes;help:No;resizable:No;status:No"
End Function
...
Function CalNewTax()
	Dim dConRate,sExp,TempNode,iCtr
	dConRate = document.formname.txtConRate.Value
	Dim Root
	Set Root = SaleData.documentElement
	...
	For iCtr = 0 To TempNode.Length - 1
		TempNode.Item(iCtr).Attributes.getNamedItem("Amount").Value = FormatNumber(Round(Cdbl(TempNode.Item(iCtr).Attributes.getNamedItem("Amount").Value) * Cdbl(dConRate),2),2,,,0)
		TempNode.Item(iCtr).Attributes.getNamedItem("TransBasicamt").Value = FormatNumber(Round(Cdbl(TempNode.Item(iCtr).Attributes.getNamedItem("TransBasicamt").Value) * Cdbl(dConRate),2),2,,,0)
		TempNode.Item(iCtr).Attributes.getNamedItem("DisAmount").Value = FormatNumber(Round(Cdbl(TempNode.Item(iCtr).Attributes.getNamedItem("DisAmount").Value) * Cdbl(dConRate),2),2,,,0)
	...
	Set TempNode = Root.selectNodes(sExp)
	For iCtr = 0 To TempNode.Length - 1
		TempNode.Item(iCtr).Attributes.Item(0).Value = FormatNumber(Round(Cdbl(TempNode.Item(iCtr).Attributes.Item(0).Value) * Cdbl(dConRate),2),2,,,0)
		TempNode.Item(iCtr).Attributes.Item(1).Value = FormatNumber(Round(Cdbl(TempNode.Item(iCtr).Attributes.Item(1).Value) * Cdbl(dConRate),2),2,,,0)
		TempNode.Item(iCtr).Attributes.Item(2).Value = FormatNumber(Round(Cdbl(TempNode.Item(iCtr).Attributes.Item(2).Value) * Cdbl(dConRate),2),2,,,0)
	Next
	...
	Set TempNode = Root.selectNodes(sExp)
	For iCtr = 0 To TempNode.Length - 1
		TempNode.Item(iCtr).Attributes.Item(5).Value = FormatNumber(Round(Cdbl(TempNode.Item(iCtr).Attributes.Item(5).Value) * Cdbl(dConRate),2),2,,,0)
	Next

	sExp = "//Entry"
	Set TempNode = Root.selectNodes(sExp)
	For iCtr = 0 To TempNode.Length - 1
		document.formname.txtBasVal.Value = TempNode.Item(iCtr).Attributes.getNamedItem("TransBasicamt").Value
		document.formname.txtDisVal.Value = TempNode.Item(iCtr).Attributes.getNamedItem("DisAmount").Value
		document.formname.txtNetVal.Value = TempNode.Item(iCtr).Attributes.getNamedItem("Amount").Value
	Next

	sExp = "//TaxDetails"
	Set TempNode = Root.selectNodes(sExp)
	For iCtr = 0 To TempNode.Length - 1
		document.formname.txtInvVal.Value = TempNode.Item(iCtr).Attributes.Item(0).Value
		document.formname.txtTotNetVal.Value = TempNode.Item(iCtr).Attributes.Item(1).Value
		document.formname.txtTotBasVal.Value = TempNode.Item(iCtr).Attributes.Item(1).Value
		document.formname.txtTotDisVal.Value = "0.00"
	Next

	sExp = "//Tax"
	Set TempNode = Root.selectNodes(sExp)
	For iCtr = 0 To TempNode.Length - 1
		iTaxCd = TempNode.Item(iCtr).Attributes.Item(1).Value
		iCatCd = TempNode.Item(iCtr).Attributes.Item(0).Value
		Set dTaxVal = Eval("document.formname.txtTaxVal"&iCatCd&"Z"&iTaxCd)
		dTaxVal.Value = TempNode.Item(iCtr).Attributes.Item(5).Value
	Next
