...
'*****************************************************************************
Function init()
'	For iCnt = 0 to CInt(document.formname.selOrganisaion.length)-1
'		if document.formname.selOrganisaion(iCnt).value = document.formname.hOrgCode.value then
'			document.formname.selOrganisaion.selectedIndex = iCnt
'		end if
'	Next

	if 	document.formname.hContra.value	="1" then
		document.formname.chkContra.checked = true
	else
		document.formname.chkSummary.checked = false
	end if

	if 	document.formname.hSubLed.value = "1" then
		document.formname.chkParty.checked = true
	else
		document.formname.chkSummary.checked = false
	end if

	if document.formname.hTDS.value="1" then
		document.formname.chkTDS.checked = true
	else
		document.formname.chkSummary.checked = false
	end if

	if document.formname.hSumPosting.value="1" then
		document.formname.chkSummary.checked = true
	else
		document.formname.chkSummary.checked = false
	end if
...
Function CheckSubmit()
	'document.formname.hOrgCode.value = document.formname.selOrganisaion(document.formname.selOrganisaion.selectedIndex).value

	if 	document.formname.chkContra.checked = true  then
		document.formname.hContra.value	="1"
	else
		document.formname.hContra.value	="0"
	end if

	if 	document.formname.chkParty.checked = true then
		document.formname.hSubLed.value = "1"
	else
		document.formname.hSubLed.value = "0"
	end if

	if document.formname.chkTDS.checked = true then
		document.formname.hTDS.value="1"
	else
		document.formname.hTDS.value="0"
	end if

	if document.formname.chkSummary.checked = true then
		 document.formname.hSumPosting.value="1"
	else
		document.formname.hSumPosting.value="0"
	end if
...
Function GetAccHead(obj)
	Dim sGroupCode,objhttp,ndRoot,ndChild
	sGroupCode = obj.value
	set objhttp = CreateObject("Microsoft.XMLHTTP")
	objhttp.open "GET","GetAccHeadForGLGroup.asp?GCode="& sGroupCode,false
	objhttp.send
	if trim(objhttp.responseXML.xml)<>"" then
		AccHeadData.LoadXML(objhttp.responseXML.xml)
	else
		alert(objhttp.responseText)
		exit function
	end if
	set ndRoot = AccHeadData.documentElement
	if ndRoot.hasChildNodes() then
		document.formname.selGLGroup.length = 1
		for each ndChild in ndRoot.childNodes
			document.formname.selGLGroup.length = document.formname.selGLGroup.length+ 1
			document.formname.selGLGroup(document.formname.selGLGroup.length-1).value = ndChild.getAttribute("No")
			document.formname.selGLGroup(document.formname.selGLGroup.length-1).text =  ndChild.getAttribute("Name")
		next
...
End Function
'**********************************************************************
Function AccountsCategory(Index)
	document.formname.action = "GLACCHEADGRID.ASP?Category="&eval("document.formname.radAccHead("&Index&")").value
	document.formname.submit
End Function
'*************************************************************
Function EditAccHead(sUnitID,sAccNo,sAccName,sGrpCode,sGrpName)
	document.formname.selUnitId.value = sUnitID
	document.formname.hHeadValue.value = sAccNo
	document.formname.hHeadName.value = sAccName
	document.formname.GCode.value = sGrpCode
	document.formname.GName.value = sGrpName
