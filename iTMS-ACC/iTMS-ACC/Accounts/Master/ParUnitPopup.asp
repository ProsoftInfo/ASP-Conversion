...
'*******************************
Function AddNew()
    sValue = showModalDialog("ParTypeCreatEntryPopup.asp","","Status:No;Help:No;")
    if sValue="Done" then
		...
		For Each nodHeader in Root.childNodes
			if nodHeader.nodeName="Units" then
				For each nodUnit in nodHeader.childNodes
					For each ndChild in nodUnit.childNodes
						nodUnit.removeChild(ndChild)
					next
				Next
			end if 'if nodHeader.nodeName="Units" then
		Next
	...
	For Each nodHeader In Root.childNodes
		if StrComp(nodHeader.nodeName,"Units") = 0 then
			for Each nodUnit in nodHeader.childNodes
				...
				'alert(iUnitid)
				iRowCnt =eval("document.formname.hRowCntUnitZ"&iUnitId).value
				for iCnt = 1 to cint(iRowCnt)
					if eval("document.formname.chkParType"&iUnitId&"Z"&iCnt).checked=true then
						sParValue= sParValue &","& Trim(eval("document.formname.chkParType"&iUnitId&"Z"&iCnt).value)
					end if ' if eval("document.formname.chkParType"&iUnitId).checked=true then
				next
				'alert(sParValue)
				if sParValue<>"" then
					sParValue = mid(sParValue,2)
					arrParType=Split(sParValue,",")
					if UBound(arrParType) =0 then
							sTemp=Split(trim(arrParType(0)),"?")
							Set newElem = PartyData.createElement("Partytype")
							newElem.setAttribute "Type", sTemp(0)
							newElem.setAttribute "SubType", sTemp(1)
							newElem.setAttribute "OpBalance", eval("document.formname.txtBalance"&iUnitId&"Z"&sTemp(0)&"Z"&sTemp(1)&"Z"&sTemp(2)).value

							if eval("document.formname.radCRDR"&iUnitId&"Z"&sTemp(0)&"Z"&sTemp(1)&"Z"&sTemp(2))(0).checked = true then
								newElem.setAttribute "OpCRDR",eval("document.formname.radCRDR"&iUnitId&"Z"&sTemp(0)&"Z"&sTemp(1)&"Z"&sTemp(2))(0).value
							else
								newElem.setAttribute "OpCRDR",eval("document.formname.radCRDR"&iUnitId&"Z"&sTemp(0)&"Z"&sTemp(1)&"Z"&sTemp(2))(1).value
							end if

							newElem.setAttribute "OpeningMonthYear",  document.formname.hFinFromYear.value
							newElem.setAttribute "ClosingMonthYear",  document.formname.hFinToYear.value
							nodUnit.appendChild newElem
							'Hard Coded Value for Agent type

							if trim(sTemp(0))="CR" and cint(sTemp(1))< 3 then
								bAgentFlag=true
							end if
					else
						for iCounter=0 to UBound(arrParType)
							sTemp=Split(trim(arrParType(iCounter)),"?")
...
Set newElem = PartyData.createElement("Agent")
if bAgentFlag then
...
End Function
'********************************************
Function CheckPartyChk(sChkVal,sObj)
	IF CStr(sChkVal) = "A"  Then 'Reference Can't be Removed
		MsgBox "Party Type has Transaction Entry Not able to Remove the Party Type Reference!! "
		sObj.checked = True
	End IF
End Function
</SCRIPT>
