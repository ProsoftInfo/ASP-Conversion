		...
		for each ndChild in ndRoot.childNodes
			if ndChild.nodeName = "Analytical" then
				for each ndAN in ndChild.childNodes
					sArrAnyCode = ndAN.getAttribute("UNCode")&":"&ndAN.getAttribute("Code")&":"&ndAN.getAttribute("GRCode")
					For iCnt = 1 to cint(document.formname.hTotalAnalCode.value)
						if eval("document.formname.chkAnalyticalZ"&iCnt).value = sArrAnyCode then
							eval("document.formname.chkAnalyticalZ"&iCnt).checked = true
							exit for
						end if
					Next
...
Function CheckSubmit()
	Dim ndChild,ndApp,sAnaFlag
	sAnaFlag = False
	if ndRoot.hasChildNodes() then
		for each ndChild in ndRoot.childNodes
			if ndChild.nodeName = "Analytical" then
				for each ndAN in ndChild.childNodes
					if ndAn.getAttribute("UNCode") = document.formname.hUnitCode.value then
	...
	if not sAnaFlag then
		set ndChild = objTemp.createElement("Analytical")
		ndRoot.appendChild ndChild
	end if

	For iCnt = 1 to cint(document.formname.hTotalAnalCode.value)
		if eval("document.formname.chkAnalyticalZ"&iCnt).checked = true then
			sArrAnyCode = split(eval("document.formname.chkAnalyticalZ"&iCnt).value,":")
			set ndAN = objTemp.createElement("AN")
				ndAN.setAttribute "UNCode", sArrAnyCode(0)
				ndAN.setAttribute "Code", sArrAnyCode(1)
				ndAN.setAttribute "GRCode", sArrAnyCode(2)
			ndChild.appendChild ndAN
		end if 'if eval("document.formname.chkAnalyticalZ"&iCnt).checked = true then
	Next
	window.close
End Function
'=====================
