	...
	if ndRoot.hasChildNodes() then
		for each ndChild in ndRoot.childNodes
			if ndChild.nodeName = "CostCenter" then
				for each ndCC in ndChild.childNodes
					sArrAnyCode = ndCC.getAttribute("UNCode")&":"&ndCC.getAttribute("GRCode")&":"&ndCC.getAttribute("CCCode")
					For iCnt = 1 to cint(document.formname.hTotalCostCenter.value)
						if eval("document.formname.chkCostCenter"&iCnt).value = sArrAnyCode then
							eval("document.formname.chkCostCenter"&iCnt).checked = true
							exit for
						end if
					Next
				next 'for each ndAN in ndChild.childNodes
			end if 'if ndAN.nodeName = "Analytical" then
		next ' for each ndChild in ndRoot.childNodes
	end if 'if ndRoot.hasChildNodes() then
End Function
'**********************************************
Function CheckSubmit()
Dim sCCFlag,ndChild,ndCC
	sCCFlag = False
	if ndRoot.hasChildNodes() then
		for each ndChild in ndRoot.childNodes
			if ndChild.nodeName = "CostCenter" then
				for each ndCC in ndChild.childNodes
					if ndCC.getAttribute("UNCode") = document.formname.hUnitCode.value then
						ndChild.removeChild ndCC
					end if 'if ndAn.getAttribute("UNCode") = document.formname.hUnitCode.value then
				next
				sCCFlag = True
				exit for
			end if 'if ndAN.nodeName = "CostCenter" then
		next ' for each ndChild in ndRoot.childNodes
	end if 'if ndRoot.hasChildNodes() then

	if not sCCFlag then
		set ndChild = objTemp.createElement("CostCenter")
		ndRoot.appendChild ndChild
	end if

	For iCnt = 1 to cint(document.formname.hTotalCostCenter.value)
		if eval("document.formname.chkCostCenter"&iCnt).checked = true then
			sArrAnyCode = split(eval("document.formname.chkCostCenter"&iCnt).value,":")
			set ndCC = objTemp.createElement("CC")
				ndCC.setAttribute "UNCode", sArrAnyCode(0)
				ndCC.setAttribute "GRCode", sArrAnyCode(1)
				ndCC.setAttribute "CCCode", sArrAnyCode(2)
			ndChild.appendChild ndCC
		end if 'if eval("document.formname.chkCostCenter"&iCnt).checked = true then
	Next
	window.close
End Function
'=====================
