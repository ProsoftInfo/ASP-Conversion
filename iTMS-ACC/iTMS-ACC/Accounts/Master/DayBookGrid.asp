...
Function CreateNewParty()
	document.formname.action = "BooksCreationEntry.asp"
...
Function CheckSubmit()
	document.formname.hBookName.value = document.formname.txtBookName.value
	document.formname.hBookType.value = document.formname.selBookType(document.formname.selBookType.selectedIndex).value
	'document.formname.hOrgCode.value = document.formname.selOrganisaion(document.formname.selOrganisaion.selectedIndex).value
	document.formname.hBookTypeName.value = document.formname.selBookType(document.formname.selBookType.selectedIndex).text
	document.formname.submit
End Function
'************************************************
Function EditBook(sOrgCode,sBookCode,sBookNumber,FromAccHead)
Dim ReturnValue
	ReturnValue = showModalDialog("BooksEditEntryPopup.asp?OrgCode="&sOrgCode&"&BookCode="& sBookCode&"&BookNumber="&sBookNumber&"&FromAcc="&FromAccHead ,"","Status:No;")
...
End Function
'************************************************
Function GlChange(sOrgId,iBookNo,iBookId)
	dim iGlHead,saTemp,sRetVal,sAccName

	sRetVal = ""
	iGlHead =  ShowGLHead(sOrgId)

	if CInt(iGlHead) >0 then
		set objhttp = CreateObject("MSXML2.XMLHTTP")

		objhttp.Open "GET","GetAccountHeadMaped.asp?OrgCode=" & sOrgId&"&AccHead="&iGlHead, false
		objhttp.send
		if Trim(objhttp.responseText)<>"" then
		    sCount = objhttp.responseText
		    if IsNumeric(sCount) then
		        if cdbl(sCount) > 0 then
		            alert("Selected account head has already been mapped to another daybook. Please select some other account head")
		            sRetVal = ""
		        end if
		        do while CDbl(sCount)<>0
		            iGlHead =  ShowGLHead(sOrgId)
		            objhttp.Open "GET","GetAccountHeadMaped.asp?OrgCode=" & sOrgId&"&AccHead="&iGlHead, false
		            objhttp.send
		            if Trim(objhttp.responseText)<>"" then
    ...
    if CInt(iGlHead) >0 then
        set objhttp = CreateObject("MSXML2.XMLHTTP")
		objhttp.Open "GET","BooksCodeUpdate.asp?orgID=" & sOrgId&"&BookId="&iBookId&"&BookNo="&iBookNo&"&GlHead="&iGlHead, false
		objhttp.send
...
End Function
'***********************************************
Function ViewContraDet(sOrgCode,FromAccHead)
	Dim Retrunvalue
	Returnvalue = showModalDialog("ContraListPopup.asp?OrgCode="&sOrgCode&"&FromAcc="&FromAccHead,"","dialogHeight:430px;dialogWidth:465px;Status:No;Help:No;")
	if Returnvalue="Done" then
		document.formname.submit
	end if
End Function
