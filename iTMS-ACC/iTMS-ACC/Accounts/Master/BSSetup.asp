...
<%
	...
	Do WHile Not Objrs1.EOF
		...
		'Response.Write "<p style='color:red'>" & CStr(sEntTy)
				...
				<tr>
					...
					<%IF CStr(sEntTy) = "D" Then 'IF it is an Data Entry the User Will enter the Value %>
						<td class="ExcelInputCell" align="right">
							<input type="text" name="txtCurrVal<%=sCatCode%>?<%=iBSSubID%>?<%=iBSSubSubID%>" size="15" value="<%=FormatNumber(dPrvVal,2,,,0)%>" style="text-align:right" maxlength="13" class="FormElem">						
						</td>
					<%ElseIF CStr(sEntTy) = "S" Then 'If is is and Scheduled Type
					
						'N,D,A,S - entrytype
						'kalai check : if data is exist in bs display it
						'else
						'display data from sch table
						
						sQuery = "Select isNull(S.ApplicableACHeadCode,0),S.ComputeMode,isNull(S.ScheduleSubHeadValue,0),isNull(V.EntryType,'') EntryType  "&_
								 " From Acc_T_ScheduleACDetail S, ACC_T_BSACDetail P, Vw_Acc_SchSetup V " &_
								 " Where P.BSHeadID = "&sCatCode&" and P.BSSubID = "&iBSSubID&" "&_
								 " and P.BSSubSubID = "&iBSSubSubID&" and P.ScheduleID = S.ScheduleID "&_
								 " and P.ScheduleSubID = S.ScheduleSubID and P.ScheduleSubSubID = S.ScheduleSubSubID " & _
								 ...
								 " and V.ScheduleID = S.ScheduleID "&_
								 " and V.ScheduleSubID = S.ScheduleSubID and V.ScheduleSubSubID = S.ScheduleSubSubID " & _
						...
						IF Not Objrs2.EOF Then
							...
							Do While Not Objrs2.EOF
								...
								'Response.Write "<p style='color:red'>" & Objrs2("EntryType")
								
								if Objrs2("EntryType") = "D" then
									'IF CStr(Objrs2(1)) = "+" Then
										dAddTot = cdbl(dAddTot) + cdbl(Objrs2(2))
									'ElseIF CStr(Objrs2(1)) = "-" Then
									'	dSubTot = cdbl(dSubTot) + cdbl(Objrs2(2))
									'End IF
								elseif Objrs2("EntryType") = "N" then
								
								elseif Objrs2("EntryType") = "A" then
								...
								elseif Objrs2("EntryType") = "S" then
					...
					%>
							<td class="ExcelDisplayCell" align="right"><input type="text" readonly name="txtCurrVal<%=sCatCode%>?<%=iBSSubID%>?<%=iBSSubSubID%>" size="15" value="<%=FormatNumber(dSchTot,2,,,0)%>" style="text-align:right" maxlength="13" class="FormElemRead">
					...
					Elseif CStr(sEntTy) = "A" and CStr(iAccHead) <> "0" Then 'If is an Account Head Type
						'Response.Write "<p> " & sOrgID
						'Response.Write "<p> " & iAccHead
						'Response.Write "<p> " & sLastDay
					%>
						<td class="ExcelDisplayCell" align="right">
