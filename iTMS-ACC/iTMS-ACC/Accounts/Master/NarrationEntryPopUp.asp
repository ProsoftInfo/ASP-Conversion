...
function checkSubmit()
	...
	Dim objHttp,Root,NewElem,sRoot
	
	set Root= OutData.documentElement
	set sRoot = RetData.documentElement
	
	if trim(document.formname.txtShortDesc.value)="" then
		MsgBox "Enter Narration Short Description"
		document.formname.txtShortDesc.focus
		exit function
	end if
	if trim(document.formname.txtDesc.value)="" then
		MsgBox "Enter Narration Description"
		document.formname.txtDesc.focus
		exit function
	end if
	
	If Root.hasChildNodes() Then
		For Each Node In Root.childNodes
			Set a = Root.removeChild(Node)
		Next
	End If
	
	set NewElem = OutData.createElement("Desc")
	NewElem.setAttribute("Type"),Trim(document.formname.hType.value)
	NewElem.setAttribute("ShortDesc"),Trim(document.formname.txtShortDesc.value)
	NewElem.setAttribute("Desc"),Trim(document.formname.txtDesc.value)
	NewElem.setAttribute("BookCode"),Trim(document.formname.hSelBookCode.value)
	NewElem.setAttribute("BookNo"),Trim(document.formname.hSelBookNo.value)
	NewElem.setAttribute("NarrNo"),Trim(document.formname.hNarrNo.value)
	Root.appendChild NewElem
	
	'alert(Root.xml)
	'Exit Function
	set objHttp = CreateObject("Microsoft.XMLHTTP")
	objHttp.open "POST","NarrationUpdate.asp",False
	objHttp.send OutData.XMLDocument
	
	If objHttp.responseText <> "" Then
		alert(objHttp.responsetext)
	Else
		If document.formname.hType.value ="N" Then
			alert("Narration Details Inserted Successfully")
		Else
			alert("Narration Details Updated Successfully")
		End IF
...
Function selBook()
	...
	sMode = "M"
	sBookCode = Trim(document.formname.hBookCode.value)
	set Root = OutData.documentElement
	
	Set OutValue = showModalDialog("BookSelectionPopUp.asp?hSelectMode="&sMode&"&BookCode="&sBookCode,BookDet,"dialogHeight:480px;dialogWidth:420px;center:Yes;help:No;resizable:No;status:No")
    ...
    if OutValue.hasChildNodes() then
		For Each Node in OutValue.childNodes 
			If Node.nodeName = "Entry" Then
				iBookCode = iBookCode & "," & Node.getAttribute("RetField0")	'Book Code
				sBookNo	  = sBookNo   & "," & Node.getAttribute("RetField1")	'Book No
				sBookName = sBookName & "," & Node.getAttribute("RetField2")	'Book Name
			End If
		Next
    End IF
    
    If iBookCode <> "" Then 
		iBookCode = Mid(iBookCode,2)
		document.formname.hSelBookCode.value = Trim(iBookCode)
	End IF
    If sBookNo <> "" Then 
		sBookNo = Mid(sBookNo,2)
		document.formname.hSelBookNo.value = Trim(sBookNo)
	End IF
    If sBookName <> "" Then sBookName = Mid(sBookName,2)
    
    UsedInBook.innerText = sBookName
End Function
