(function (window, document) {
	...
	function rowInfo(item) {
		...
		var rowText = voucherText(item) || valueOf("hChkBox" + index);
		return {
	...
	window.DisplayBook = function () {
		...
		xhr = syncGet("XMLGetOrgBook.asp?BkCode=06&orgID=" + encodeURIComponent(unitNo));
		if (xhr.responseXML && xhr.responseXML.documentElement) {
	...
	window.ChkforDelete = function () {
		...
		if (confirm("This will Permanently Delete the Voucher(s)\nClick OK to Delete")) {
			form().action = "VouDNPRDeletion.asp";
			window.GetFormDet();
			form().submit();
		}
		return false;
	...
	window.ChkforEdit = function () {
		...
		var voucherType;
		var actions = {
			OT: "VoudnOtherAmend.asp",
			SI: "VoudnSalInvAmd.asp",
			PR: "VoudnPurReturnAmd.asp",
			OP: "VouDNPurInvAmd.asp"
		};
		if (selected.values.length > 1) {
	...
	window.ChkforAccount = function () {
		...
		if (trim(selected.joined) !== "") {
			setValue("hTransNo", selected.joined);
			statusParts = selected.lastInfo.text.split("&");
			form().action = "AccCNDNAsGJVou.asp?BookNo=" + encodeURIComponent(statusParts[4] || "") + "&Para=D&BookCode=" + encodeURIComponent(statusParts[5] || "");
			window.GetFormDet();
			form().submit();
		}
		return false;
