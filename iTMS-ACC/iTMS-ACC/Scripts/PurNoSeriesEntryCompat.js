(function (window, document) {
	...
	function checked(name, index) {
		var list = fields(name);
		return !!(list[index] && list[index].checked);
	}

	function setChecked(name, index, value) {
		var list = fields(name);
		if (list[index]) {
	...
	function appendExistingSeriesRow(entry, rowIndex, readOnly, numberLength) {
		var row = table("tblBook").insertRow(rowIndex);
		var entryNo = attr(entry, 9) || rowIndex;
		var inputSize = (Number(numberLength) || 0) + 2 || 8;
		...
		if (readOnly) {
			...
			insertCell(row, 2, "txtStartNo" + entryNo, attr(entry, 2), "ExcelInputCell", "", "", inputSize, Number(numberLength) || 8);
	...
	function displayTable() {
		...
		loadNumberSeries("XMLGetNoSeries.asp?sVal=" + encodeURIComponent([field("hSeriesNo") && field("hSeriesNo").value || "", field("hSeriesCode") && field("hSeriesCode").value || "", selectedValue(field("selUnit"))].join(":")), "NoSeries");
		...
		if (existingRows.length) {
			...
			existingRows.forEach(function (entry) {
				appendExistingSeriesRow(entry, rowIndex, field("hDispCheck") && field("hDispCheck").value === "Y", numberLength);
	...
	function numForName() {
		if (checked("chkFor", 0) && checked("chkFor", 1)) {
			return "Both";
		}
		if (checked("chkFor", 0)) {
			return "Domestic";
	...
	function numForCode() {
		var name = numForName();
		return name === "Import" ? "I" : name.charAt(0);
	}
	...
	function numForOverlap(currentCode, oldValue) {
		...
		return (currentCode === "B" && oldCode === "B") ||
			...
			(currentCode === "I" && oldCode === "B") ||
			(currentCode === "B" && oldCode === "D") ||
			(currentCode === "B" && oldCode === "I");
	}

	function itemOverlap(node) {
		var oldType = attrAny(node, ["ItemTy"], 5);
		return oldType === "All" || oldType === "Specific";
	}

	function checkDuplicate() {
		var currentNumFor = numForCode();
		var selectedClass = field("hClassCode") && field("hClassCode").value || "";
		var duplicate = false;
		selectNodes(xmlRoot("SeriesList"), "//NumSeriesList").some(function (node) {
			duplicate = numForOverlap(currentNumFor, attrAny(node, ["NumFor"], 1)) && itemOverlap(node) && trim(selectedClass) === trim(attr(node, "SelClass"));
			return duplicate;
		});
		return !duplicate;
	}
	...
	function formReset() {
		...
		setChecked("chkFor", 0, true);
		setChecked("chkFor", 1, true);
		setValue("hItemType", "");
		setValue("hItemValue", "");
		setValue("hTransNo", "");
		...
		setDisabled("selCategory", false);
		return false;
	}

	function postSeriesAndSubmit() {
		var xhr = syncPost("XMLSave.asp?Name=NoSeries&Mod=PUR", serializeXml("SeriesList"));
	...
	function validateForm(callType) {
		...
		if (category) {
			setValue("hCatCode", selectedValue(category));
		}
		if (callType === "U") {
			if (!amendDuplicate()) {
	...
	function setSeriesNo(transNo) {
		...
		if (attrAny(node, ["NumFor"], 1) === "Both") {
			...
			setChecked("chkFor", 0, true);
			setChecked("chkFor", 1, false);
		} else {
			setChecked("chkFor", 0, false);
			setChecked("chkFor", 1, true);
		}
		...
		setText("txtClass", attr(node, "SelClassName") || attr(node, "SelCatName"));
		displayTable();
	...
	function getSeriesList(callType) {
		...
		clearTable();
		if (callType === "U") {
			loadNumberSeries("GetNumberSeriesListPur.asp?sVal=" + encodeURIComponent(unit + ":" + activity), "SeriesList");
		}
		childElements(xmlRoot("SeriesList"), "NumSeriesList").forEach(function (node, index) {
			...
			insertCell(row, 1, "", attrAny(node, ["ItemDesc", "ItemValue"], 6), "ExcelDisplayCell", "Center", "top");
	...
	function popupDet(obj, type) {
		...
		var url;
		resetSeries();
		if ((selected === "S" || selected === "Y") && type === "I") {
			setValue("hItemType", selected);
			value = selectedValue(field("selUnit")) + ":" + type + ":0";
	...
	window.InsertCellReadOnly = insertCell;
