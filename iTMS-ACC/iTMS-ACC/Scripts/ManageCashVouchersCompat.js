(function (window, document) {
	...
	window.ChkforEdit = function () {
		...
		form().action = "CashVoucher.ASP?Val=" + encodeURIComponent(actionValue) + "&VouTy=" + encodeURIComponent(voucherParts[0] || "");
		window.GetFormDet();
		form().submit();
		return false;
	};
	...
	window.ChkforPrint = function () {
		var selected = collectVouchers(":", true);
		if (trim(selected.joined) !== "") {
			openDialog("CashVouchView_San.asp?TransNo=" + encodeURIComponent(selected.joined), "", "dialogHeight:410px;dialogWidth:670px;center:Yes;help:No;resizable:No;status:No", function () {});
		} else {
	...
	window.RegVoucherNo = function () {
		...
		form().action = "AccVoucherNo_Generate.asp?BookCode=01";
		form().submit();
		return false;
	};
	...
	window.ChkforDelete = function () {
		...
		if (parts.status === "01") {
			...
			form().action = "CASHVOUCHERDELETE.ASP?Val=" + encodeURIComponent(selected.joined);
			window.GetFormDet();
			form().submit();
		}
		return false;
	...
	window.ChkforMove = function () {
		...
		openDialog("ManageVouchers_MoveBookPop.asp?CallFrom=CASH&TransNo=" + encodeURIComponent(selected.joined), "", "dialogHeight:220px;Status:no", function (outValue) {
			...
			form().action = "ManageVouchers_MoveUpdate_forCASH.asp";
			window.GetFormDet();
			form().submit();
		});
		return false;
	...
	window.ChkforEdit = function () {
		...
		parts = selectedVoucherParts(selected);
		if (parts.status === "01") {
			submitVoucherPage("CashVoucher.ASP", selected, parts);
		} else if (parts.status === "04" && userCanChange(parts.status)) {
			submitVoucherPage("AMDCASHVOUCHER.ASP", selected, parts);
		}
		return false;
	};
	...
	window.showBillDetails = function (transNo) {
		var xhr = new XMLHttpRequest();
		xhr.open("POST", "CheckVouAdjDet.asp?BookCode=01&TransNo=" + encodeURIComponent(transNo), false);
		xhr.send(null);
