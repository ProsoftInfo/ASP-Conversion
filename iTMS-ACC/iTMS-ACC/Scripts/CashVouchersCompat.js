(function (window, document) {
	...
	window.ChkforApprove = function () {
		...
		if (trim(valueOf("hCnt")) !== "1" && trim(textParts[2]) === "") {
			alert("Approve is not Possible.Select A/c. Head or Party.");
			return false;
		}
		window.GetFormDet();
		if (trim(selected.lastInfo.frmAppNo) === "2" && trim(selected.lastInfo.purBillType) !== "M") {
			setValue("hAppNo", selected.lastInfo.frmAppNo);
			form().action = "AppOtherCAView.asp?TransNo=" + encodeURIComponent(selected.joined) + "&sPara=App";
			form().submit();
			return false;
		}
	...
	window.ChkforEdit = function () {
		...
		if (trim(selected.lastInfo.frmAppNo) === "2" && trim(selected.lastInfo.purBillType) !== "M") {
			setValue("hAppNo", selected.lastInfo.frmAppNo);
			form().action = "AppOtherCAView.asp?TransNo=" + encodeURIComponent(selected.joined) + "&sPara=Edt";
			form().submit();
			return false;
		}
	...
	window.ChkforAccount = function () {
		...
		if (trim(selected.lastInfo.frmAppNo) === "2" && trim(selected.lastInfo.purBillType) !== "M") {
			setValue("hAppNo", selected.lastInfo.frmAppNo);
			form().action = "AppOtherCAView.asp?TransNo=" + encodeURIComponent(selected.joined) + "&sPara=Acc";
			form().submit();
			return false;
		}
		if (trim(selected.joined) !== "") {
	...
	window.CheckCashAcc = function (transNo) {
		var xhr = syncGet("XMLGetCashStatus.asp?TransNo=" + encodeURIComponent(transNo));
		var value = trim(xhr.responseText);
		var parts;
		if (value === "A:A") {
			return true;
		}
		parts = value.split(":");
		if (parts[0] === "R") {
			alert("Negative Cash Balance Transaction not allowed ");
			return false;
		}
		if (parts[1] === "R") {
			alert("Amount greater than 20,000 Transaction not allowed ");
			return false;
		}
		if (parts[1] === "W" && !confirm("Amount greater than 20,000 Transaction, Do you wnat to Continue")) {
			return false;
		}
		return true;
	};
	...
	window.ShowVouch = function (transNo) {
		openDialog("CashVouchView_San.asp?TransNo=" + encodeURIComponent(transNo), "", "dialogHeight:440px;dialogWidth:650px;center:Yes;help:No;resizable:No;status:No", function () {});
		return false;
	};
	...
	window.Validate = function () {
		...
		if (valueOf("hVouDtFlag") === "VouDate" && from && to && from > to) {
	...
	window.ChkforPrint = function () {
		var selected = collectVouchers(":", true);
		if (trim(selected.joined) !== "") {
			openDialog("PRNCashRecVouView2.asp?Value=" + encodeURIComponent(selected.joined), "", "dialogHeight:200px;dialogWidth:300px;center:Yes;help:No;resizable:No;status:No", function () {});
		} else {
