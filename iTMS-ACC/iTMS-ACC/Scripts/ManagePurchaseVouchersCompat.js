(function (window, document) {
	...
	window.DisplayBook = function () {
		...
		xhr = syncGet("XMLGetOrgBook.asp?BkCode=04&orgID=" + encodeURIComponent(unitNo));
		if (xhr.responseXML && xhr.responseXML.documentElement) {
	...
	window.ChkforEdit = function () {
		...
		form().action = "VouAmdPURBkSel.asp?Val=" + encodeURIComponent(actionValue) + "&VouTy=" + encodeURIComponent(voucherParts[0] || "");
		window.GetFormDet();
		form().submit();
		return false;
	};
	...
	window.ShowVouch = function (transNo) {
		openDialog("PurchaseVouchView_San.asp?TransNo=" + encodeURIComponent(transNo), "", "dialogHeight:410px;dialogWidth:670px;center:Yes;help:No;resizable:No;status:No", function () {});
		return false;
	};
	...
	window.ChkforPrint = function () {
		var selected = collectVouchers(":", true);
		if (trim(selected.joined) !== "") {
			openDialog("PurchaseVouchView_San.asp?TransNo=" + encodeURIComponent(selected.joined), "", "dialogHeight:410px;dialogWidth:670px;center:Yes;help:No;resizable:No;status:No", function () {});
		} else {
	...
	window.RegVoucherNo = function () {
		...
		form().action = "AccVoucherNo_Generate.asp?BookCode=04";
		form().submit();
		return false;
	};
	...
	window.CheckVouForEdit = function () {
		...
		xhr.open("POST", "CheckVouAdjDet.asp?BookCode=04&TransNo=" + encodeURIComponent(selected.joined), false);
		xhr.send(null);
	...
	window.ChkforDelete = function () {
		...
		if (parts.status === "01") {
			...
			form().action = "PURCHASEVOUCHERDELETE.ASP?Val=" + encodeURIComponent(selected.joined);
			window.GetFormDet();
			form().submit();
		}
		return false;
	...
	window.ChkforMove = function () {
		...
		openDialog("ManageVouchers_MoveBookPop.asp?CallFrom=PUR&TransNo=" + encodeURIComponent(selected.joined), "", "dialogHeight:220px;Status:no", function (outValue) {
			...
			form().action = "ManageVouchers_MoveUpdate_forPUR.asp";
			window.GetFormDet();
			form().submit();
		});
		return false;
	...
	window.ChkforEdit = function () {
		...
		setValue("sFlag", "True");
		if (parts.status === "01") {
			form().action = "VouAmdPURBkSel.asp";
		} else if (parts.status === "04" && userCanChange(parts.status)) {
			form().action = "VouAccAmdPURBkSel.asp?TransNo=" + encodeURIComponent(selected.joined) + "&AmdType=A";
		} else {
			return false;
		}
		window.GetFormDet();
	...
	window.DispVal = function (value) {
		alert("Amendment Not allowed \nPurchase Bill is been adjusted with the following Voucher(s)! \n\n" + value);
	};

	window.showBillDetails = function (transNo) {
		var xhr = new XMLHttpRequest();
		xhr.open("POST", "CheckVouAdjDet.asp?BookCode=04&TransNo=" + encodeURIComponent(transNo), false);
		xhr.send(null);
		if (trim(xhr.responseText) === "") {
