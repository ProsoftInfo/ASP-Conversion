(function (window, document) {
	...
	function selectedUnitInfo() {
		var unit = field("selUnitId");
		var unitNo = unit ? unit.value : valueOf("hUnitNo");
		setValue("hUnitNo", unitNo);
		return {
			value: unitNo,
			index: unit ? unit.selectedIndex : ""
		};
	}
	...
	window.DisplayBook = function () {
		var unitInfo = selectedUnitInfo();
		var unitNo = unitInfo.value;
		...
		xhr = syncGet("XMLGetOrgBook.asp?BkCode=05&orgID=" + encodeURIComponent(unitNo));
		if (xhr.responseXML && xhr.responseXML.documentElement) {
	...
	window.ChkforEdit = function () {
		...
		form().action = "AmdSALBkSel.asp?Val=" + encodeURIComponent(actionValue) + "&VouTy=" + encodeURIComponent(voucherParts[0] || "");
		window.GetFormDet();
		form().submit();
		return false;
	};
	...
	window.ChkforAccount = function () {
		...
		if (selected.items.length > 0 && voucherParts[1] === "R" && Number(statusParts[2] || 0) > 0) {
			alert("Only eligible vouchers can be accounted");
			setBtnAcc(false);
			return false;
		}
		if (selected.values.length > 1) {
	...
	window.ShowVouch = function (transNo) {
		openDialog("SalesVouchView_San.asp?TransNo=" + encodeURIComponent(transNo), "", "dialogHeight:410px;dialogWidth:670px;center:Yes;help:No;resizable:No;status:No", function () {});
		return false;
	};
	...
	window.SelectAccHead = function () {
		...
		if (!book || book.value === "S" || book.selectedIndex < 1) {
			alert("Select Book");
			if (book && book.focus) {
				book.focus();
			}
	...
	window.ChkforPrint = function () {
		var selected = collectVouchers(":", true);
		if (trim(selected.joined) !== "") {
			openDialog("SalesVouchView_San.asp?TransNo=" + encodeURIComponent(selected.joined), "", "dialogHeight:410px;dialogWidth:670px;center:Yes;help:No;resizable:No;status:No", function () {});
		} else {
	...
	window.RegVoucherNo = function () {
		...
		form().action = "AccVoucherNo_Generate.asp?BookCode=05";
		form().submit();
		return false;
	};

	function manageCriteria() {
		var checked = fields("OptCriteria").filter(function (item) {
			return item.checked;
		})[0];
		return checked ? checked.value : "";
	}
	...
	function selectedVoucher(separator) {
		return collectVouchers(separator, true);
	}

	function selectedVoucherParts(selected) {
		var textParts = selected.lastInfo.text.split("&");
		var detailParts = String(textParts[1] || "").split("@");
		return {
			number: textParts[0] || "",
			type: detailParts[0] || "",
	...
	function userCanChange(status) {
		if (status === "04" && typeof window.CheckUserType === "function") {
			return !!window.CheckUserType(valueOf("hUserType"), status);
		}
		return true;
	}

	function voucherActionValue(selected, parts) {
		return selected.joined + "~" + parts.number + "~A";
	}

	function submitVoucherPage(page, selected, parts) {
		setValue("hTransNo", selected.joined);
		form().action = page + "?Val=" + encodeURIComponent(voucherActionValue(selected, parts)) + "&VouTy=" + encodeURIComponent(parts.type);
		window.GetFormDet();
		form().submit();
	}

	window.OptSelection = function () {
		var criterion = manageCriteria();
		sFlag = criterion;
		setValue("hFlag", criterion);

		if (criterion === "VouNo") {
			setValue("txtFromAmount", "");
			setValue("txtToAmount", "");
			setReadOnly("txtVouNoFrom", false);
			setReadOnly("txtVouNoTo", false);
			setReadOnly("txtFromAmount", true);
			setReadOnly("txtToAmount", true);
			setDisabled("selAccHead", true);
		} else if (criterion === "VouDate") {
			["txtVouNoFrom", "txtVouNoTo", "txtFromAmount", "txtToAmount"].forEach(function (name) {
				setValue(name, "");
				setReadOnly(name, true);
			});
			setDisabled("selAccHead", true);
		} else if (criterion === "VouAmount") {
			setValue("txtVouNoFrom", "");
			setValue("txtVouNoTo", "");
			setReadOnly("txtVouNoFrom", true);
			setReadOnly("txtVouNoTo", true);
			setReadOnly("txtFromAmount", false);
			setReadOnly("txtToAmount", false);
			setDisabled("selAccHead", true);
		} else if (criterion === "AccHead") {
			setValue("txtVouNoFrom", "");
			setValue("txtVouNoTo", "");
			setValue("txtFromAmount", "");
			setValue("txtToAmount", "");
	...
	window.Validate = function () {
		...
		if (criterion === "VouDate" && from && to && from > to) {
			alert("To Date Should be Greater than From Date");
			return false;
		}
		if (criterion === "VouNo") {
	...
	window.ChkReset = function () {
		...
		["txtFromAmount", "txtToAmount", "txtVouNoFrom", "txtVouNoTo", "txtAccHead", "hFlag", "hUnitNo"].forEach(function (name) {
			setValue(name, "");
		});
		sFlag = "";
		window.DisplayBook();
	...
	window.SetDate = function () {
		...
		var acc;
		ensureCompat();
		if (unit && trim(valueOf("hUnitNo")) !== "") {
			Array.prototype.slice.call(unit.options).some(function (option, index) {
				if (option.value === valueOf("hUnitNo")) {
	...
	window.GetFormDet = function () {
		var book = field("selBook");
		var unitInfo = selectedUnitInfo();
		var criterion = manageCriteria() || "0";
		var formValue = unitInfo.value;
		formValue += "|" + unitInfo.index;
		formValue += "|" + (book ? book.value : "");
		formValue += "|" + (book ? book.selectedIndex : "");
		formValue += "|" + getDateControl("ctlVouFromDate");
		formValue += "|" + getDateControl("ctlVouToDate");
	...
	window.CheckVouForEdit = function () {
		...
		xhr.open("POST", "CheckVouAdjDet.asp?BookCode=05&TransNo=" + encodeURIComponent(selected.joined), false);
		xhr.send(null);
	...
	window.ChkforDelete = function () {
		var selected = selectedVoucher("~");
		var parts = selectedVoucherParts(selected);
		if (trim(selected.joined) === "") {
			alert("Select Voucher");
			return false;
		}
		if (!userCanChange(parts.status)) {
			return false;
		}
		...
		if (parts.status === "01") {
			...
			form().action = "SALESVOUCHERDELETE.ASP?Val=" + encodeURIComponent(selected.joined);
			window.GetFormDet();
			form().submit();
		}
		return false;
	...
	window.ChkforMove = function () {
		var selected = selectedVoucher("~");
		var parts;
		if (!requireSingle(selected, "Edit One by One")) {
			return false;
		}
		parts = selectedVoucherParts(selected);
		if (!userCanChange(parts.status)) {
			return false;
		}
		openDialog("ManageVouchers_MoveBookPop.asp?CallFrom=SAL&TransNo=" + encodeURIComponent(selected.joined), "", "dialogHeight:220px;Status:no", function (outValue) {
			var root = dialogNode(outValue);
			var bookNode;
			if (trim(attr(root, "Done")).toUpperCase() !== "Y") {
				return;
			}
			bookNode = root && root.getElementsByTagName ? root.getElementsByTagName("Book")[0] : null;
			setValue("hTransNo", selected.joined);
			setValue("hNewBookNo", attr(bookNode, "NewBookNo"));
			setValue("hNewBookAccHead", attr(bookNode, "NewBookAccHead"));
			form().action = "ManageVouchers_MoveUpdate_forSAL.asp";
			window.GetFormDet();
			form().submit();
		});
		return false;
	...
	window.ChkforEdit = function () {
		...
		parts = selectedVoucherParts(selected);
		if (!window.CheckVouForEdit()) {
			return false;
		}
		setValue("hTransNo", selected.joined);
		setValue("sFlag", "True");
		if (parts.status === "01") {
			form().action = "AmdSALBkSel.asp";
		} else if (parts.status === "04" && userCanChange(parts.status)) {
			form().action = "AmdAccSALBkSel.asp";
		} else {
			return false;
		}
		window.GetFormDet();
	...
	window.DispVal = function (value) {
		alert("Sales Bill is been adjusted with the following Voucher(s)! \n Amendment Not allowed \n " + value);
	};

	window.showBillDetails = function (transNo) {
		var xhr = new XMLHttpRequest();
		xhr.open("POST", "CheckVouAdjDet.asp?BookCode=05&TransNo=" + encodeURIComponent(transNo), false);
		xhr.send(null);
		if (trim(xhr.responseText) === "") {
			alert("This Voucher is Not been Adjusted with any Bills ");
		} else {
			window.DispVal(trim(xhr.responseText));
		}
		return false;
	...
	window.showAdjDet = function (transNo) {
		openDialog("BillAdjustedDetails.asp?CrTransNo=" + encodeURIComponent(transNo), "", "dialogHeight:400px;dialogWidth:590px;center:Yes;help:No;resizable:No;status:No", function () {});
		return false;
	};

	window.ChkAdjAdvance = function () {
		var selected = selectedVoucher("~");
		var parts;
		var actionValue;
		if (!requireSingle(selected, "Select any one voucher")) {
			return false;
		}
		parts = selectedVoucherParts(selected);
		if (!userCanChange(parts.status)) {
		...
		setValue("hTransNo", selected.joined);
		setValue("sFlag", "True");
		actionValue = selected.joined + "~" + parts.number + "~A";
		form().action = "AdjustPurchaseVoucherForPayRec.asp?Val=" + encodeURIComponent(actionValue) + "&VouTy=" + encodeURIComponent(parts.type);
		form().submit();
		return false;
	};

	window.ChkforCancel = function () {
		alert("Cancel is not available from this page.");
		return false;
	};

	window.ChkforReverse = function () {
		alert("Reverse is not available from this page.");
		return false;
	};
