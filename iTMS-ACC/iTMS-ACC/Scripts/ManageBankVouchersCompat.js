(function (window, document) {
	...
	window.OptSelection = function () {
		...
		if (criterion === "VouNo") {
			...
			setValue("txtFromChqNo", "");
			setValue("txtToChqNo", "");
			setReadOnly("txtVouNoFrom", false);
			setReadOnly("txtVouNoTo", false);
			setReadOnly("txtFromAmount", true);
			setReadOnly("txtToAmount", true);
			setReadOnly("txtFromChqNo", true);
			setReadOnly("txtToChqNo", true);
			setDisabled("selAccHead", true);
		} else if (criterion === "VouDate") {
			["txtVouNoFrom", "txtVouNoTo", "txtFromAmount", "txtToAmount", "txtFromChqNo", "txtToChqNo"].forEach(function (name) {
				setValue(name, "");
			...
			setReadOnly("txtFromAmount", false);
			setReadOnly("txtToAmount", false);
			setReadOnly("txtFromChqNo", false);
			setReadOnly("txtToChqNo", false);
			setDisabled("selAccHead", true);
	...
	window.ChkReset = function () {
		...
		["txtFromAmount", "txtToAmount", "txtVouNoFrom", "txtVouNoTo", "txtAccHead", "txtFromChqNo", "txtToChqNo", "hFlag"].forEach(function (name) {
			setValue(name, "");
		});
		sFlag = "";
		window.DisplayBook();
	...
	window.chkForAdjust = function () {
		var selected = selectedVoucher("~");
		var parts;
		if (!requireSingle(selected, "Adjust One by One")) {
			return false;
		}
		...
		submitVoucherPage("AdjustVoucherWithBill.ASP", selected, parts);
		return false;
	};
	...
	window.ChkforDelete = function () {
		...
		if (parts.status === "01") {
			...
			form().action = "BANKVOUCHERDELETE.ASP?Val=" + encodeURIComponent(selected.joined);
			window.GetFormDet();
			form().submit();
		}
		return false;
	...
	window.ChkforMove = function () {
		...
		openDialog("ManageVouchers_MoveBookPop.asp?CallFrom=BANK&TransNo=" + encodeURIComponent(selected.joined), "", "dialogHeight:220px;Status:no", function (outValue) {
			...
			form().action = "ManageVouchers_MoveUpdate_forBANK.asp";
			window.GetFormDet();
			form().submit();
		});
		return false;
	...
	window.ChkforEdit = function () {
		...
		parts = selectedVoucherParts(selected);
		if (parts.status === "01") {
			submitVoucherPage("BankVoucher.ASP", selected, parts);
		} else if (parts.status === "04" && userCanChange(parts.status)) {
			submitVoucherPage("AMDBANKVOUCHER.ASP", selected, parts);
		}
		return false;
	};

	window.ShowVouch = function (transNo) {
		openDialog("BankVouchView_San.asp?TransNo=" + encodeURIComponent(transNo), "", "dialogHeight:410px;dialogWidth:670px;center:Yes;help:No;resizable:No;status:No", function () {});
		return false;
	};
	...
	window.showBillDetails = function (transNo) {
		var xhr = new XMLHttpRequest();
		xhr.open("POST", "CheckVouAdjDet.asp?BookCode=02&TransNo=" + encodeURIComponent(transNo), false);
		xhr.send(null);
