(function (window, document) {
	...
	window.DisplayBook = function () {
		...
		xhr = syncGet("XMLGetOrgBook.asp?BkCode=08&orgID=" + encodeURIComponent(unitNo));
		if (xhr.responseXML && xhr.responseXML.documentElement) {
	...
	window.ChkforEdit = function () {
		...
		form().action = "JournalVoucher.asp?Val=" + encodeURIComponent(actionValue) + "&VouTy=" + encodeURIComponent(voucherParts[0] || "");
		window.GetFormDet();
		form().submit();
		return false;
	};
	...
	window.ShowVouch = function (transNo) {
		openDialog("GJVouchView_San.asp?TransNo=" + encodeURIComponent(transNo), "", "dialogHeight:390px;dialogWidth:640px;center:Yes;help:No;resizable:No;status:No", function () {});
		return false;
	};
