(function (window, document) {
	...
	function parseDate(value) {
		...
		parts = text.split(/[\/\-]/);
		if (parts.length >= 3) {
			var month = isNaN(Number(parts[1])) ? monthNames[String(parts[1]).substring(0, 3).toLowerCase()] : Number(parts[1]) - 1;
	...
	function importIntoOldVoucher(node, beforeNode) {
		var root = xmlRoot("OldVouData");
		var doc = xmlDocument("OldVouData");
		var imported;
		if (!root || !node) {
			return null;
		}
		imported = doc && node.ownerDocument !== doc ? doc.importNode(node, true) : node;
		if (beforeNode && beforeNode.parentNode) {
			beforeNode.parentNode.insertBefore(imported, beforeNode);
		} else {
	...
	window.VouCreate = function () {
		...
		var currentDate = valueOf("hCurrDate");
		var systemDate = parseDate(currentDate);
		var selectedDate = parseDate(invoiceDate);
		var saleType = field("selSaleType");
		var book = field("selBook");
		var partyType = field("selPartyType");
		...
		if (trim(valueOf("txtInvoiceNo")) === "") {
			...
			if (field("txtInvoiceNo")) {
				field("txtInvoiceNo").focus();
			}
			return false;
		}
		if (selectedDate && systemDate && selectedDate > systemDate) {
			alert("Voucher Date Should be Less than the System Date ");
			return false;
		}

		setValue("hInvDate", invoiceDate);
		setValue("hOrgName", valueOf("horgNameN", valueOf("hOrgNameN")));
		setValue("hSalType", selectedText(saleType));

		node = firstNode(root, "//Organization");
		if (node) {
			setAttr(node, "OrgId", valueOf("horgID", valueOf("hOrgID")));
			node.textContent = valueOf("horgNameN", valueOf("hOrgNameN"));
		}
	...
	window.showAgent = function (flag) {
		...
		openDialog("AgentCommisionEntry.asp?OrgID=" + encodeURIComponent(valueOf("horgID", valueOf("hOrgID"))) + "&AgentType=" + encodeURIComponent(flag), xmlObject("OutData"), "dialogHeight:400px;dialogWidth:550px;center:Yes;help:No;resizable:No;status:No", function (returnValue) {
			var returnedRoot = xmlRoot(returnValue) || returnValue && returnValue.nodeType === 1 && returnValue || null;
			var header = firstNode(root, "//Voucher/Header") || firstNode(root, "//Header");
			if (returnedRoot && childElements(returnedRoot).length) {
				importIntoOldVoucher(returnedRoot, header);
	...
	window.SaveXML = function () {
		var xhr = syncPost("XMLSave.asp?Mod=SAL&Name=Voucher AMD", serializeXml("OldVouData"));
	...
	window.PopulateSalTy = function () {
		...
		xhr = syncGet("XMLGetBookSalPurType.asp?BkCode=05&orgID=" + encodeURIComponent(orgId) + "&BookNo=" + encodeURIComponent(bookNo));
		if (xhr.responseXML && xhr.responseXML.documentElement) {
	...
	window.DispOldVal = function () {
		...
		if (node) {
			value = attr(node, "BookId");
			setValue("hBkAccHead", attr(node, "BKAccHead"));
			select = field("selBook");
			if (select) {
		...
		if (agentFlag === "Y") {
			node = firstNode(root, "//AgentDetails/Agent");
			if (node) {
				agentType = attr(node, "PartyType");
				agentName = attr(node, "Agentname");
			}
		}
		setText("spAgentName", agentName);
		setValue("hCommName", agentName);
		if (agentType === "CR" && fields("optAgentExist")[0]) {
			fields("optAgentExist")[0].checked = true;
		} else if (agentFlag === "DR" && fields("optAgentExist")[1]) {
			fields("optAgentExist")[1].checked = true;
		} else if (fields("optAgentExist")[2]) {
			fields("optAgentExist")[2].checked = true;
		}
		node = firstNode(root, "//SaleInvoice");
		if (node) {
			setValue("txtInvoiceNo", attr(node, "InvNo"));
			setDateControl("ctlDate", attr(node, "InvDate"));
			setValue("txtRefNo", attr(node, "RefNo"));
		}
		node = firstNode(root, "//SalesType");
		if (node && field("selSaleType")) {
			Array.prototype.some.call(field("selSaleType").options, function (option, index) {
				if (String(option.value) === String(attr(node, "SalType"))) {
					field("selSaleType").selectedIndex = index;
					return true;
				}
				return false;
			});
